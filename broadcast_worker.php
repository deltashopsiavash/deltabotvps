...
// --- HTTP worker security (prevents random public triggering) ---
// Allow CLI always. For HTTP, require a valid key.
// Key is derived from current bot token + admin id to be stable per bot instance.
if (php_sapi_name() !== 'cli') {
    $givenKey = $_GET['k'] ?? '';
    $tokenToUse = $GLOBALS['botToken'] ?? ($botToken ?? '');
    $adminToUse = (string)($GLOBALS['admin'] ?? ($admin ?? ''));
    $expectedKey = hash('sha256', $tokenToUse . '|' . $adminToUse);
