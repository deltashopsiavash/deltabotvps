...
// PasarGuard: warn exactly once when volume/date ends, allow 3 days for renewal, then delete.
// The global switch lives in BOT_STATES as pgExpiryAlertsState. Per-user switch is pg_expiry_alerts.
$pgGlobalAlerts = (($botState['pgExpiryAlertsState'] ?? 'on') === 'on');
$sql="SELECT o.*,COALESCE(u.pg_renew_suggestion,1) AS pg_renew_suggestion,COALESCE(u.pg_expiry_alerts,1) AS pg_expiry_alerts FROM orders_list o JOIN server_config s ON s.id=o.server_id LEFT JOIN users u ON u.userid=o.userid WHERE o.status=1 AND s.type='pasarguard' ORDER BY o.id ASC LIMIT 300";
...
if($res){
 while($order=$res->fetch_assoc()){
  ...
  if($ended && (int)$order['expired_warned_at']===0){
    ...
    // Atomic deletion claim prevents duplicate delete reports from overlapping cron runs.
    $oldDue=(int)$order['delete_after'];
    $claim=-1;
    $stmt=$connection->prepare("UPDATE orders_list SET delete_after=? WHERE id=? AND delete_after=?");
    $stmt->bind_param('iii',$claim,$order['id'],$oldDue);
    $stmt->execute();
    $claimed=($stmt->affected_rows===1);
    $stmt->close();
    if(!$claimed) continue;
    ...
    if($del!==null && (!is_object($del) || !isset($del->success) || $del->success!==false)){
      ...
      // Deletion failed; release the claim so a later cron run can retry safely.
