...
$sql="SELECT o.*,u.pg_renew_suggestion,u.pg_expiry_alerts FROM orders_list o JOIN server_config s ON s.id=o.server_id LEFT JOIN users u ON u.userid=o.userid WHERE o.status=1 AND s.type='pasarguard' ORDER BY o.id ASC LIMIT 300";
...
if($res){
 while($order=$res->fetch_assoc()){
  ...
  if($ended && (int)$order['expired_warned_at']===0){
    ...
    if((int)($order['pg_expiry_alerts'] ?? 1)===1){
