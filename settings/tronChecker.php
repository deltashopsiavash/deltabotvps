...
function __runTronChecker(){
    global $connection, $paymentKeys, $botState, $botUrl, $admin;
    ...
    if($success == "SUCCESS" && isset($success)){
        ...
        if($dayStamp > $dateDiff && $wallet == $to_address){
            if($amount >= $tronPrice && $amount <= $tronPrice+1 ){
                ...
                // اگر کاربر استثنا شده باشد، تایید خودکار انجام نشود
                $noAuto = getSettingValue("USER_NO_AUTOAPPROVE_" . (int)$user_id, "0");
                if($noAuto == "1"){
                ...
                if($payType == "BUY_SUB"){
                    ...
                    for($i =1; $i<= $accountCount; $i++){
                        ...
                        if(is_null($response)){
                            ...
                            sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id توسط درگاه ترون اضافه شد میخواست کانفیگ بخره، اتصال به سرور برقرار نبود", null, null);                
                    	...
                    	if($response == "inbound not Found"){
                            ...
                            sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id توسط درگاه ترون اضافه شد میخواست کانفیگ بخره، ولی انباند پیدا نشد", null, null);                
                    	...
                    	if(!$response->success){
                            ...
                            sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ولی خطا داد", null, null);                
                            exit;
                        }
                ...
                🌐 subscription : <code>$subLink</code>
                	        ...
                	        $backgroundImage = imagecreatefromjpeg(getQrBackgroundPathForLoad());
                ...
                elseif($payType == "RENEW_ACCOUNT"){
                	...
                	if(is_null($response)){
                        ...
                        sendToAdmins("✅ مبلغ " . number_format($price) . " تومان ($tronPrice ترون) به کیف پول کاربر $user_id اضافه شد، میخواست کانفیگش رو تمدید کنه، ولی اتصال به سرور برقرار نبود", null, null);
                		exit;
                	}
                ...
                exit;
                        ...
                        sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست زمان سرویسشو افزایش بده", null, null);
                        exit;
                    }
                }
                ...
                exit;
                        ...
                        sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست حجم کانفیگشو افزایش بده", null, null);                
                ...
                elseif($payType == "RENEW_SCONFIG"){
                	...
                	if(is_null($response)){
                        ...
                        sendToAdmins("✅ مبلغ " . number_format($price) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست کانفیگشو تمدید کنه", null, null);                
                ...
                sendToAdmins(str_replace(["TYPE", "USERID", "TAXID", "USERNAME", "NAME"], [$payDescription, $user_id, $hash_id, $userInfo['username'], $userInfo['name']], $mainValues['partially_paid_user_taxid']), null, "html");
            ...
            sendToAdmins(str_replace(["TYPE", "USERID", "TAXID", "USERNAME", "NAME"], [$payDescription, $user_id, $hash_id, $userInfo['username'], $userInfo['name']], $mainValues['incorrect_user_taxid_rejected']), null, "html");
        ...
        if($state >= 5){
            sendToAdmins(str_replace(["TYPE", "USERID", "TAXID", "USERNAME", "NAME"], [$payDescription, $user_id, $hash_id, $userInfo['username'], $userInfo['name']], $mainValues['user_taxid_rejected']), null, "html");
...
// Run for mother bot (current DB/token)
__runTronChecker();

// Run for active reseller bots as well (so auto-approve works on child bots)
try{
    if(function_exists('ensureResellerTables')){
        ensureResellerTables();
        $origConn = $GLOBALS['connection'] ?? null;
        $origBotToken = $GLOBALS['botToken'] ?? null;
        $origAdmin = $GLOBALS['admin'] ?? null;
        $origDbName = $GLOBALS['dbName'] ?? null;

        $res = $origConn ? $origConn->query("SELECT `id`,`bot_token`,`admin_userid`,`db_name` FROM `reseller_bots` WHERE `status`=1") : null;
        if($res){
            while($r = $res->fetch_assoc()){
                $childToken = $r['bot_token'];
                $childAdmin = (int)$r['admin_userid'];
                $childDb = $r['db_name'];
                if(empty($childToken) || empty($childDb)) continue;

                // switch connection
                $childConn = new mysqli('localhost', $GLOBALS['dbUserName'], $GLOBALS['dbPassword'], $childDb);
                if($childConn->connect_error) continue;
                $childConn->set_charset('utf8mb4');

                $GLOBALS['connection'] = $childConn;
                $GLOBALS['dbName'] = $childDb;
                $GLOBALS['botToken'] = $childToken;
                $GLOBALS['admin'] = $childAdmin;
                ...
                @mysqli_close($childConn);
            }
        }

        // restore
        if($origConn){
            $GLOBALS['connection'] = $origConn;
            $connection = $origConn;
        }
        if($origBotToken !== null){ $GLOBALS['botToken'] = $origBotToken; $botToken = $origBotToken; }
        if($origAdmin !== null){ $GLOBALS['admin'] = $origAdmin; $admin = $origAdmin; }
        if($origDbName !== null){ $GLOBALS['dbName'] = $origDbName; $dbName = $origDbName; }
    }
}catch(Exception $e){
    // ignore
}
