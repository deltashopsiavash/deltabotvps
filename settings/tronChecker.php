    ...
    if($success = "SUCCESS" && isset($success)){
        ...
        if($dayStamp > $dateDiff && $wallet == $to_address){
            if($amount >= $tronPrice && $amount <= $tronPrice+1 ){
                ...
                $offerPaymentError='';
                if($payType === 'BUY_SUB' && !timedOfferEnsurePayReservation($payParam,900,$offerPaymentError)){
                    $stmt = $connection->prepare("UPDATE `pays` SET `state`='need_admin' WHERE `id`=?");
                    $stmt->bind_param('i',$rowId); $stmt->execute(); $stmt->close();
                    sendToAdmins("⚠️ پرداخت ترون تأیید شد اما رزرو پیشنهاد نیاز به بررسی دارد\nفاکتور: <code>".timedOfferEsc($payParam['hash_id'])."</code>\nخطا: ".timedOfferEsc($offerPaymentError),null,'HTML');
                    sendMessage('پرداخت شما تأیید شد و برای بررسی پیشنهاد ویژه به مدیر ارسال شد.',null,null,$user_id);
                    continue;
                }
                ...
                if($payType == "BUY_SUB"){
                    ...
                    $accountCount = function_exists('xuiResolvePayAccountCount') ? xuiResolvePayAccountCount($payParam) : (($payParam['agent_count'] ?? 0) != 0 ? (int)$payParam['agent_count'] : 1);
                ...
                xuiBuildOrderLinksText($botState, $botUrl, $uniqid, $subLink);
                	        ...
                	        $bgPath = "qrcodes/qr_main.jpg";
if(isset($GLOBALS['currentBotInstanceId']) && (int)$GLOBALS['currentBotInstanceId'] > 0){
    $bgPath = "qrcodes/qr_rb" . (int)$GLOBALS['currentBotInstanceId'] . ".jpg";
}
...
$backgroundImage = imagecreatefromjpeg($bgPath);
                    ...
                    $offerCompleteError='';
                    if(!timedOfferCompletePay($payParam,$offerCompleteError)){
                        error_log('Timed offer completion failed for '.$payParam['hash_id'].': '.$offerCompleteError);
