            ...
            if($amount >= $tronPrice && $amount <= $tronPrice+1 ){
                ...
                if($payType == "BUY_SUB"){
                    ...
                    for($i =1; $i<= $accountCount; $i++){
                        ...
                        if($serverType == "marzban"){
                            $uniqid = $token = extractSubscriptionToken($response->sub_link);
                            $subLink = getOrderSubscriptionLinkByServer($server_id, $token);
                            $vraylink = [$subLink];
                            $vray_link = json_encode($response->vray_links);
                        }
                        else{
                            $vraylink = getConnectionLink($server_id, $uniqid, $protocol, $remark, $port, $netType, $inbound_id, $rahgozar, $customPath, $customPort, $customSni);
                            $token = extractSubscriptionToken($response->sub_link ?? '');
                            $subLink = getOrderSubscriptionLinkByServer($server_id, $token);
