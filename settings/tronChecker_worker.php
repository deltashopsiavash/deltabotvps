        ...
        if($dayStamp > $dateDiff && $wallet == $to_address){
            if($amount >= $tronPrice && $amount <= $tronPrice+1 ){
                ...
                🌐 subscription : <code>$subLink</code>
                	        ...
                	        // Pick QR background image (supports per-reseller bot image)
$baseDir = __DIR__; // settings directory
$bgPath = $baseDir . "/qrcodes/qr_main.jpg";
if(isset($GLOBALS['currentBotInstanceId']) && (int)$GLOBALS['currentBotInstanceId'] > 0){
    $candidate = $baseDir . "/qrcodes/qr_rb" . (int)$GLOBALS['currentBotInstanceId'] . ".jpg";
    if(file_exists($candidate)){
        $bgPath = $candidate;
    }
}
if(!file_exists($bgPath)){
    $bgPath = $baseDir . "/QRCode.jpg";
}
