...
// --- run reseller rewardReport via bid (so auto-accept / reports work on reseller DBs too) ---
if(isset($isChildBot) && !$isChildBot){
    if(function_exists('ensureResellerTables')){
        ensureResellerTables();
        $ctx = stream_context_create(['http'=>['timeout'=>6]]);
        $stmt = $connection->prepare("SELECT `id` FROM `reseller_bots` WHERE `status`=1 AND (`db_name` IS NOT NULL AND `db_name`!='')");
        if($stmt){
            $stmt->execute();
            $res = $stmt->get_result();
            $stmt->close();
            ...
            if($base !== ''){
                while($row = $res->fetch_assoc()){
                    $rid = (int)$row['id'];
                    if($rid <= 0) continue;
                    // Call same script with bid param; child request will NOT re-loop.
                    @file_get_contents($base . "/settings/rewardReport.php?bid=" . $rid, false, $ctx);
                }
            }
        }
    }
