<?php
// RID mode for reseller cron: php rewardReport.php rid=123
$ridMode = 0;
if(php_sapi_name()==='cli' && isset($argv) && is_array($argv)){
    foreach($argv as $a){
        if(preg_match('/^rid=(\d+)$/', $a, $m)){
            $ridMode = (int)$m[1];
            if($ridMode>0){
                $_GET['bid'] = $ridMode; // let config.php switch token+db
            }
        }
    }
}
...
// RUN_FOR_ALL_RESELLERS: when called without rid=, process mother + all reseller bots
if(php_sapi_name()==='cli' && empty($ridMode)){
    try{
        ensureResellerTables();
        $res = $connection->query("SELECT `id` FROM `reseller_bots` WHERE `status`=1");
        if($res){
            $self = __FILE__;
            while($rb = $res->fetch_assoc()){
                $rid = (int)$rb['id'];
                if($rid<=0) continue;
                // Run the same script in rid= mode so all original logic (including BUY_SUB fulfillment) is applied.
                $cmd = 'php ' . escapeshellarg($self) . ' rid=' . (int)$rid . ' >/dev/null 2>&1';
                @shell_exec($cmd);
            }
        }
    }catch(Exception $e){
