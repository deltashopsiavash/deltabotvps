...
function saveBotStatesSnapshot($botState){
    global $connection;
...
if($rewaredTime>0 && $rewaredChannel != null){
    ...
    if(time() > $lastTime){
        ...
        saveBotStatesSnapshot($botState);
...
if(($botState['dailySalesReportState'] ?? 'off') == 'on'){
    $lastDailyReport = (int)($botState['lastDailySalesReport'] ?? 0);
    if(time() > $lastDailyReport){
        $since = time() - (24 * 60 * 60);

        $stmt = $connection->prepare("SELECT COUNT(*) AS c, COALESCE(SUM(price),0) AS total FROM `pays` WHERE `request_date` >= ? AND (`state` = 'paid' OR `state` = 'approved' OR `state` = 'paid_with_wallet')");
        $stmt->bind_param('i', $since);
        $stmt->execute();
        $sales = $stmt->get_result()->fetch_assoc();
        $stmt->close();

        $stmt = $connection->prepare("SELECT COUNT(*) AS c FROM `orders_list` WHERE `date` >= ?");
        $stmt->bind_param('i', $since);
        $stmt->execute();
        $orders = (int)($stmt->get_result()->fetch_assoc()['c'] ?? 0);
        $stmt->close();

        $stmt = $connection->prepare("SELECT COUNT(*) AS c FROM `users` WHERE `date` >= ?");
        $stmt->bind_param('i', $since);
        $stmt->execute();
        $newUsers = (int)($stmt->get_result()->fetch_assoc()['c'] ?? 0);
        $stmt->close();

        $amount = number_format((int)($sales['total'] ?? 0));
        $salesCount = (int)($sales['c'] ?? 0);

        $txt = "📊 گزارش فروش 24 ساعت گذشته\n\n💰 مبلغ فروش: {$amount} تومان\n🛒 تعداد پرداخت موفق: {$salesCount}\n📦 تعداد سفارش ساخته شده: {$orders}\n👤 کاربران جدید: {$newUsers}";
        sendToAdmins($txt, null, null);

        $botState['lastDailySalesReport'] = time() + (24 * 60 * 60);
        saveBotStatesSnapshot($botState);
    }
}
