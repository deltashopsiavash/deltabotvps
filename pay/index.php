...
if(mysqli_num_rows($payInfo)==0){
    ...
    // Special-offer invoices are intentionally wallet-only. Reject even old
    // gateway URLs which may still exist in a user's Telegram history.
    if((int)($payParam['special_offer_id'] ?? 0)>0){
        showForm('پرداخت پیشنهاد امروز فقط از موجودی کیف پول داخل ربات امکان‌پذیر است.');
        exit();
    }
    // The gateway must be opened during the original five-minute reservation.
    // Once opened successfully, allow enough time for the provider callback.
    if(!specialOfferReservationIsValid($payParam,true)){
        showForm('زمان رزرو پنج‌دقیقه‌ای این پیشنهاد تمام شده است؛ لطفاً فاکتور جدید بسازید.');
        exit();
    }
    specialOfferExtendReservation($payParam,1200);
