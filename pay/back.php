...
if($payType === 'BUY_SUB' && !timedOfferEnsurePayReservation($payParam, 900, $offerPaymentError)){
    ...
    $stmt->bind_param('i',$payRowId); $stmt->execute(); $stmt->close();
    sendToAdmins("⚠️ پرداخت اینترنتی تأیید شد اما رزرو پیشنهاد نیاز به بررسی دارد\nفاکتور: <code>".timedOfferEsc($payParam['hash_id'])."</code>\nخطا: ".timedOfferEsc($offerPaymentError),null,'HTML');
    sendMessage('پرداخت شما تأیید شد و برای بررسی پیشنهاد ویژه به مدیر ارسال شد.',null,null,$user_id);
    showForm('پرداخت تأیید شد؛ سفارش برای بررسی پیشنهاد ویژه به مدیریت ارسال شد.',$payDescription,true);
    return;
}
...
if($payType == "BUY_SUB"){
    ...
    if($acount == 0 and $inbound_id != 0){
        ...
        sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ظرفیت پر بود", null, null);                
    ...
    for($i =1; $i<= $accountCount; $i++){
        ...
        if(is_null($response)){
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
            sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، اتصال به سرور برقرار نبود", null, null);                
    	...
    	if($response == "inbound not Found"){
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
            sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ولی انباند پیدا نشد", null, null);                
    	...
    	if(!$response->success){
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
            sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ولی خطا داد", null, null);                
            exit;
        }
        
        if($serverType == "marzban"){
            $uniqid = $token = str_replace("/sub/", "", $response->sub_link);
            $subLink = $botState['subLinkState'] == "on"?xuiResolveClientSubLink($server_id, $panelUrl, $response->sub_link ?? '', $inbound_id, $uniqid, $remark):"";
            $vraylink = [$subLink];
...
🌐 subscription : <code>$subLink</code>
	        ...
	        $bgPath = "../settings/qrcodes/qr_main.jpg";
                if(isset($GLOBALS['currentBotInstanceId']) && (int)$GLOBALS['currentBotInstanceId'] > 0){
                    $bgPath = "../settings/qrcodes/qr_rb" . (int)$GLOBALS['currentBotInstanceId'] . ".jpg";
                }
...
$qrImage = imagecreatefrompng($file);
        	...
        	sendPhoto($file, $acc_text,json_encode(['inline_keyboard'=>[[['text'=>"صفحه اصلی 🏘",'callback_data'=>"mainMenu"]]]]),"HTML", $user_id);
            unlink($file);
        }
        $date = time();
    ...
    $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"خرید از درگاه $gateType 💞",'callback_data'=>'deltach'],
            ],
        ]]);
sendToAdmins("
👨‍👦‍👦 خرید ( درگاه $gateType )
...
elseif($payType == "RENEW_ACCOUNT"){
	...
	if(is_null($response)){
        ...
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
        sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست کانفیگش رو تمدید کنه، ولی اتصال به سرور برقرار نبود", null, null);
		exit;
	}
...
exit;
        ...
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
        sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست زمان سرویسشو افزایش بده", null, null);
        exit;
    }
}
...
exit;
        ...
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " تومان به حساب شما اضافه شد",null,null,$user_id);
        sendToAdmins("✅ مبلغ " . number_format($amount) . " تومان به کیف پول کاربر $user_id اضافه شد، میخواست حجم کانفیگشو افزایش بده", null, null);                
