...
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/deltabotvps/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'delta'") ]]; then
        ...
        file_path="/var/www/html/deltabotvps/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/deltabotvps/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/deltabotvps/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/deltabotvps/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/deltabotvps/baseInfo.php
        ...
        # ---------------- VPSBot (Cloud Server) install + bridge ----------------
        echo -e "\n\e[33m[+] \e[36mInstalling VPSBot bridge...\033[0m"
        ...
        rm -rf /opt/vpsbot
        mkdir -p /opt/vpsbot
        cp -r /var/www/html/deltabotvps/vpsbot/* /opt/vpsbot/

        python3 -m venv /opt/vpsbot/venv
        /opt/vpsbot/venv/bin/pip install --upgrade pip
        /opt/vpsbot/venv/bin/pip install -r /opt/vpsbot/requirements.txt

        # Write .env for VPSBot (uses same BOT_TOKEN + admin id)
        cat > /opt/vpsbot/.env <<EOF
BOT_TOKEN=${YOUR_BOT_TOKEN}
ADMIN_IDS=${YOUR_CHAT_ID}
DOMAIN=${YOUR_DOMAIN}
HCLOUD_TOKEN=${YOUR_HCLOUD_TOKEN}
DB_PATH=/opt/vpsbot/vpsbot.sqlite3
BRIDGE_HOST=127.0.0.1
BRIDGE_PORT=9010
EOF

        # systemd service
        cp /opt/vpsbot/vpsbridge.service /etc/systemd/system/vpsbridge.service
        systemctl daemon-reload
        systemctl enable vpsbridge.service
        systemctl restart vpsbridge.service
        # ----------------------------------------------------------------------

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/deltabotvps/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        ...
        sudo rm /var/www/html/deltabotvps/createDB.php
