...
# Install the integrated repo (DeltaBot + VPSBot module)
...
if [ ! -d "/root/confdelta" ]; then
    ...
    #echo "${ASAS}paths = '${RANDOM_CODE}';" >> /root/confdelta/dbrootdelta.txt
...
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/deltabot/settings/messagedelta.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/deltabot/settings/rewardReport.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/deltabot/settings/warnusers.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/deltabot/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/deltabot/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'delta'") ]]; then
        clear
        ...
        read YOUR_BOT_TOKEN
        printf "\e[33m[+] \e[36mChat id (Admin numeric id): \033[0m"
        read YOUR_CHAT_ID
        printf "\e[33m[+] \e[36mDomain: \033[0m"
        read YOUR_DOMAIN
        printf "\e[33m[+] \e[36mHetzner API Token: \033[0m"
        read HETZNER_TOKEN
        echo " "
        if [ "$YOUR_BOT_TOKEN" = "" ] || [ "$YOUR_DOMAIN" = "" ] || [ "$YOUR_CHAT_ID" = "" ] || [ "$HETZNER_TOKEN" = "" ]; then
           exit
        fi
        ...
        # print file
        echo -e "<?php" >> /var/www/html/deltabot/baseInfo.php
        ...
        curl -F "url=https://${YOUR_DOMAIN}/deltabot/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        MESSAGE="✅ The delta bot has been successfully installed! @deltach"
        curl -s -X POST "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/sendMessage" -d chat_id="${YOUR_CHAT_ID}" -d text="$MESSAGE"

        # ----------------------------
        # VPSBot (Cloud Server module) auto-setup
        # ----------------------------
        echo -e "\n\e[92m[Cloud] Installing VPSBot module dependencies...\033[0m\n"

        # Python + build deps
        sudo apt-get install -y python3 python3-venv python3-pip build-essential

        VPSBOT_DIR="/var/www/html/deltabot/vpsbot_backend"
        if [ ! -d "$VPSBOT_DIR" ]; then
          echo -e "\e[91m[Cloud] vpsbot_backend not found in repo!\033[0m"
          exit 1
        fi

        # Write .env for VPSBot
        cat > "${VPSBOT_DIR}/.env" <<EOF
...
EOF
        ...
        # Ensure requirements include aiohttp (bridge server)
        if ! grep -q "^aiohttp" "${VPSBOT_DIR}/requirements.txt"; then
          echo "aiohttp==3.9.5" >> "${VPSBOT_DIR}/requirements.txt"
        fi
        ...
        # systemd service for bridge server (DeltaBot forwards updates here)
        cat > /etc/systemd/system/deltabot-vps-bridge.service <<'EOF'
[Unit]
Description=DeltaBot VPSBot Bridge (Cloud Module)
After=network.target

[Service]
Type=simple
WorkingDirectory=/var/www/html/deltabot/vpsbot_backend
EnvironmentFile=/var/www/html/deltabot/vpsbot_backend/.env
ExecStart=/var/www/html/deltabot/vpsbot_backend/.venv/bin/python /var/www/html/deltabot/vpsbot_backend/bridge_server.py
Restart=always
RestartSec=3
...
EOF
        ...
        systemctl enable deltabot-vps-bridge.service
        systemctl restart deltabot-vps-bridge.service

        echo -e "\e[92m[Cloud] VPS module installed and bridge started on 127.0.0.1:8090\033[0m\n"
        ...
        sudo rm -r /var/www/html/deltabot/webpanel
        sudo rm -r /var/www/html/deltabot/install
        sudo rm /var/www/html/deltabot/createDB.php
	rm /var/www/html/deltabot/updateShareConfig.php
	rm /var/www/html/deltabot/README.md
	rm /var/www/html/deltabot/README-fa.md
	rm /var/www/html/deltabot/LICENSE
	rm /var/www/html/deltabot/update.sh
	rm /var/www/html/deltabot/delta.sh
	rm /var/www/html/deltabot/tempCookie.txt
	rm /var/www/html/deltabot/settings/messagedelta.json
