...
$arrays = [
    ...
    "ALTER TABLE `server_config` ADD COLUMN `sub_base_url` varchar(255) DEFAULT NULL AFTER `panel_url`;",
    ...
    "ALTER TABLE `server_plans` ADD `show_full_renew` TINYINT(1) NOT NULL DEFAULT 1 AFTER `quota_charge_volume`;",
    ...
    "ALTER TABLE `users` ADD `pg_renew_suggestion` TINYINT(1) NOT NULL DEFAULT 1 AFTER `spam_info`;",
    "ALTER TABLE `users` ADD `pg_expiry_alerts` TINYINT(1) NOT NULL DEFAULT 1 AFTER `pg_renew_suggestion`;",
    "ALTER TABLE `orders_list` ADD `expired_warned_at` INT NOT NULL DEFAULT 0 AFTER `agent_bought`, ADD `delete_after` INT NOT NULL DEFAULT 0 AFTER `expired_warned_at`;",
    ];
function updateBot(){
    global $arrays, $connection, $walletdelta, $nowPaymentKey, $zarinpalId;
    ...
    if(isset($nowPaymentKey) && isset($zarinpalId)){
        $paymentKeys = array();
        $paymentKeys['bankAccount'] = $walletdelta;
    ...
    if(file_exists(getcwd() . '/settings/messagedelta.json')) unlink('../settings/messagedelta.json');
}
?>
