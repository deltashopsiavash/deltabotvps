...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu'){
    global $buttonValues;
    $rows = [];
    $configLinks = xuiNormalizeConfigLinks($configLinks);
    if(!empty($configLinks)){
        $rows[] = [[
            'text' => '📋 کپی همه کانفیگ ها',
...
function xuiSendOrderDeliveryPhoto($chatId, $protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = '', $backCallback = 'mainMenu'){
    ...
    $keyboard = xuiBuildOrderCopyButtons($configLinks, $subLink, $backCallback);
...
 /");
    ...
    $extractWsExternalTargets = function($node) use ($normalizeHostValue) {
        $targets = [];
        $hostKeys = ['host','domain','server','address','addr','dest','destination','targetHost','target','remoteHost','ip'];
        $portKeys = ['port','serverPort','targetPort','destPort','remotePort'];
        ...
        $walk = function($value) use (&$walk, &$appendTarget, $hostKeys, $portKeys) {
            if(is_object($value)) $value = get_object_vars($value);
            if(!is_array($value)) return;

            $candidateHost = '';
            foreach($hostKeys as $hostKey){
                if(isset($value[$hostKey])){
                    $candidateHost = $value[$hostKey];
                    break;
                }
            }
            ...
            foreach($portKeys as $portKey){
                if(isset($value[$portKey])){
                    $candidatePort = $value[$portKey];
                    break;
                }
            }

            if($candidateHost !== '' && $candidatePort !== null){
                $appendTarget($candidateHost, $candidatePort);
            }

            foreach($value as $child){
                if(is_array($child) || is_object($child)) $walk($child);
            }
        };
        ...
        if($inbound_id == 0){
            ...
            if($clients[0]->id == $uniqid || $clients[0]->password == $uniqid) {
                ...
                if($netType == 'ws') {
                    ...
                    $path = $wsSettingsObj->path ?? ($wsSettingsAssoc['path'] ?? '/');
                    $host = $wsSettingsObj->headers->Host ?? ($wsSettingsAssoc['headers']['Host'] ?? '');
