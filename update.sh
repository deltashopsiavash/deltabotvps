...
do
	case $opt in
		"Update bot")
			...
			mv /var/www/html/deltabot/baseInfo.php /root/
			...
			sleep 3
			mv /root/baseInfo.php /var/www/html/deltabot/

			sleep 1

   		db_namedelta=$(cat /var/www/html/deltabot/baseInfo.php | grep '$dbName' | cut -d"'" -f2)
		  db_userdelta=$(cat /var/www/html/deltabot/baseInfo.php | grep '$dbUserName' | cut -d"'" -f2)
		  db_passdelta=$(cat /var/www/html/deltabot/baseInfo.php | grep '$dbPassword' | cut -d"'" -f2)
			bot_token=$(cat /var/www/html/deltabot/baseInfo.php | grep '$botToken' | cut -d"'" -f2)
			bot_token2=$(cat /var/www/html/deltabot/baseInfo.php | grep '$botToken' | cut -d'"' -f2)
			bot_url=$(cat /var/www/html/deltabot/baseInfo.php | grep '$botUrl' | cut -d'"' -d"'" -f2)
			
			filepath="/var/www/html/deltabot/baseInfo.php"
			...
			rm /var/www/html/deltabot/createDB.php
		...
		"Update panel")
			...
			wait
   			cd /var/www/html/ && find . -mindepth 1 -maxdepth 1 ! -name deltabot -type d -exec rm -r {} \;
		...
		"Backup")
			...
			wget https://raw.githubusercontent.com/deltashopsiavash/deltabot/main/dbbackupdelta.sh | chmod +x dbbackupdelta.sh
			./dbbackupdelta.sh
		...
		"Delete")
			...
			pathsss=$(cat /root/confdelta/dbrootdelta.txt | grep '$path' | cut -d"'" -f2)
			passsword=$(cat /var/www/html/deltabot/baseInfo.php | grep '$dbPassword' | cut -d"'" -f2)
   			userrrname=$(cat /var/www/html/deltabot/baseInfo.php | grep '$dbUserName' | cut -d"'" -f2)
			...
			sudo rm -r /var/www/html/deltabot
