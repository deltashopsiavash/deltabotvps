...
function deltaPanelLoginCookies($server_info){
    ...
    return [$curl, array_keys($cookies)[0].'='.$cookies[array_keys($cookies)[0]], null];
}
...
function deltaSetXuiStateExact($order,$wantActive){
    // از توابع اصلی خود سورس برای تغییر وضعیت استفاده می‌کنیم تا با نوع پنل فعلی سازگار باشد.
    // اول وضعیت فعلی را از پنل می‌خوانیم؛ اگر همان وضعیت درخواستی بود، کاری انجام نمی‌دهیم.
    ...
    $row=$found['row'];
    $client=$found['client'];
    $actualUuid=(string)$found['uuid'];
    $current=true;
    if(isset($row->enable)) $current = $current && (bool)$row->enable;
    if(isset($client['enable'])) $current = $current && (bool)$client['enable'];
    if(isset($row->clientStats) && is_array($row->clientStats) && !empty($found['email'])){
        foreach($row->clientStats as $st){
            if(isset($st->email) && (string)$st->email === (string)$found['email'] && isset($st->enable)){
                $current = $current && (bool)$st->enable;
            }
        }
    }
    if($current === (bool)$wantActive) return (object)['success'=>true,'already'=>true];

    // در همه حالت‌ها وقتی کلاینت داخل settings پیدا شده، همان کلاینت را با تابع اصلی toggle می‌کنیم.
    // برای سفارش‌های قدیمی که inbound_id صفر دارند، id واقعی inbound از پنل گرفته می‌شود.
    if($inboundId <= 0 && isset($row->id)) $inboundId = (int)$row->id;
    if($inboundId > 0) $res = changeClientState($server_id,$inboundId,$actualUuid);
    else $res = changeInboundState($server_id,$actualUuid);
...
function deltaInactiveRenewAndReturn($oid){
    ...
    if($serverType==='marzban'){
        ...
        // برای تغییر UID از توابع اصلی سورس استفاده می‌کنیم تا مسیر درست panel/xui بر اساس نوع پنل انتخاب شود.
        if($inboundId>0) $upd=renewClientUuid($server_id,$inboundId,$uuid);
        else $upd=renewInboundUuid($server_id,$uuid);
