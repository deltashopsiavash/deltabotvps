...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if($specialOffer && !empty($hash_id) && !preg_match('/^discountSelectPlan/', (string)($userInfo['step'] ?? ''))){
        ...
        if(!specialOfferAttachPay($hash_id,(int)$specialOffer['id'],$offerQty,$offerAttachError)){
            ...
            $attachMessage=$offerAttachError==='user_limit'
                ...
                : 'موجودی این پیشنهاد همین حالا تمام شد؛ لطفاً پیشنهادهای فعال را دوباره بررسی کنید.';
    ...
    $invoiceOfferId=0;
    if(!empty($hash_id)){
        $invoiceStmt=$connection->prepare("SELECT `price`,`special_offer_id` FROM `pays` WHERE `hash_id`=? LIMIT 1");
...
<code>{$safeDeviceId}</code>";
        ...
        $keyboard = getReceiptAdminKeyboard("accept" . $match[1], "decline$uid", $uid);
        setUser('', 'temp');
        $res = sendPhotoToAdmins($fileid, $msg, $keyboard, "HTML");
        $msgId = $res->result->message_id;
