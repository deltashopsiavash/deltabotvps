...
if(preg_match('/^payPhotoReceipt(.+)$/', $data, $match)){
    ...
    setUser($originStep);
    sendMessage("❌سفارش شما لغو شد لطفا مراحل خرید را از اول طی کنید سپس اطلاعات واریزی را بفرستید❌", $cancelKey);
    exit;
}
...
if(preg_match('/^npvDeviceForPlan(\d+)_(\d+)_(\w+)_([^\s]*)$/', $userInfo['step'], $npvMatch) && $text != $buttonValues['cancel']){
    ...
    if(!function_exists('npvIsValidDeviceId') || !npvIsValidDeviceId($deviceIdInput)){
        sendMessage('❌ Device ID معتبر نیست. Device ID باید حداقل ۲۰ کاراکتر، بدون فاصله و شامل حروف و عدد باشد. لطفاً Device ID واقعی و کامل NapsternetV را ارسال کنید.', $cancelKey);
        exit();
    }
