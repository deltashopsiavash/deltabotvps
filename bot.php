...
// ------------------------------------------------------------
// NapsternetV device-lock support for PasarGuard unlimited plans
// Version 1: the bot collects Device ID, creates the PasarGuard user,
// hides raw links from the customer, and sends Device ID + sub link to admins
// so the locked .npvtsub file can be created manually and sent.
// ------------------------------------------------------------
if(!function_exists('npvEnsureSchema')){
    function npvEnsureSchema(){
        static $done=false; if($done) return; $done=true;
        global $connection;
        if(!isset($connection) || !$connection) return;
        @$connection->query("ALTER TABLE `server_plans` ADD `npv_lock` TINYINT(1) NOT NULL DEFAULT 0");
    }
}
npvEnsureSchema();
...
if(!function_exists('npvPlanRequiresDeviceLock')){
    function npvPlanRequiresDeviceLock($plan){
        ...
        return !empty($plan['npv_lock']) && (string)($plan['type'] ?? '') === 'pasarguard';
    }
}
...
if(!function_exists('npvValidateDeviceId')){
    function npvValidateDeviceId($deviceId){
        ...
        return (strlen($deviceId) >= 32 && strlen($deviceId) <= 200 && preg_match('/^[A-Za-z0-9_\-\.]+$/', $deviceId));
    }
}
...
if(!function_exists('npvSendLockedManualDelivery')){
    function npvSendLockedManualDelivery($chatId, $plan, $remark, $volume, $days, $subLink, $deviceId, $backCallback='mainMenu'){
        global $buttonValues, $admin, $botUrl;
        $txt = "✅ سفارش شما ساخته شد.\n\n";
        $txt .= "🔒 این پلن مخصوص NapsternetV و قفل روی Device ID است.\n";
        $txt .= "🆔 Device ID ثبت‌شده:\n<code>" . htmlspecialchars((string)$deviceId) . "</code>\n\n";
        $txt .= "لینک خام برای این پلن ارسال نمی‌شود. فایل قفل‌شده توسط پشتیبانی آماده و همینجا برای شما ارسال می‌شود.";
...
if((preg_match('/^discountCustomPlanDay(\d+)/',$userInfo['step'], $match) || preg_match('/enterCustomPlanName(\d+)_(\d+)_(\d+)_(\d+)_(?<buyType>\w+)/',$userInfo['step'], $match)) && ($botState['sellState']=="on" ||$from_id ==$admin) && $text != $buttonValues['cancel']){
    ...
    if(npvPlanRequiresDeviceLock($respd)){
        ...
        if(!npvValidateDeviceId($npvDeviceId)){
            ...
            setUser('npvDevice_selectPlan' . $id . '_' . $call_id . '_' . $match['buyType']);
            exit();
        }
    }
    if(!userCanAccessServer($from_id, $sid) && $from_id != $admin && $userInfo['isAdmin'] != true){
...
if(preg_match('/^npvDevice_(selectPlan\d+_\d+_\w+)/', $userInfo['step'], $npvStepMatch) && $text != $buttonValues['cancel']){
    $deviceId = trim($text);
...
if(preg_match('/(addNewRahgozarPlan|addNewPlan|addNewMarzbanPlan|addNewPasarguardPlan)/',$userInfo['step']) and $text!=$buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if($step==54 and $text!=$buttonValues['cancel']){
        ...
        if($userInfo['step'] == "addNewPlan"){
            ...
            // For PasarGuard plans ask whether this plan must be delivered as a NapsternetV device-locked file.
            $sql = ("UPDATE `server_plans` SET `volume`=?, `type`='pasarguard', `inbound_id`=0, `acount`=999999, `limitip`=0, `step`=151 WHERE `active`=0");
            $msg = '🔒 آیا این پلن نامحدود/ناپستر و قفل روی Device ID باشد؟';
        ...
        if($userInfo['step'] == "addNewPasarguardPlan"){
            sendMessage($msg, json_encode(['inline_keyboard'=>[
                [['text'=>'✅ بله، Device ID بگیرد و لینک خام ندهد','callback_data'=>'pgNpvLock_1']],
                [['text'=>'❌ خیر، معمولی باشد','callback_data'=>'pgNpvLock_0']]
            ]], JSON_UNESCAPED_UNICODE), 'HTML');
        }else{
            sendMessage($msg);
        }
    }
    if($step==151 and $text!=$buttonValues['cancel'] and preg_match('/^pgNpvLock_([01])$/', $data, $npvLockMatch)){
        ...
        $stmt->execute();
        $stmt->close();
        smartSendOrEdit($message_id, '🔻یه توضیح برای پلن پاسارگارد بنویس:');
        exit();
    }
