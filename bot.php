...
function deltaInactiveSetState($order,$wantActive){
    global $connection;
    $cur=deltaInactiveStatus($order);
    if($cur===null) return (object)['success'=>false,'msg'=>'اشتراک در پنل پیدا نشد یا اتصال پنل مشکل دارد'];
    if((bool)$cur===(bool)$wantActive) return (object)['success'=>true,'already'=>true];
    $server_id=(int)$order['server_id']; $uuid=$order['uuid']??'0'; $remark=$order['remark']??''; $inboundId=(int)($order['inbound_id']??0);
    $stmt=$connection->prepare("SELECT `type` FROM `server_config` WHERE `id`=? LIMIT 1");
    $stmt->bind_param('i',$server_id); $stmt->execute(); $server=$stmt->get_result()->fetch_assoc(); $stmt->close();
    if(!$server) return (object)['success'=>false,'msg'=>'پنل پیدا نشد'];
    if($server['type']==='marzban') $res=changeMarzbanState($server_id,$remark);
    elseif($inboundId==0) $res=changeInboundState($server_id,$uuid);
    else $res=changeClientState($server_id,$inboundId,$uuid);
    if($res && !empty($res->success)) return (object)['success'=>true];
    return (object)['success'=>false,'msg'=>($res->msg??'عملیات ناموفق بود')];
}
...
function deltaInactiveRenewAndReturn($oid){
    ...
    if($serverType==='marzban'){
        ...
        $response=getJson($server_id)->obj??[]; $port='';
        foreach($response as $row){
            if($inboundId==0){
                $clients=xuiDecodeField($row->settings)->clients??[];
                if(isset($clients[0]) && (($clients[0]->id??'')==$uuid || ($clients[0]->password??'')==$uuid)){
                    $port=$row->port; $protocol=$row->protocol; $netType=xuiDecodeField($row->streamSettings)->network; break;
                }
            }else if((int)$row->id===$inboundId){
                $port=$row->port; $protocol=$row->protocol; $netType=xuiDecodeField($row->streamSettings)->network; break;
            }
        }
        $upd=($inboundId==0)?renewInboundUuid($server_id,$uuid):renewClientUuid($server_id,$inboundId,$uuid);
