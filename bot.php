...
if($specialReservationHash!==''){
    ...
    if($reservationPay && (int)($reservationPay['special_offer_id'] ?? 0)>0){
        ...
        // Crypto/payment-provider flows may need confirmation time after the
        // user enters them. They must still be initiated during the first five minutes.
        if(preg_match('/^(?:payWithTronWallet|payWithWeSwap)/',$reservationData)){
            specialOfferExtendReservation($reservationPay,1200);
        }
    }
}
...
if($data==='specialOfferToday'){
    ...
    $progress=specialOfferProgressBar((int)$offer['stock_sold'],(int)$offer['stock_total']);
    $txt="🔥 {$offer['title']}\n\n📦 {$offer['plan_volume']}GB / {$offer['plan_days']} روزه\n💰 قیمت اصلی: <s>".number_format((int)$offer['plan_price'])." تومان</s>\n🎁 قیمت ویژه: <b>".number_format($finalPrice)." تومان</b>\n🏷 تخفیف: <b>".(int)$offer['discount_percent']."٪</b>\n👤 سقف خرید هر کاربر: <b>{$limitLabel}</b>\n🎟 سهمیه باقی‌مانده شما: <b>{$remainingLabel}</b>\n\n📊 <b>روند فروش واقعی</b>\n<code>{$progress}</code>\n✅ خرید موفق: <b>".(int)$offer['stock_sold']." عدد</b>\n⏳ در حال رزرو: <b>".(int)$offer['reserved']." عدد</b>\n🔥 آماده خرید: <b>".(int)$offer['available_remaining']." عدد</b>\n\n⏱ زمان باقی‌مانده: <b>".specialOfferCountdownText($offer['ends_at'])."</b>\n🕛 پایان: <b>".specialOfferFormatDate($offer['ends_at'])."</b>";
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if($price == 0 or ($from_id == $admin)){
        ...
        if(!preg_match('/^discountSelectPlan/', $userInfo['step'])){
            ...
            $price = applyUserPercentDiscount($from_id, $price);
            if($specialOffer) $price = specialOfferApplyDiscount($price, $specialOffer);
    ...
    if($specialOffer && !empty($hash_id) && !preg_match('/^discountSelectPlan/', (string)($userInfo['step'] ?? ''))){
        ...
        if(!specialOfferAttachPay($hash_id,(int)$specialOffer['id'],$offerQty,$offerAttachError)){
            ...
            $attachMessage=$offerAttachError==='user_limit'
                ? 'سهمیه خرید شما از این پیشنهاد تمام شده است.'
                : 'موجودی این پیشنهاد فروخته یا همین حالا توسط کاربران دیگر رزرو شد؛ چند دقیقه دیگر دوباره بررسی کنید.';
