...
if(preg_match('/^adminResDBDropAsk_([A-Za-z0-9_]+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $dbn = $m[1];
    if(isset($isChildBot) && $isChildBot){ alert('❌ این بخش فقط در ربات مادر فعال است.', true); exit; }
    $baseDb = $GLOBALS['dbName'] ?? ($dbName ?? '');
    if(!($dbn === $baseDb || (strpos($dbn, $baseDb . "_rb") === 0))){
        alert("❌ دیتابیس معتبر نیست.", true);
        exit;
...
if(preg_match('/^adminResDBDropYes_([A-Za-z0-9_]+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Try drop (always from MAIN DB connection)
    if(isset($isChildBot) && $isChildBot){ alert('❌ این بخش فقط در ربات مادر فعال است.', true); exit; }

    $dbEsc = str_replace('`','',$dbn);

    // Open a fresh connection to MAIN DB (never drop using child bot connection)
    $mainDb = $GLOBALS['mainDbName'] ?? ($mainDbName ?? ($baseDb ?? ''));
    $mainConn = new mysqli('localhost', $dbUserName, $dbPassword, $mainDb);
    if($mainConn->connect_error){
        smartSendOrEdit($message_id, "❌ اتصال به دیتابیس اصلی انجام نشد.

".$mainConn->connect_error, ['inline_keyboard'=>[[['text'=>"🔙 برگشت",'callback_data'=>"adminResDBInfo_".$dbn]]]]);
        exit;
    }
    $mainConn->set_charset('utf8mb4');

    $ok = $mainConn->query("DROP DATABASE `{$dbEsc}`");
    if($ok){
        $dbnSafe = $mainConn->real_escape_string($dbn);
        $mainConn->query("DELETE FROM reseller_bots WHERE db_name='{$dbnSafe}'");
