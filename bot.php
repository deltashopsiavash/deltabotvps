...
// Handle start-approval actions BEFORE VPSBot bridge routing.
// Otherwise if an admin is inside VPSBot mode, approve/reject clicks get
// forwarded to the bridge and appear to do nothing.
if(preg_match('/^(approveStartAccess|rejectStartAccess)_(\d+)$/', $data ?? '', $m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)) {
    $targetUserId = (int)$m[2];
    $newStatus = $m[1] === 'approveStartAccess' ? 'approved' : 'rejected';
    setUserAccessStatus($targetUserId, $newStatus);
    ...
    $stmt->close();
    $targetName = htmlspecialchars((string)($targetRow['name'] ?? $targetUserId), ENT_QUOTES, 'UTF-8');
    $targetUsername = trim((string)($targetRow['username'] ?? ''));
    $targetUsername = ($targetUsername !== '' && $targetUsername !== ' ندارد ') ? '@' . ltrim($targetUsername, '@') : 'ندارد';

    if($newStatus === 'approved') {
        sendMessage("دسترسی شما به ربات آزاد شد✅

" . $mainValues['start_message'], getMainKeys(), 'HTML', $targetUserId);
        $adminText = "✅ دسترسی کاربر تایید شد.

👤 <a href='tg://user?id={$targetUserId}'>{$targetName}</a>
🆔 <code>{$targetUserId}</code>
🔎 <code>{$targetUsername}</code>";
        smartSendOrEdit($message_id, $adminText, json_encode(['inline_keyboard'=>[]]), 'HTML');
        bot('answerCallbackQuery', ['callback_query_id'=>$callbackId, 'text'=>'کاربر تایید شد', 'show_alert'=>false]);
    } else {
        sendMessage(getStartApprovalRejectedText(), null, 'HTML', $targetUserId);
        $adminText = "❌ دسترسی کاربر رد شد.

👤 <a href='tg://user?id={$targetUserId}'>{$targetName}</a>
🆔 <code>{$targetUserId}</code>
🔎 <code>{$targetUsername}</code>";
        smartSendOrEdit($message_id, $adminText, json_encode(['inline_keyboard'=>[]]), 'HTML');
        bot('answerCallbackQuery', ['callback_query_id'=>$callbackId, 'text'=>'کاربر رد شد', 'show_alert'=>false]);
    }
    exit;
}
...
if(isStartApprovalRequired() && $from_id != $admin && (($userInfo['isAdmin'] ?? false) != true)) {
    $accessStatus = trim((string)($userInfo['access_status'] ?? 'pending'));

    if(preg_match('/^\/([Ss]tart)/', (string)$text) || $data == 'mainMenu') {
        setUser();
        setUserAccessStatus($from_id, 'pending');
        $userInfo['access_status'] = 'pending';
        sendStartApprovalRequestToAdmins($from_id, $first_name ?? '', $username ?? '');
        if(isset($data) && $data == 'mainMenu'){
            smartSendOrEdit($message_id, getStartApprovalPendingText(), json_encode(['inline_keyboard'=>[]]), 'HTML');
        } else {
            sendMessage(getStartApprovalPendingText(), null, 'HTML');
        }
        exit;
    }
    ...
    if($accessStatus !== 'approved') {
        if(isset($data) && !empty($data)) {
            bot('answerCallbackQuery', ['callback_query_id'=>$callbackId, 'text'=>($accessStatus === 'rejected' ? 'شما حق استفاده از ربات را ندارید' : 'درخواست شما برای مدیر ارسال شده است'), 'show_alert'=>false]);
        } else {
            sendMessage($accessStatus === 'rejected' ? getStartApprovalRejectedText() : getStartApprovalPendingText(), null, 'HTML');
        }
        exit;
    }
}
