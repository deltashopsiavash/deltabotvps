...
function deltaInactiveOrdersKeyboard($sid){
    global $connection;
    $sid=(int)$sid; $rows=[];
    $q=$connection->query("SELECT * FROM orders_list WHERE server_id={$sid} ORDER BY id DESC LIMIT 300");
    if($q){
        while($o=$q->fetch_assoc()){
            $st=deltaInactiveStatus($o);
            if($st===false){
