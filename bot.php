...
if(preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    for($i = 1; $i<= $text; $i++){
        ...
        if($serverType == "marzban"){
            ...
            $nativeSubLink = (($botState['nativeSubLinkState'] ?? 'off') == "on") ? getClientNativeSubscriptionLink($server_id, $uniqid, $inbound_id, $remark) : "";
        ...
        🔮 $remark \n " . ($botState['configLinkState'] != "off" && $serverType != "marzban"?"<code>$link</code>":"");
            if($botState['subLinkState'] == "on" && !empty($subLink)) $acc_text .= 
            " \n🌐 subscription : <code>$subLink</code>";
            if((($botState['nativeSubLinkState'] ?? 'off') == "on") && !empty($nativeSubLink) && $nativeSubLink != $subLink) $acc_text .= "\n📡 panel subscription : <code>$nativeSubLink</code>";
...
if(preg_match('/havePaiedWeSwap(.*)/',$data,$match)) {
    ...
    for($i = 1; $i <= $accountCount; $i++){
        ...
        if($serverType == "marzban"){
            $uniqid = $token = str_replace("/sub/", "", $response->sub_link);
            $subLink = $botState['subLinkState'] == "on"?$panelUrl . $response->sub_link:"";
            $nativeSubLink = (($botState['nativeSubLinkState'] ?? 'off') == "on") ? ($panelUrl . $response->sub_link) : "";
            $vraylink = [$subLink];
            $vray_link = json_encode($response->vray_links);
        }else{
            $token = RandomString(30);
