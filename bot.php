...
if($data==='specialOfferToday'){
    ...
    $txt="🔥  {$safeTitle}\n\n"
        ...
        ."🕛 پایان: ".specialOfferFormatDate($offer['ends_at']);
    $reservationWindowOpen=((int)$offer['ends_at']-time())>=300;
    ...
    elseif(!$reservationWindowOpen) $txt.="\n\n⏳ کمتر از ۵ دقیقه تا پایان طرح مانده و صدور فاکتور جدید بسته شده است.";
    $keyRows=[];
    if($userAllowance['can_buy'] && (int)$offer['available_remaining']>0 && $reservationWindowOpen) $keyRows[]=[['text'=>'🛍 خرید این پیشنهاد','callback_data'=>'specialOfferBuy'.(int)$offer['id']]];
...
if(preg_match('/^specialOfferBuy(\d+)$/',$data,$m)){
    ...
    if(((int)$offer['ends_at']-time())<300){ alert('⏳ کمتر از ۵ دقیقه تا پایان طرح مانده و دیگر فاکتور جدید صادر نمی‌شود.',true); exit(); }
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if($specialOffer){
        ...
        specialOfferSyncCompletedSales();
        if(((int)$specialOffer['ends_at']-time())<300){
            $endingMessage='⏳ کمتر از ۵ دقیقه تا پایان طرح مانده و دیگر فاکتور جدید صادر نمی‌شود.';
            if(isset($update->callback_query)) alert($endingMessage,true); else sendMessage($endingMessage);
            setUser(); setUser('','temp');
            exit();
        }
    ...
    if($specialOffer && !empty($hash_id) && !preg_match('/^discountSelectPlan/', (string)($userInfo['step'] ?? ''))){
        ...
        if(!specialOfferAttachPay($hash_id,(int)$specialOffer['id'],$offerQty,$offerAttachError)){
            ...
            elseif($offerAttachError==='ending_soon') $attachMessage='⏳ کمتر از ۵ دقیقه تا پایان طرح مانده و دیگر فاکتور جدید صادر نمی‌شود.';
