...
if(!$isChildBot && $data === 'vpsbotStart'){
    // Switch user to VPS child bot mode
    setUser('vpsbot','step');

    // IMPORTANT: Do NOT forward the same callback update.
    // Some child-bot flows need an explicit init trigger (e.g. /start) after step switch.
    // So we ACK the callback immediately and then send a synthetic /start update to VPS bot.
    ...
    forwardUpdateToVpsbot(json_encode($fakeUpdate, JSON_UNESCAPED_UNICODE));
    exit;
}
