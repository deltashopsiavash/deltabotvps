...
global $rawUpdate, $isChildBot, $userInfo, $from_id, $admin;
...
if(!$isChildBot && isset($userInfo['step']) && $userInfo['step'] === 'vpsbot'){
    ...
    forwardUpdateToVpsbot($rawUpdate);
    exit;
}
...
// Safety: child bot MUST have a dedicated DB, otherwise operations like backup/restore would affect mother.
// If DB is not ready, stop processing.
if(isset($isChildBot) && $isChildBot && !empty($childDbNotReady) && $childDbNotReady){
    if(isset($from_id) && isset($childBotRow) && ($from_id == (int)$childBotRow['owner_userid'] || $from_id == (int)$childBotRow['admin_userid'])){
        sendMessage("⚠️ این ربات نمایندگی هنوز دیتابیس اختصاصی ندارد و به همین دلیل موقتاً غیرفعال شده است.

✅ برای فعال‌سازی باید ربات اصلی بتواند برای این ربات دیتابیس جدا بسازد.
از مدیر بخواهید دسترسی CREATE DATABASE به یوزر دیتابیس بدهد و سپس دوباره از ربات اصلی بخش «ربات‌های من» اقدام به فعال‌سازی/ساخت نمایید.");
    }
    exit;
}
    ...
    if(isset($update->message) && isset($update->message->document)){
        ...
        if($fileId){
            ...
            if($in && $out){
                ...
                // امنیت: اگر ربات نمایندگی دیتابیس اختصاصی ندارد، اجازه ریستور نده (خطر ریستور روی دیتابیس مادر)
                $dbToUseCheck = $GLOBALS['dbName'] ?? ($dbName ?? '');
                if(!empty($isChildBot) && ($dbToUseCheck === ($mainDbName ?? ''))){
                    sendMessage("❌ این ربات نمایندگی دیتابیس اختصاصی ندارد و امکان بازگردانی بکاپ وجود ندارد چون ممکن است روی دیتابیس ربات مادر اعمال شود.

✅ لطفاً ابتدا دیتابیس اختصاصی این ربات را از ربات اصلی فعال کنید (ساخت دیتابیس) و سپس بکاپ را ریستور کنید.");
                    @unlink($path);
                    exit;
                }
...
لطفا دوباره توکن صحیح رو ارسال کن.");
        ...
        if(!$dbOk){
            // سخت‌گیرانه: بدون دیتابیس اختصاصی اجازه فعال‌سازی نداریم (برای جلوگیری از قاطی شدن اطلاعات/بکاپ)
            $stmt = $connection->prepare("UPDATE `reseller_bots` SET `status`=0 WHERE `id`=? AND `owner_userid`=?");
            if($stmt){
            ...
            setUser("none","step");
            sendMessage("❌ ساخت دیتابیس اختصاصی برای ربات نمایندگی ناموفق بود و ادامه فرایند متوقف شد.

⚠️ اگر ادامه بدهیم، این ربات مجبور می‌شود از دیتابیس ربات مادر استفاده کند و باعث قاطی شدن اطلاعات و حتی خاموش شدن ربات مادر در زمان بکاپ/ریستور می‌شود.

✅ لطفاً دسترسی CREATE DATABASE را برای یوزر دیتابیس فعال کنید و دوباره از ربات اصلی اقدام کنید.");
            exit;
        }
...
if($data=="toggleCloudServerState" && ($from_id == $admin || $userInfo['isAdmin'] == true) && !$isChildBot){
    $cur = $botState['cloudServerState'] ?? 'on';
    $newValue = ($cur=='on')?'off':'on';
    setSettings('cloudServerState', $newValue);
    smartSendOrEdit($message_id, $mainValues['change_bot_settings_message'], getBotSettingKeys());
}
...
// --- Change QRCode background image (mother bot) ---
if($data=="changeQrBg" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("🖼 لطفا تصویر جدید QRCODE را ارسال کنید (JPG/PNG).\n\n✅ بعد از ارسال، برای همه QRCode های اشتراک استفاده می‌شود.", $cancelKey);
    setUser("changeQrBg");
}
if($userInfo['step']=="changeQrBg" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    // Accept photo or document image
    $fileId = null;
    if(isset($update->message->photo)){
        $photos = $update->message->photo;
        $fileId = end($photos)->file_id;
    }elseif(isset($update->message->document) && isset($update->message->document->mime_type) && strpos($update->message->document->mime_type, 'image/') === 0){
        $fileId = $update->message->document->file_id;
    }
    if(empty($fileId)){
        sendMessage("❌ لطفا فقط عکس ارسال کنید.", $cancelKey);
        exit;
    }
    $url = getFileUrl($fileId);
    $imgData = @file_get_contents($url);
    if($imgData === false){
        sendMessage("❌ دریافت فایل از تلگرام ناموفق بود. دوباره تلاش کنید.", $cancelKey);
        exit;
    }
    $savePath = __DIR__ . "/settings/QRCode.jpg";
    if(!empty($isChildBot)){
        $bid = $GLOBALS['currentBotInstanceId'] ?? 0;
        if($bid>0){
            @mkdir(__DIR__ . '/settings/qrcodes', 0775, true);
            $savePath = __DIR__ . "/settings/qrcodes/qr_rb{$bid}.jpg";
        }
    }else{
        @mkdir(__DIR__ . '/settings/qrcodes', 0775, true);
        $savePath = __DIR__ . "/settings/qrcodes/qr_main.jpg";
    }
    @file_put_contents($savePath, $imgData);
    resetUserFlow();
    sendMessage("✅ تصویر QRCODE با موفقیت تغییر کرد.", getAdminKeys());
}
// --- end change QRCode ---
...
if(preg_match('/^yesSend2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(file_exists($worker) && function_exists('shell_exec')){
        $bidArg = '';
        if(isset($isChildBot) && $isChildBot === true && isset($currentBotInstanceId) && (int)$currentBotInstanceId>0){
            $bidArg = ' ' . (int)$currentBotInstanceId;
        }
        $cmd = 'nohup php ' . escapeshellarg($worker) . $bidArg . ' >/dev/null 2>&1 &';
        @shell_exec($cmd);
    }
