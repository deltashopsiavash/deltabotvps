...
function deltaSetXuiStateExact($order,$wantActive){
    ...
    if(($serverType == 'sanaei' || $serverType == 'alireza') && $inboundId>0){
        ...
        $url = ($serverType == 'sanaei') ? "$panel_url/panel/inbound/updateClient/".rawurlencode($actualUuid) : "$panel_url/xui/inbound/updateClient/".rawurlencode($actualUuid);
    }else{
        ...
        $url = ($serverType == 'sanaei') ? "$panel_url/panel/inbound/update/{$row->id}" : "$panel_url/xui/inbound/update/{$row->id}";
    }
    ...
    $raw=curl_exec($curl); $err=curl_error($curl); curl_close($curl);
    if($raw===false) return (object)['success'=>false,'msg'=>'خطای curl: '.$err];
    $res=json_decode($raw);
    ...
    return (object)['success'=>false,'msg'=>'پاسخ نامعتبر از پنل: '.mb_substr(strip_tags((string)$raw),0,120)];
}
...
function deltaInactiveRenewAndReturn($oid){
    ...
    if($serverType==='marzban'){
        ...
        $upd=($order['inbound_id']==0)?renewInboundUuid($server_id,$uuid):renewClientUuid($server_id,$inboundId,$uuid);
