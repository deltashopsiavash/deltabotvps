...
function pgRenewableOrdersKeyboard($userId,$serverId,$buyType,$page=0){
    global $connection;
    $page=max(0,(int)$page); $per=5; $offset=$page*$per; $fetch=$per+1;
    ...
    $stmt->bind_param('iiii',$userId,$serverId,$fetch,$offset); $stmt->execute(); $res=$stmt->get_result(); $stmt->close();
    $items=[]; while($o=$res->fetch_assoc()) $items[]=$o;
    ...
    foreach($items as $o){
        ...
        $exp=(int)$o['expire_date']; if($exp>20000000000) $exp=(int)floor($exp/1000);
        if($exp>0) $label .= ' | '.jdate('Y/m/d',$exp);
        $rows[]=[['text'=>$label,'callback_data'=>'pgRenewMenu'.(int)$o['id']]];
    }
    $nav=[];
    if($page>0) $nav[]=['text'=>'⬅️ قبلی','callback_data'=>'pgRenewPage'.$serverId.'_'.$buyType.'_'.($page-1)];
    if($hasNext) $nav[]=['text'=>'بعدی ➡️','callback_data'=>'pgRenewPage'.$serverId.'_'.$buyType.'_'.($page+1)];
    if($nav) $rows[]=$nav;
    $rows[]=[['text'=>'↩️ بازگشت','callback_data'=>'buySubscription']];
...
// Personal settings
if($data==='myPersonalSettings'||$data==='myToggleRenewSuggestion'||$data==='myToggleExpiryAlerts'){
    if($data==='myToggleRenewSuggestion'){$st=$connection->prepare("UPDATE users SET pg_renew_suggestion=IF(COALESCE(pg_renew_suggestion,1)=1,0,1) WHERE userid=?");$st->bind_param('i',$from_id);$st->execute();$st->close();}
    if($data==='myToggleExpiryAlerts'){$st=$connection->prepare("UPDATE users SET pg_expiry_alerts=IF(COALESCE(pg_expiry_alerts,1)=1,0,1) WHERE userid=?");$st->bind_param('i',$from_id);$st->execute();$st->close();}
    $st=$connection->prepare("SELECT COALESCE(pg_renew_suggestion,1) renew_s,COALESCE(pg_expiry_alerts,1) expiry_s FROM users WHERE userid=?");$st->bind_param('i',$from_id);$st->execute();$p=$st->get_result()->fetch_assoc();$st->close();
    $kb=json_encode(['inline_keyboard'=>[[['text'=>(int)$p['renew_s']===1?'🔔 پیشنهاد تمدید: روشن':'🔕 پیشنهاد تمدید: خاموش','callback_data'=>'myToggleRenewSuggestion']],[['text'=>(int)$p['expiry_s']===1?'⚠️ گزارش پایان و حذف ۳روزه: روشن':'🔇 گزارش پایان و حذف ۳روزه: خاموش','callback_data'=>'myToggleExpiryAlerts']],[['text'=>$buttonValues['back_button'],'callback_data'=>'myInfo']]]],JSON_UNESCAPED_UNICODE);
    smartSendOrEdit($message_id,"⚙️ تنظیمات شخصی\n\nگزینه موردنظر را روشن یا خاموش کنید.",$kb);exit;
}
// Admin-only whole wallet transfer
if(preg_match('/^uTransferWallet(\d+)$/',$data,$m)&&($from_id==$admin||!empty($userInfo['isAdmin']))){$src=(int)$m[1];setUser('adminTransferWholeWallet_'.$src);delMessage();sendMessage("آیدی عددی کاربر مقصد را ارسال کنید.",$cancelKey);exit;}
if(preg_match('/^adminTransferWholeWallet_(\d+)$/',$userInfo['step']??'',$m)&&$text!=$buttonValues['cancel']&&($from_id==$admin||!empty($userInfo['isAdmin']))){$src=(int)$m[1];if(!ctype_digit(trim((string)$text))){sendMessage('فقط آیدی عددی ارسال کنید');exit;}$dst=(int)$text;if($dst<=0||$dst===$src){sendMessage('آیدی مقصد معتبر نیست');exit;}$connection->begin_transaction();try{$st=$connection->prepare("SELECT wallet FROM users WHERE userid=? FOR UPDATE");$st->bind_param('i',$src);$st->execute();$a=$st->get_result()->fetch_assoc();$st->close();$st=$connection->prepare("SELECT userid FROM users WHERE userid=? FOR UPDATE");$st->bind_param('i',$dst);$st->execute();$b=$st->get_result()->fetch_assoc();$st->close();if(!$a||!$b)throw new Exception('کاربر پیدا نشد');$amount=(int)$a['wallet'];if($amount<=0)throw new Exception('موجودی مبدا صفر است');$st=$connection->prepare("UPDATE users SET wallet=0 WHERE userid=?");$st->bind_param('i',$src);$st->execute();$st->close();$st=$connection->prepare("UPDATE users SET wallet=wallet+? WHERE userid=?");$st->bind_param('ii',$amount,$dst);$st->execute();$st->close();$connection->commit();setUser();$f=number_format($amount).' تومان';sendMessage("✅ انتقال انجام شد\nاز: {$src}\nبه: {$dst}\nمبلغ: {$f}",$removeKeyboard);sendMessage("🔻 مبلغ {$f} از کیف پول شما توسط مدیریت منتقل شد.",null,null,$src);sendMessage("🔺 مبلغ {$f} توسط مدیریت به کیف پول شما اضافه شد.",null,null,$dst);}catch(Throwable$e){$connection->rollback();sendMessage('❌ '.$e->getMessage());}exit;}

if($data=="transferMyWallet" && ($from_id == $admin || !empty($userInfo['isAdmin']))){
...
if(preg_match('/havePaiedWeSwap(.*)/',$data,$match)) {
    ...
    elseif($payType == "RENEW_ACCOUNT"){
        ...
        if($serverType == "pasarguard"){
            $response = editPasarguardUser($server_id, $remark, [
                'days'=>$days,
                'volume'=>$volume
            ]);
        }elseif($serverType == "marzban"){
        ...
        $stmt = $connection->prepare("UPDATE `orders_list` SET `expire_date` = ?, `notif` = 0, `expired_warned_at` = 0, `expired_delete_at` = 0, `expired_delete_claimed_at` = 0 WHERE `id` = ?");
...
if(preg_match('/^pgRenewPage(\d+)_(\w+)_(\d+)$/',$data,$m)){
    $sid=(int)$m[1]; $buyType=$m[2]; $page=(int)$m[3];
    $renewKb=pgRenewableOrdersKeyboard($from_id,$sid,$buyType,$page);
    if(!$renewKb){ alert('اشتراک دیگری در این صفحه پیدا نشد',true); exit; }
    smartSendOrEdit($message_id,"🔔 لیست اشتراک‌های قابل تمدید شما
...
if(preg_match('/selectServer(?<continue>Continue)?(?<serverId>\d+)_(?<buyType>\w+)/',$data, $match) && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true)) ) {
    ...
    if(empty($match['continue']) && pgUserRenewSuggestionEnabled($from_id)){
        ...
        $stmtPg->bind_param('i',$sid); $stmtPg->execute(); $pgType=$stmtPg->get_result()->fetch_assoc()['type'] ?? ''; $stmtPg->close();
        if($pgType === 'pasarguard'){
            $renewKb=pgRenewableOrdersKeyboard($from_id,(int)$sid,$match['buyType']);
            if($renewKb){
                smartSendOrEdit($message_id,"🔔 شما قبلاً از این پنل اشتراک دارید. می‌توانید یکی از اشتراک‌های زیر را تمدید کنید و نیازی به ساخت اشتراک جدید نیست.

تمدید اجباری نیست؛ برای خرید اشتراک جدید روی دکمه «خرید اشتراک جدید» بزنید.",$renewKb);
                exit;
            }
        }
    }
