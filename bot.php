...
if(preg_match('/changAccountConnectionLink(\d+)/', $data,$match)){
    ...
    if($serverType == "marzban"){
        ...
        if(!$response || !isset($response->obj) || !is_array($response->obj)){
            sendMessage("❌ اتصال به پنل سرور برقرار نشد یا پاسخ پنل نامعتبر است.", null, null, $from_id);
            return;
        }
        $found = false;
        $port = $protocol = $netType = null;
        if($inboundId == 0){
            foreach($response->obj as $row){
                $settingsObj = xuiDecodeField($row->settings);
                $clients = $settingsObj->clients ?? [];
                $firstClient = $clients[0] ?? null;
                if($firstClient && ((isset($firstClient->id) && $firstClient->id == $uuid) || (isset($firstClient->password) && $firstClient->password == $uuid))) {
                    $port = $row->port;
                    $protocol = $row->protocol;
                    $netType = xuiDecodeField($row->streamSettings)->network;
                    $found = true;
            ...
            if(!$found){
                sendMessage("❌ کانفیگ با UUID فعلی داخل پنل پیدا نشد؛ ممکن است از پنل حذف شده یا UUID دیتابیس با پنل یکی نباشد.", null, null, $from_id);
                return;
            }
            $update_response = renewInboundUuid($server_id, $uuid);
        }else{
            foreach($response->obj as $row){
                if((int)$row->id == $inboundId) {
                    $port = $row->port;
                    $protocol = $row->protocol;
                    $netType = xuiDecodeField($row->streamSettings)->network;
                    $found = true;
            ...
            if(!$found){
                sendMessage("❌ اینباند مربوط به این کانفیگ داخل پنل پیدا نشد.", null, null, $from_id);
                return;
            }
...
function deltaPanelLoginCookies($server_info){
    ...
    foreach($loginPaths as $loginPath){
        ...
        curl_setopt_array($curl,[
            ...
            CURLOPT_TIMEOUT=>15,
            CURLOPT_POSTFIELDS=>json_encode(['username'=>$server_info['username'], 'password'=>$server_info['password']]),
            CURLOPT_HEADER=>1,
        ...
        if($response === false){
            curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query(['username'=>$server_info['username'], 'password'=>$server_info['password']]));
            curl_setopt($curl, CURLOPT_HTTPHEADER, ['Content-Type: application/x-www-form-urlencoded']);
            $response = curl_exec($curl);
        }
        if($response === false){ $err=curl_error($curl); curl_close($curl); continue; }
    ...
    return [null, null, 'کوکی یا نشست معتبر از پنل دریافت نشد'];
}
...
function deltaXuiPostClientOrInbound($server_info,$row,$settings,$client_key,$oldUuid){
    ...
    $urls=[]; $payloads=[];
    // مسیر استاندارد 3x-ui / سنایی / علیرضا برای ویرایش یک کلاینت
    $singleSetting=json_encode(['clients'=>[$editedClient]], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
    $singleData=['id'=>$inboundId,'settings'=>$singleSetting];
    $urls[]="$panel_url/panel/inbound/updateClient/".rawurlencode($oldUuid); $payloads[]=$singleData;
    $urls[]="$panel_url/xui/inbound/updateClient/".rawurlencode($oldUuid); $payloads[]=$singleData;
    // مسیر جایگزین: آپدیت کل inbound برای پنل‌های قدیمی
    $allSettings=$settings; $allSettings['clients']=array_values($allSettings['clients']);
    $fullData=['up'=>$row->up??0,'down'=>$row->down??0,'total'=>$row->total??0,'remark'=>$row->remark??'','enable'=>true,'expiryTime'=>$row->expiryTime??0,'listen'=>$row->listen??'','port'=>$row->port??0,'protocol'=>$row->protocol??'','settings'=>json_encode($allSettings, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),'streamSettings'=>$row->streamSettings??'{}','sniffing'=>$row->sniffing??'{}'];
    $urls[]="$panel_url/panel/inbound/update/$inboundId"; $payloads[]=$fullData;
    $urls[]="$panel_url/xui/inbound/update/$inboundId"; $payloads[]=$fullData;
    $last=''; $err='';
    foreach($urls as $i=>$url){
        $payload=$payloads[$i];
        $headers=['User-Agent: Mozilla/5.0','Accept: application/json, text/plain, */*','X-Requested-With: XMLHttpRequest','Cookie: '.$cookie];
        curl_setopt_array($curl,[CURLOPT_URL=>$url,CURLOPT_RETURNTRANSFER=>true,CURLOPT_ENCODING=>'',CURLOPT_MAXREDIRS=>10,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>18,CURLOPT_FOLLOWLOCATION=>true,CURLOPT_HTTP_VERSION=>CURL_HTTP_VERSION_1_1,CURLOPT_CUSTOMREQUEST=>'POST',CURLOPT_POSTFIELDS=>$payload,CURLOPT_SSL_VERIFYHOST=>false,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_HEADER=>false,CURLOPT_HTTPHEADER=>$headers]);
        $raw=curl_exec($curl); $err=curl_error($curl); $last=$raw;
        if(($raw===false || trim((string)$raw)==='') && is_array($payload)){
        ...
        if($res && isset($res->success)){
            ...
            // اگر مسیر معتبر بود ولی uuid قدیمی را قبول نکرد، مسیرهای بعدی را هم امتحان کن
            $msg=(string)($res->msg??'');
            if(stripos($msg,'not')!==false || stripos($msg,'find')!==false || stripos($msg,'invalid')!==false) continue;
        }
    }
    curl_close($curl);
    if($last===false) return (object)['success'=>false,'msg'=>'خطای اتصال به پنل: '.$err];
    return (object)['success'=>false,'msg'=>'پاسخ معتبر از پنل دریافت نشد یا پنل تغییر را نپذیرفت'];
}
