...
if(preg_match('/^sendNpvOrderFile(\d+)$/', $data, $match) && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true))){
    ...
    $filePath = function_exists('npvFindExistingLockedFile') ? npvFindExistingLockedFile($order['remark']) : (__DIR__ . '/settings/npvtsub/' . $order['remark'] . '.npvtsub');
    if(!$filePath || !file_exists($filePath)){
        sendMessage('⚠️ فایل قفل‌شده این اشتراک روی سرور پیدا نشد. لطفاً به پشتیبانی پیام بدهید.');
        exit;
    }
    $safeRemark = htmlspecialchars((string)$order['remark'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
