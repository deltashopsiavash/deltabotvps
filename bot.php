...
if(preg_match('/^adminQuotaUsers(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $off = (int)$m[1];
    smartSendOrEdit($message_id, "📦 کاربران دارای سهمیه خرید", getAdminQuotaUsersKeys($off), "HTML");
}
if(preg_match('/^adminUser_(\d+)_([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = $m[1];
    $backOff = (int)$m[2];
    $txt = renderUserInfoTitle($uid);
    smartSendOrEdit($message_id, $txt, getUserInfoKeys($uid, "adminUsersList{$backOff}"), "HTML");
}
if(preg_match('/^adminBannedUser_(\d+)_([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = $m[1];
    $backOff = (int)$m[2];
    $txt = renderUserInfoTitle($uid);
    $keys = getUserInfoKeys($uid, "adminBannedUsers{$backOff}");
    smartSendOrEdit($message_id, $txt, $keys, "HTML");
}
if(preg_match('/^adminUser_(\d+)_disc([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $backOff = (int)$m[2];
    $txt = renderUserInfoTitle($uid);
    smartSendOrEdit($message_id, $txt, getUserInfoKeys($uid, "adminDiscountUsers{$backOff}"), "HTML");
}
if(preg_match('/^adminQuotaUser_(\d+)_([0-9]+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = $m[1];
    $backOff = (int)$m[2];
    $txt = renderUserInfoTitle($uid);
    smartSendOrEdit($message_id, $txt, getUserInfoKeys($uid, "adminQuotaUsers{$backOff}"), "HTML");
}
...
if(preg_match('/^uBuyLimitSetPanel(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    ...
    $val = trim((string)$text);
    if(strtolower($val) === 'all' || $val === '*'){
        upsertSettingValue("USER_BUY_VOLUME_LIMIT_" . $uid, '');
...
if(preg_match('/^uBuyQuotaZero(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid=(int)$match[1];
    upsertSettingValue("USER_BUY_VOLUME_LIMIT_" . $uid, '0');
    alert("✅ سهمیه این کاربر صفر شد");
    refreshUserInfoPanel($uid, $message_id);
}
if(preg_match('/^uBuyQuotaNormal(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid=(int)$match[1];
    upsertSettingValue("USER_BUY_VOLUME_LIMIT_" . $uid, '');
    alert("✅ کاربر به حالت عادی برگشت");
    refreshUserInfoPanel($uid, $message_id);
}
if(preg_match('/^uBuyQuotaInc(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("➕ مقدار افزایش سهمیه را به گیگ بفرستید", $cancelKey);
    setUser("uBuyQuotaIncSetPanel" . $match[1]);
}
if(preg_match('/^uBuyQuotaIncSetPanel(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    ...
    $newLimit = $current + (int)$text;
    upsertSettingValue("USER_BUY_VOLUME_LIMIT_" . $uid, (string)$newLimit);
    setUser();
    sendMessage("✅ سهمیه افزایش پیدا کرد",$removeKeyboard);
    sendMessage(renderUserInfoTitle($uid), getUserInfoKeys($uid), "HTML");
}
if(preg_match('/^uBuyQuotaDec(\d+)/',$data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    sendMessage("➖ مقدار کاهش سهمیه را به گیگ بفرستید", $cancelKey);
    setUser("uBuyQuotaDecSetPanel" . $match[1]);
}
if(preg_match('/^uBuyQuotaDecSetPanel(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']){
    if(!is_numeric($text) || (int)$text < 0){
        sendMessage("فقط عدد 0 یا بیشتر بفرست");
        exit();
    }
    ...
    upsertSettingValue("USER_BUY_VOLUME_LIMIT_" . $uid, (string)$newLimit);
    setUser();
    sendMessage("✅ سهمیه کاهش پیدا کرد",$removeKeyboard);
    sendMessage(renderUserInfoTitle($uid), getUserInfoKeys($uid), "HTML");
}
...
if($data=="myInfo"){
    ...
    $myBoughtVolume = getUserBoughtVolumeTotal($from_id);
...
📦 حجم خریدشده: <code> {$myBoughtVolume} گیگ </code>
📏 سقف خرید: <code> {$limitText} </code>
📉 باقیمانده سقف خرید: <code> {$remainText} </code>
