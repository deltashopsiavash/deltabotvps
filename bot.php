...
if($data==='specialOfferToday'){
    specialOfferSyncCompletedSales(); $offer=getActiveSpecialOffer();
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if(!empty($hash_id)){
        $invoiceStmt=$connection->prepare("SELECT `price` FROM `pays` WHERE `hash_id`=? LIMIT 1");
        $invoiceStmt->bind_param('s',$hash_id);
        $invoiceStmt->execute();
        $invoicePay=$invoiceStmt->get_result()->fetch_assoc();
        $invoiceStmt->close();
        if($invoicePay) $price=(int)$invoicePay['price'];
    }
    $keyboard[] = [['text' => '🔁 تغییر پلن', 'callback_data' => "selectCategory{$call_id}_{$sid}_{$match['buyType']}"]];
	$keyboard[] = [['text' => $buttonValues['back_to_main'], 'callback_data' => "mainMenu"]];
