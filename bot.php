...
// ------------------------------------------------------------
// NapsternetV locked subscription support for PasarGuard plans
// Safe v2: no runtime ALTER TABLE; run createDB on fresh installs, and
// use the SQL note below for existing databases if npv_lock column is missing.
// ------------------------------------------------------------
if(!function_exists('npvPlanRequiresDeviceLock')){
    function npvPlanRequiresDeviceLock($plan){
        if(!is_array($plan)) return false;
        return ((string)($plan['type'] ?? '') === 'pasarguard') && !empty($plan['npv_lock']);
    }
}
if(!function_exists('npvValidateDeviceId')){
    function npvValidateDeviceId($deviceId){
        $deviceId = trim((string)$deviceId);
        return (strlen($deviceId) >= 16 && strlen($deviceId) <= 200 && preg_match('/^[A-Za-z0-9_\-\.]+$/', $deviceId));
    }
}
if(!function_exists('npvPackDescription')){
    function npvPackDescription($oldDescription, $deviceId){
        $oldDescription = (string)$oldDescription;
        $deviceId = trim((string)$deviceId);
        if($deviceId === '') return $oldDescription;
        $json = json_decode($oldDescription, true);
        if(is_array($json)){
            $json['npv_device_id'] = $deviceId;
            return json_encode($json, JSON_UNESCAPED_UNICODE);
        }
        if($oldDescription !== '' && strpos($oldDescription, '__NPV_DEVICE_ID__=') !== false){
            return preg_replace('/__NPV_DEVICE_ID__=([^\n\r]+)/', '__NPV_DEVICE_ID__='.$deviceId, $oldDescription);
        }
        return trim($oldDescription . "\n__NPV_DEVICE_ID__=" . $deviceId);
    }
}
if(!function_exists('npvExtractDeviceIdFromPay')){
    function npvExtractDeviceIdFromPay($payInfo){
        if(is_object($payInfo)) $payInfo = (array)$payInfo;
        if(!is_array($payInfo)) return '';
        $desc = (string)($payInfo['description'] ?? '');
        $json = json_decode($desc, true);
        if(is_array($json) && !empty($json['npv_device_id'])) return trim((string)$json['npv_device_id']);
        if(preg_match('/__NPV_DEVICE_ID__=([^\n\r]+)/', $desc, $m)) return trim($m[1]);
        return '';
    }
}
if(!function_exists('npvSendLockedManualDelivery')){
    function npvSendLockedManualDelivery($chatId, $plan, $remark, $volume, $days, $subLink, $deviceId, $backCallback='mainMenu'){
        global $buttonValues;
        $deviceId = trim((string)$deviceId);
        $txt = "✅ سفارش شما ساخته شد.\n\n";
        $txt .= "🔒 این پلن مخصوص NapsternetV است و روی Device ID قفل می‌شود.\n";
        $txt .= "🆔 Device ID ثبت‌شده:\n<code>" . htmlspecialchars($deviceId) . "</code>\n\n";
        $txt .= "لینک خام برای این پلن ارسال نمی‌شود. فایل قفل‌شده توسط پشتیبانی آماده و همینجا ارسال می‌شود.";
        $kb = json_encode(['inline_keyboard'=>[[['text'=>$buttonValues['back_to_main'] ?? 'بازگشت','callback_data'=>$backCallback]]]], JSON_UNESCAPED_UNICODE);
        sendMessage($txt, $kb, 'HTML', $chatId);

        $adminTxt = "🔐 درخواست ساخت فایل Napsternet قفل‌شده\n\n";
        $adminTxt .= "👤 کاربر: <code>" . htmlspecialchars((string)$chatId) . "</code>\n";
        $adminTxt .= "📦 پلن: " . htmlspecialchars((string)($plan['title'] ?? '')) . "\n";
        $adminTxt .= "📝 ریمارک: <code>" . htmlspecialchars((string)$remark) . "</code>\n";
        $adminTxt .= "📆 مدت: " . htmlspecialchars((string)$days) . " روز\n";
        $adminTxt .= "🔋 حجم: " . htmlspecialchars((string)$volume) . " GB\n";
        $adminTxt .= "🆔 Device ID:\n<code>" . htmlspecialchars($deviceId) . "</code>\n\n";
        $adminTxt .= "🔗 Sub Link:\n<code>" . htmlspecialchars((string)$subLink) . "</code>\n\n";
        $adminTxt .= "بعد از ساخت فایل .npvtsub، فایل را برای همین کاربر ارسال کنید.";
        sendToAdmins($adminTxt, null, 'HTML');
        return true;
    }
}
...
if(preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    for($i = 1; $i<= $text; $i++){
        ...
        if(npvPlanRequiresDeviceLock($file_detail ?? [])){ npvSendLockedManualDelivery($uid, $file_detail, $remark, $volume, $days, $subLink, npvExtractDeviceIdFromPay($payInfo ?? []), "mainMenu"); } else { xuiSendOrderDeliveryPhoto($uid, $protocol, $remark, $volume, $days, $botState, $serverType, $vraylink, $botUrl, $uniqid, $subLink, "mainMenu"); }
    ...
    if(!preg_match('/^discountCustomPlanDay/', $userInfo['step'])){
        ...
        if(($match['buyType'] ?? '') == 'much'){
            ...
            $stmt = $connection->prepare("INSERT INTO `pays` (`hash_id`, `description`, `user_id`, `type`, `plan_id`, `volume`, `day`, `price`, `request_date`, `state`, `agent_bought`)
                                        VALUES (?, ?, ?, 'BUY_SUB', ?, ?, ?, ?, ?, 'pending', ?)");
            $payDesc = npvPackDescription($text, $npvDeviceId ?? '');
            $stmt->bind_param("ssiiiiiii", $hash_id, $payDesc, $from_id, $id, $volume, $days, $price, $time, $agentBought);
        }
        $stmt->execute();
...
// First capture Device ID for PasarGuard Napsternet locked plans before continuing purchase
if(preg_match('/^selectPlan(\d+)_(\d+)_(\w+)/', $data, $npvPreMatch) && ($botState['sellState']=="on" || $from_id == $admin || $userInfo['isAdmin'] == true)){
    $npvPlanId = (int)$npvPreMatch[1];
    $stmt = $connection->prepare("SELECT * FROM `server_plans` WHERE `id`=? AND `active`=1 LIMIT 1");
    $stmt->bind_param("i", $npvPlanId);
    $stmt->execute();
    $npvPlan = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if(npvPlanRequiresDeviceLock($npvPlan)){
        delMessage();
        sendMessage("🔒 این پلن مخصوص NapsternetV است و روی Device ID قفل می‌شود.\n\nلطفاً Device ID برنامه NapsternetV را دقیق ارسال کنید:", $cancelKey, 'HTML');
        setUser('npvDevice_' . $data);
        exit();
    }
}
if(preg_match('/^npvDevice_(selectPlan\d+_\d+_\w+)/', (string)($userInfo['step'] ?? ''), $npvStepMatch) && $text != $buttonValues['cancel']){
    $deviceId = trim((string)$text);
    if(!npvValidateDeviceId($deviceId)){
        sendMessage("❌ Device ID معتبر نیست.\nلطفاً Device ID داخل NapsternetV را بدون فاصله اضافه ارسال کنید.", $cancelKey, 'HTML');
        exit();
    }
    setUser($deviceId, 'temp');
    if(is_array($userInfo)) $userInfo['temp'] = $deviceId;
    $data = $npvStepMatch[1];
    $text = '';
}
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
	...
	$sid = $respd['server_id'];
    $npvDeviceId = '';
    if(npvPlanRequiresDeviceLock($respd)){
        $npvDeviceId = trim((string)($userInfo['temp'] ?? ''));
        if(!npvValidateDeviceId($npvDeviceId)){
            sendMessage("🔒 لطفاً Device ID برنامه NapsternetV را ارسال کنید:", $cancelKey, 'HTML');
            setUser('npvDevice_selectPlan' . $id . '_' . $call_id . '_' . ($match['buyType'] ?? 'one'));
            exit();
        }
    }
    $requestedVolume = (float)($respd['volume'] ?? 0) * (isset($accountCount) ? (int)$accountCount : 1);
    ...
    if($price == 0 or ($from_id == $admin)){
        if(!empty($freeVolumeQuota)){
            ...
            if(isset($accountCount)){
                ...
                $payDesc = npvPackDescription($remark, $npvDeviceId ?? '');
                $stmt->bind_param("ssiiii", $hash_id, $payDesc, $from_id, $id, $time, $agentBought);
            }
            $stmt->execute();
            $stmt->close();
        ...
        if(!preg_match('/^discountSelectPlan/', $userInfo['step'])){
            ...
            if(isset($accountCount)){
                ...
                $stmt = $connection->prepare("INSERT INTO `pays` (`hash_id`, `description`, `user_id`, `type`, `plan_id`, `volume`, `day`, `price`, `request_date`, `state`, `agent_bought`)
                                            VALUES (?, ?, ?, 'BUY_SUB', ?, '0', '0', ?, ?, 'pending', ?)");
                $payDesc = npvPackDescription($remark, $npvDeviceId ?? '');
                $stmt->bind_param("ssiiiii", $hash_id, $payDesc, $from_id, $id, $price, $time, $agentBought);
            }
            $stmt->execute();
            $rowId = $stmt->insert_id;
            $stmt->close();
    ...
    if(!empty($npvDeviceId)) setUser('', 'temp');
}
...
" . ($response->msg), null, null);
        ...
        if(npvPlanRequiresDeviceLock($file_detail ?? [])){ npvSendLockedManualDelivery($from_id, $file_detail, $remark, $volume, $days, $subLink, npvExtractDeviceIdFromPay($payInfo ?? []), 'mainMenu'); } else { xuiSendOrderDeliveryPhoto($from_id, $protocol, $remark, $volume, $days, $botState, $serverType, $vraylink, $botUrl, $uniqid, $subLink, 'mainMenu'); }
...
if(preg_match('/(addNewRahgozarPlan|addNewPlan|addNewMarzbanPlan|addNewPasarguardPlan)/',$userInfo['step']) and $text!=$buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if($step==54 and $text!=$buttonValues['cancel']){
        ...
        if($userInfo['step'] == "addNewPlan"){
            ...
            // For PasarGuard plans we ask whether the delivery must be NapsternetV Device-ID locked.
            $sql = ("UPDATE `server_plans` SET `volume`=?, `type`='pasarguard', `inbound_id`=0, `acount`=999999, `limitip`=0, `step`=156 WHERE `active`=0");
            $msg = '🔒 آیا این پلن با Device ID برنامه NapsternetV قفل شود؟';
        ...
        if($userInfo['step'] == "addNewPasarguardPlan"){
            sendMessage($msg, json_encode(['inline_keyboard'=>[
                [['text'=>'✅ بله، قفل شود', 'callback_data'=>'npvPlanLock_1']],
                [['text'=>'❌ خیر، عادی باشد', 'callback_data'=>'npvPlanLock_0']]
            ]]), 'HTML');
        }else{
            sendMessage($msg);
        }
    }
    if($step==156 and $text!=$buttonValues['cancel'] and preg_match('/^npvPlanLock_(0|1)$/', $data, $npvLockMatch)){
        $npvLock = (int)$npvLockMatch[1];
        $stmt = $connection->prepare("UPDATE `server_plans` SET `npv_lock`=?, `step`=4 WHERE `active`=0");
        $stmt->bind_param("i", $npvLock);
        $stmt->execute();
        $stmt->close();
        smartSendOrEdit($message_id, ($npvLock ? "✅ قفل Device ID فعال شد." : "✅ پلن عادی ثبت می‌شود.") . "

🔻یه توضیح برای پلن پاسارگارد بنویس:", null, 'HTML');
        exit();
    }
