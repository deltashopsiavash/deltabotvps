...
if((($data=="botSettings" || $data=="bot_setting" || $data=="botSetting") or preg_match("/^changeBot(\w+)/",$data,$match)) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if($data!="botSettings"){
...
if(preg_match('/^yesSend2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Determine current bot instance id (0 = mother bot)
    $bidArg = 0;
    if(isset($_GET['bid'])){
        $bidArg = (int)$_GET['bid'];
    }elseif(isset($GLOBALS['currentBotInstanceId'])){
        $bidArg = (int)$GLOBALS['currentBotInstanceId'];
    }

    // Worker auth key (used for HTTP fallback)
    $tokenToUse = $GLOBALS['botToken'] ?? ($botToken ?? '');
    $adminToUse = (string)($GLOBALS['admin'] ?? ($admin ?? ''));
    $workerKey = hash('sha256', $tokenToUse . '|' . $adminToUse);

    // 1) Preferred: run via CLI in background (fast + reliable)
    if(file_exists($worker) && function_exists('shell_exec')){
        ...
        // 2) Fallback: trigger worker via HTTP (works when shell_exec is disabled)
        // NOTE: broadcast_worker.php will validate ?k= to prevent public abuse.
        $baseUrl = $GLOBALS['botUrl'] ?? ($botUrl ?? '');
        if(!empty($baseUrl)){
            $u = rtrim($baseUrl, '/');
            $url = $u . '/broadcast_worker.php?bid=' . urlencode((string)$bidArg) . '&k=' . urlencode($workerKey);
