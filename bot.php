...
// Callback data helpers (supports db names with '-' etc.)
function cbEncode($s){
...
if(preg_match('/^adminResDBInfo_(.+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[
        [
            ['text'=>"📤 بکاپ بگیر", 'callback_data'=>"adminResDBBackup_".cbEncode($dbn)],
            ['text'=>"🗑 حذف", 'callback_data'=>"adminResDBDropAsk_".cbEncode($dbn)],
        ],
        [
...
if(preg_match('/^adminResDBDropYes_(.+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // IMPORTANT: answer callback ASAP to avoid Telegram "loading..." stuck state
    if(isset($update->callback_query) && !empty($update->callback_query->id)){
        bot('answerCallbackQuery', ['callback_query_id'=>$update->callback_query->id]);
    }

    // Show progress immediately (prevents client from looking frozen)
    smartSendOrEdit($message_id, "⏳ در حال حذف دیتابیس...\n\n`{$dbn}`\n\nلطفاً چند ثانیه صبر کنید.", null, "Markdown");

    // Prefer async worker (DROP DATABASE can be slow on some hosts)
// Run the DROP after finishing the HTTP response when possible.
// This avoids Telegram webhook timeouts and removes the dependency on shell_exec/nohup.
@ignore_user_abort(true);
@set_time_limit(0);

// If we're on PHP-FPM, we can return 200 OK to Telegram immediately, then continue work.
if (function_exists('fastcgi_finish_request')) {
    ...
    // Fallback: synchronous drop (best effort)
    @set_time_limit(60);
