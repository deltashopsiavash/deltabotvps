...
function deltaPanelLoginCookies($server_info){
    ...
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $panel_url . '/login');
    curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1);
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($curl, CURLOPT_POST, 1);
    ...
    if($response === false){ $err=curl_error($curl); curl_close($curl); return [null, null, 'خطا در اتصال به پنل: '.$err]; }
    ...
    if(isset($loginResponse['success']) && !$loginResponse['success']){ curl_close($curl); return [null, null, $loginResponse['msg'] ?? 'ورود به پنل ناموفق بود']; }
    if(!$cookies){ curl_close($curl); return [null, null, 'کوکی پنل دریافت نشد']; }
...
function deltaXuiPostClientOrInbound($server_info,$row,$settings,$client_key,$oldUuid){
    ...
    foreach($urls as $i=>$url){
        curl_setopt_array($curl,[CURLOPT_URL=>$url,CURLOPT_RETURNTRANSFER=>true,CURLOPT_ENCODING=>'',CURLOPT_MAXREDIRS=>10,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>18,CURLOPT_FOLLOWLOCATION=>true,CURLOPT_HTTP_VERSION=>CURL_HTTP_VERSION_1_1,CURLOPT_CUSTOMREQUEST=>'POST',CURLOPT_POSTFIELDS=>$payloads[$i],CURLOPT_SSL_VERIFYHOST=>false,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_HEADER=>false,CURLOPT_HTTPHEADER=>['User-Agent: Mozilla/5.0','Accept: application/json, text/plain, */*','X-Requested-With: XMLHttpRequest','Cookie: '.$cookie]]);
