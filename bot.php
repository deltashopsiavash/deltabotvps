...
// --- Callback safe encode/decode (for db names) ---
if(!function_exists('cbEncode')){
    function cbEncode($s){ return rawurlencode($s); }
}
if(!function_exists('cbDecode')){
    function cbDecode($s){ return rawurldecode($s); }
}
// --- Fire-and-forget local HTTP request (no shell_exec needed) ---
if(!function_exists('fireAndForgetLocal')){
    function fireAndForgetLocal($path, $query = []){
        $host = '127.0.0.1';
        $ports = [80, 443];
...
if(preg_match('/^adminResDBList_(\d+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[]];
    foreach($slice as $dbn){
        $keys['inline_keyboard'][] = [[ 'text'=>"🗄 ".$dbn, 'callback_data'=>"adminResDBInfo_".cbEncode($dbn) ]];
    }
...
if(preg_match('/^adminResDBBackup_([A-Za-z0-9_]+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Notify
    smartSendOrEdit($message_id, "⏳ در حال ساخت بکاپ...\nلطفاً صبر کنید", ['inline_keyboard'=>[[['text'=>"🔙 برگشت",'callback_data'=>"adminResDBInfo_".cbEncode($dbn)]]]]);
...
if(preg_match('/^adminResDBDropAsk_(.+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $keys = ['inline_keyboard'=>[
        [
            ['text'=>"✅ بله حذف کن", 'callback_data'=>"adminResDBDropYes_".cbEncode($dbn)],
            ['text'=>"❌ نه", 'callback_data'=>"adminResDBInfo_".cbEncode($dbn)],
        ],
    ]];
    smartSendOrEdit($message_id, $text, $keys, "Markdown");
}
...
if(preg_match('/^adminResDBDropYes_(.+)$/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Answer callback ASAP so Telegram UI doesn't hang
    if(isset($update->callback_query) && !empty($update->callback_query->id)){
        bot('answerCallbackQuery', ['callback_query_id'=>$update->callback_query->id]);
    }

    // Show progress immediately
    smartSendOrEdit($message_id, "⏳ در حال حذف دیتابیس...\n\n`{$dbn}`\n\nلطفاً چند ثانیه صبر کنید.", null, "Markdown");

    // Trigger worker via local HTTP (fire-and-forget) to avoid webhook timeout
    $tokenToUse = $GLOBALS['botToken'] ?? ($botToken ?? '');
    $sig = hash_hmac('sha256', $from_id . '|' . $message_id . '|' . $dbn, $tokenToUse);

    $okSpawn = fireAndForgetLocal('dbdrop_worker_http.php', [
        'chat_id' => $from_id,
    ...
    if(!$okSpawn){
        // As a fallback, try synchronous drop (may hang on some hosts)
        @set_time_limit(60);
        ...
        if($ok){
            ...
            smartSendOrEdit($message_id, "❌ حذف انجام نشد.\n\nخطا: {$err}", ['inline_keyboard'=>[[['text'=>"🔙 برگشت",'callback_data'=>"adminResDBInfo_".cbEncode($dbn)]]]], "Markdown");
        }
    }
    exit;
}
