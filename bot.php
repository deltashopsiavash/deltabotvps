...
if($data == 'buySubscription' && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true))){
    $buttons = [
        [['text'=>$buttonValues['buy_single'],'callback_data'=>'buySubSingle']],
        [['text'=>$buttonValues['buy_group'],'callback_data'=>'buySubGroup']]
    ];

    $stmt = $connection->prepare("SELECT `fileid` FROM `orders_list` WHERE `userid`=? AND `status`=1 ORDER BY `id` DESC LIMIT 1");
    $stmt->bind_param('i', $from_id);
    $stmt->execute();
    $lastOrder = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    if(!empty($lastOrder['fileid'])){
        $planId = (int)$lastOrder['fileid'];
        $stmt = $connection->prepare("SELECT `id`,`catid`,`active` FROM `server_plans` WHERE `id`=? LIMIT 1");
        $stmt->bind_param('i', $planId);
        $stmt->execute();
        $planRow = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        if(!empty($planRow) && (int)($planRow['active'] ?? 0) === 1){
            $buttons[] = [['text'=>$buttonValues['buy_again_last'],'callback_data'=>'buySubRepeat']];
        }
    }

    $buttons[] = [['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']];
    smartSendOrEdit($message_id, "🛒 نوع خریدت رو انتخاب کن", json_encode(['inline_keyboard'=>$buttons]));
    exit;
}
if($data == 'buySubSingle') $data = 'agentOneBuy';
if($data == 'buySubGroup') $data = 'agentMuchBuy';
if($data == 'buySubRepeat' && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true))){
    $stmt = $connection->prepare("SELECT `fileid` FROM `orders_list` WHERE `userid`=? AND `status`=1 ORDER BY `id` DESC LIMIT 1");
    $stmt->bind_param('i', $from_id);
    $stmt->execute();
    $lastOrder = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if(empty($lastOrder['fileid'])){
        alert('سفارش قبلی فعالی پیدا نشد');
        exit;
    }
    $planId = (int)$lastOrder['fileid'];
    $stmt = $connection->prepare("SELECT `catid`,`active` FROM `server_plans` WHERE `id`=? LIMIT 1");
    $stmt->bind_param('i', $planId);
    $stmt->execute();
    $planRow = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if(empty($planRow) || (int)($planRow['active'] ?? 0) !== 1){
        alert('پلن آخرین خرید در دسترس نیست');
        exit;
    }
    $data = 'selectPlan' . $planId . '_' . ((int)$planRow['catid']) . '_none';
}
...
if((preg_match('/^discountCustomPlanDay(\d+)/',$userInfo['step'], $match) || preg_match('/enterCustomPlanName(\d+)_(\d+)_(\d+)_(\d+)_(?<buyType>\w+)/',$userInfo['step'], $match)) && ($botState['sellState']=="on" ||$from_id ==$admin) && $text != $buttonValues['cancel']){
    ...
    $keyboard[] = [['text' => $buttonValues['change_plan'], 'callback_data' => "selectCategory{$call_id}_{$sid}_{$match['buyType']}"]];
	$keyboard[] = [['text' => $buttonValues['cancel'], 'callback_data' => "mainMenu"]];
    $price = ($price == 0) ? 'رایگان' : number_format($price).' تومان ';
        ...
        if($match['buyType'] == "much"){
            if(is_numeric($text)){
                ...
                if($text >= 1 && $text <= 10){
                    $accountCount = $text;
                    setUser();
                }else{sendMessage('لطفا عددی بین 1 تا 10 وارد کنید'); exit(); }
...
$acc_text .= xuiBuildOrderLinksText($botState, $botUrl, $uniqid, $subLink);
        ...
        QRcode::png(xuiChooseOrderQrPayload($botState, $vray_link, $subLink), $file, $ecc, $pixel_Size, $frame_Size);
    	addBorderImage($file);
...
if(preg_match('/freeTrial(\d+)_(?<buyType>\w+)/',$data,$match)) {
    ...
    $days = $file_detail['days'];
     // apply per-user test limit as max days (if set > 0)
