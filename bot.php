...
برای پلن نامحدود پاسارگارد می‌توانید <code>all</code> یا <code>نامحدود</code> ارسال کنید.", null, 'HTML');
        ...
        if($userInfo['step'] == "addNewPlan"){
            ...
            $sql = ("UPDATE `server_plans` SET `volume`=?, `protocol`='" . $npvProtocolSql . "', `type`='pasarguard', `inbound_id`=0, `acount`=999999, `limitip`=0, `step`=4 WHERE `active`=0");
            $msg = ((float)$text <= 0) ? '🔐 پلن نامحدود قفل‌شونده ثبت شد. حالا یه توضیح برای پلن پاسارگارد بنویس:' : '🔻یه توضیح برای پلن پاسارگارد بنویس:';
