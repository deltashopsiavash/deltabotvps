...
if(preg_match('/^payTextReceipt\|([^|]+)\|(.*)$/', $userInfo['step'] ?? '', $match) && $text != $buttonValues['cancel']){
    ...
    $offerPaymentError='';
    if(!timedOfferEnsurePayReservation($payInfo,604800,$offerPaymentError)){
        sendMessage('❌ مهلت فاکتور ویژه تمام شده است: '.$offerPaymentError,$removeKeyboard);
        setUser();
        exit;
    }
...
require_once __DIR__ . '/settings/timed_offers_bot.php';
...
if(preg_match('/payWithTronWallet(.*)/',$data,$match)) {
    ...
    $offerPaymentError='';
    if(!timedOfferEnsurePayReservation($payInfo,172800,$offerPaymentError)){
        alert('این فاکتور ویژه دیگر قابل پرداخت نیست: '.$offerPaymentError,true);
        exit;
    }
...
if(preg_match('/payWithWeSwap(.*)/',$data,$match)) {
    ...
    $offerPaymentError='';
    if(!timedOfferEnsurePayReservation($payInfo,7200,$offerPaymentError)){
        alert('این فاکتور ویژه دیگر قابل پرداخت نیست: '.$offerPaymentError,true);
        exit;
    }
...
if(preg_match('/havePaiedWeSwap(.*)/',$data,$match)) {
    ...
    $offerPaymentError='';
    if(!timedOfferEnsurePayReservation($payInfo,900,$offerPaymentError)){
        alert('پرداخت تأیید شد ولی رزرو پیشنهاد نیاز به بررسی مدیر دارد.',true);
        $stmt=$connection->prepare("UPDATE `pays` SET `state`='need_admin' WHERE `id`=?");
        $payRowId=(int)$payInfo['id']; $stmt->bind_param('i',$payRowId); $stmt->execute(); $stmt->close();
        sendToAdmins("⚠️ پرداخت پیشنهاد نیاز به بررسی دارد\nفاکتور: <code>".timedOfferEsc($payInfo['hash_id'])."</code>\nخطا: ".timedOfferEsc($offerPaymentError),null,'HTML');
        sendMessage('پرداخت شما تأیید شد و برای بررسی پیشنهاد ویژه به مدیر ارسال شد.');
        exit;
    }
    $stmt = $connection->prepare("UPDATE `pays` SET `state` = 'approved' WHERE `hash_id` = ?");
    $stmt->bind_param("s", $match[1]);
    ...
    $offerCompleteError='';
    if(!timedOfferCompletePay($payInfo,$offerCompleteError)){
        error_log('Timed offer completion failed for '.$payInfo['hash_id'].': '.$offerCompleteError);
        sendToAdmins("⚠️ سرویس تحویل شد اما ثبت فروش پیشنهاد نیاز به بررسی دارد\nفاکتور: <code>".timedOfferEsc($payInfo['hash_id'])."</code>\nخطا: ".timedOfferEsc($offerCompleteError),null,'HTML');
    }
...
if(preg_match('/^npvDeviceForPlan(\d+)_(\d+)_(\w+)_([^\s]*)$/', $userInfo['step'], $npvMatch) && $text != $buttonValues['cancel']){
    ...
    setUser('__NPV_DEVICE_ID__=' . $deviceIdInput, 'temp');
    setUser();
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
    ...
    if(preg_match('/^discountSelectPlan/', $userInfo['step'])){
        ...
        $stmt->close();
        if((int)($payInfo['offer_id'] ?? 0) > 0){
            sendMessage('کد تخفیف روی فاکتور پیشنهاد ویژه قابل اعمال نیست.');
            setUser();
            exit;
        }
    ...
    $timedOfferId = function_exists('timedOfferIdFromBuyType') ? timedOfferIdFromBuyType($match['buyType'] ?? '') : 0;
    $activeTimedOffer = null;
    if($timedOfferId > 0){
        $activeTimedOffer = timedOfferGetActive($timedOfferId);
        if(!$activeTimedOffer || (int)$activeTimedOffer['plan_id'] !== (int)$id){
            sendMessage('⌛️ این پیشنهاد دیگر فعال نیست یا موجودی آن تمام شده است.', getMainKeys());
            setUser();
            exit();
        }
        if($activeTimedOffer['available_count'] !== null && (int)$activeTimedOffer['available_count'] <= 0){
            sendMessage('⏳ همه موجودی این پیشنهاد فعلاً در رزرو پرداخت است؛ کمی بعد دوباره بررسی کنید.', getMainKeys());
            setUser();
            exit();
        }
    }
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
	...
	$keyboard = array();
    $unitPrice = (int)$respd['price'];
    if($timedOfferId > 0 && $activeTimedOffer){
        $unitPrice = timedOfferDiscountedPrice($unitPrice, (int)$activeTimedOffer['discount_percent']);
    }
    $price = $unitPrice;
    ...
    // Existing free-volume entitlements remain valid. Other user/discount-code
    // reductions do not stack on top of a timed offer.
    if(!empty($freeVolumeQuota)) $price = 0;
    ...
    if(($price == 0 || $from_id == $admin) && $timedOfferId <= 0){
        if(!empty($freeVolumeQuota)){
            $hash_id = RandomString();
            $stmt = $connection->prepare("DELETE FROM `pays` WHERE `user_id` = ? AND `type` = 'BUY_SUB' AND `state` = 'pending'");
            $stmt->bind_param("i", $from_id);
        ...
        if(!preg_match('/^discountSelectPlan/', $userInfo['step'])){
            ...
            if($timedOfferId <= 0) $price = applyUserPercentDiscount($from_id, $price);
            ...
            if($timedOfferId > 0){
                $offerReserveError = '';
                if(!timedOfferReservePay($timedOfferId, $hash_id, $rowId, $from_id, $offerReserveError, 1800)){
                    $stmt=$connection->prepare("DELETE FROM `pays` WHERE `id`=? AND `state`='pending'");
                    $stmt->bind_param('i',$rowId); $stmt->execute(); $stmt->close();
                    sendMessage('❌ فاکتور پیشنهاد ساخته نشد: ' . $offerReserveError, getMainKeys());
                    setUser();
                    exit();
                }
            }
        ...
        if($timedOfferId <= 0 && !preg_match('/^discountSelectPlan/', $userInfo['step'])) $keyboard[] = [['text' => " 🎁 نکنه کد تخفیف داری؟ ",  'callback_data' => "haveDiscountSelectPlan_" . $match[1] . "_" . $match[2] . "_" . $rowId]];
...
if(preg_match('/accept(.*)/',$data, $match) and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $offerPaymentError='';
    if(!timedOfferEnsurePayReservation($payInfo,900,$offerPaymentError)){
        alert('تأیید ممکن نیست: '.$offerPaymentError,true);
        exit;
    }
...
if(preg_match('/decline(\d+)_(\d+)/',$userInfo['step'],$match) && ($from_id == $admin || $userInfo['isAdmin'] == true) and $text != $buttonValues['cancel']){
    ...
    $stmt=$connection->prepare("SELECT `hash_id` FROM `pays` WHERE `user_id`=? AND `state`='have_sent' ORDER BY `id` DESC LIMIT 1");
    $stmt->bind_param('i',$uid); $stmt->execute(); $declinedPay=$stmt->get_result()->fetch_assoc(); $stmt->close();
    if($declinedPay){
        $declinedHash=(string)$declinedPay['hash_id'];
        $stmt=$connection->prepare("UPDATE `pays` SET `state`='declined' WHERE `hash_id`=? AND `state`='have_sent'");
        $stmt->bind_param('s',$declinedHash); $stmt->execute(); $stmt->close();
        timedOfferReleasePay($declinedHash);
    }
    editKeys(
