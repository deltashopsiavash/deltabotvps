...
if(preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/',$userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    for($i = 1; $i<= $text; $i++){
        ...
        if($serverType == "marzban" || $serverType == "pasarguard"){
            $subLink = (xuiBotStateIsOn($botState, 'subLinkState', 'on') || xuiBotStateIsOn($botState, 'qrSubState', 'off'))?xuiResolveClientSubLink($server_id, $panelUrl, $response->sub_link ?? '', $inbound_id, '', $remark):"";
            $token = trim((string)$subLink);
            if($token === '') $token = trim((string)($response->sub_link ?? ''));
            // Keep callback/order lookup stable and short. Do NOT destroy full URLs with str_replace('/sub/','').
            $uniqid = RandomString(30);
            $vraylink = $subLink !== '' ? [$subLink] : [];
            $vray_link = json_encode($vraylink, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
        }
        else{
...
<pre>" . htmlspecialchars($subLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</pre>", null, 'HTML', $from_id);
...
if(preg_match('/changAccountConnectionLink(\d+)/', $data,$match)){
    ...
    if($serverType == "marzban" || $serverType == "pasarguard"){
        ...
        $vraylink = $res->links ?? [];
        $newToken = str_replace("/sub/", "", (string)($res->subscription_url ?? ''));
        $newUuid = $newToken;
        if(empty($newToken)){
            sendMessage("❌ مرزبان لینک ساب جدید برنگرداند.", null, null, $from_id);
            return;
        }
    }else{
    ...
    if(!empty($freshSubLink)) $freshText .= "🌐 subscription:\n<code>" . htmlspecialchars($freshSubLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</code>\n\n";
