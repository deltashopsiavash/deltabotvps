...
function pgRenewableOrdersKeyboard($userId,$serverId,$buyType){
    global $connection;
    $stmt=$connection->prepare("SELECT `id`,`remark`,`expire_date` FROM `orders_list` WHERE `userid`=? AND `server_id`=? AND `status`=1 ORDER BY `id` DESC LIMIT 30");
    $stmt->bind_param('ii',$userId,$serverId); $stmt->execute(); $res=$stmt->get_result(); $stmt->close();
    $rows=[];
    $serviceRows=[];
    while($o=$res->fetch_assoc()){
        $label='🔁 '.($o['remark'] ?: ('سرویس #'.$o['id']));
        if((int)$o['expire_date']>0) $label .= ' | '.jdate('Y/m/d',(int)$o['expire_date']);
        $serviceRows[]=[['text'=>$label,'callback_data'=>'pgRenewMenu'.(int)$o['id']]];
    }
    if(!$serviceRows) return null;
    $rows[]=[['text'=>'➕ خرید اشتراک جدید','callback_data'=>'selectServerContinue'.$serverId.'_'.$buyType]];
    $rows=array_merge($rows,$serviceRows);
