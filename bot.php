...
function deltaXuiPostClientOrInbound($server_info,$row,$settings,$client_key,$oldUuid){
    ...
    $urls=[]; $payloads=[];
    // فقط آپدیت کامل inbound استفاده می‌شود چون بعضی نسخه‌های x-ui/updateClient پاسخ نامعتبر می‌دهند یا هنگ می‌کنند.
    // مسیر جایگزین: آپدیت کل inbound برای همه پنل‌ها
    ...
    foreach($urls as $i=>$url){
        ...
        if(stripos((string)$raw,'success')!==false && stripos((string)$raw,'true')!==false){
            $res=json_decode($raw);
            if(!$res) $res=(object)['success'=>true];
            curl_close($curl);
            return $res;
        }
        $res=json_decode($raw);
