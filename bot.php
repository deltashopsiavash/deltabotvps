...
if($specialReservationHash!==''){
    ...
    if($reservationPay && (int)($reservationPay['special_offer_id'] ?? 0)>0){
        ...
        if(!specialOfferReservationIsValid($reservationPay,true)){
            setUser();
            $reservationMessage='⏳ زمان رزرو پنج‌دقیقه‌ای این فاکتور تمام شده است؛ دوباره از صفحه پیشنهاد فاکتور بسازید.';
...
if($data==='specialOfferToday'){
    ...
    $txt="🔥  {$safeTitle}\n\n"
        ...
        ."🕛 پایان: ".specialOfferFormatDate($offer['ends_at']);
    $invoiceWillReserve=((int)$offer['ends_at']-time())>=300;
    ...
    elseif(!$invoiceWillReserve) $txt.="\n\n⚡ کمتر از ۵ دقیقه مانده؛ فاکتور صادر می‌شود اما موجودی رزرو نمی‌شود و هنگام پرداخت دوباره بررسی خواهد شد.";
    $keyRows=[];
...
if(preg_match('/^specialOfferBuy(\d+)$/',$data,$m)){
    ...
    $offerPrice=specialOfferApplyDiscount((int)$offer['plan_price'],$offer);
    if(($botState['walletState'] ?? 'off')!=='on'){ alert('پرداخت از موجودی کیف پول در حال حاضر غیرفعال است.',true); exit(); }
    if((int)($userInfo['wallet'] ?? 0)<$offerPrice){ alert('موجودی کیف پول شما برای خرید این پیشنهاد کافی نیست.',true); exit(); }
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
    ...
    if(preg_match('/^discountSelectPlan/', $userInfo['step'])){
        ...
        if($payInfo && (int)($payInfo['special_offer_id'] ?? 0)>0 && !specialOfferReservationIsValid($payInfo,true)){
            sendMessage('⏳ زمان رزرو پنج‌دقیقه‌ای این فاکتور تمام شده است؛ دوباره از صفحه پیشنهاد فاکتور بسازید.');
            setUser(); setUser('','temp');
            exit();
        }
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if($specialOffer){
        ...
        specialOfferSyncCompletedSales();
        if(($botState['walletState'] ?? 'off')!=='on'){
            $walletMessage='پرداخت از موجودی کیف پول در حال حاضر غیرفعال است.';
            if(isset($update->callback_query)) alert($walletMessage,true); else sendMessage($walletMessage);
            setUser(); setUser('','temp');
            exit();
        }
    ...
    if($specialOffer){
        ...
        $freeVolumeQuota=false;
        if((int)($userInfo['wallet'] ?? 0)<$price){
            $missing=max(0,$price-(int)($userInfo['wallet'] ?? 0));
            $balanceMessage="💰 موجودی کیف پول شما کافی نیست.\n\nمبلغ موردنیاز: ".number_format($price)." تومان\nموجودی فعلی: ".number_format((int)($userInfo['wallet'] ?? 0))." تومان\nکسری موجودی: ".number_format($missing)." تومان";
            if(isset($update->callback_query)) alert('موجودی کیف پول شما برای این پیشنهاد کافی نیست.',true); else sendMessage($balanceMessage);
            setUser(); setUser('','temp');
            exit();
        }
    }
    ...
    if(isset($accountCount)){
    ...
    }elseif($invoiceOfferId>0){
        // Do not pass offer invoices through the configurable normal template:
        // old/custom templates can contain the original plan price.  The only
        // payable amount shown here is the exact value persisted in pays.price.
        $currentWallet = (int)($userInfo['wallet'] ?? 0);
        $walletAfter = max(0,$currentWallet-(int)$price);
        ...
        $offerPercent=(int)($specialOffer['discount_percent'] ?? 0);
        $reserveClock=$invoiceReservedUntil>0 ? (function_exists('jdate') ? jdate('H:i:s',$invoiceReservedUntil) : date('H:i:s',$invoiceReservedUntil)) : '';
        $reservationText=$invoiceReservedUntil>0
            ? "⏳ رزرو اختصاصی شما: <b>۵ دقیقه</b> (تا {$reserveClock})\n⚠️ بعد از پایان رزرو، این فاکتور خودکار منقضی می‌شود."
            : "⚡ <b>فاکتور بدون رزرو</b>\nموجودی هنگام پرداخت دوباره بررسی می‌شود و فاکتور فقط تا پایان طرح معتبر است.";
        $msg="🧾 <b>فاکتور پیشنهاد ویژه</b>\n\n"
            ."📦 پلن: <b>{$safeName}</b>\n"
            ."🔋 حجم: <b>".($respd['volume']??0)." گیگابایت</b>\n"
            ."⏰ مدت: <b>".($respd['days']??0)." روز</b>\n"
            ."🏷 تخفیف طرح: <b>{$offerPercent}٪</b>\n"
            ."✅ مبلغ نهایی قابل پرداخت: <b>{$priceC}</b>\n\n"
            ."💳 روش پرداخت: <b>فقط موجودی کیف پول</b>\n"
            ."{$reservationText}\n\n"
            ."💰 موجودی فعلی: <b>".number_format($currentWallet)." تومان</b>\n"
            ."💳 موجودی بعد از خرید: <b>".number_format($walletAfter)." تومان</b>";
        if($safeDesc!=='') $msg.="\n📝 توضیحات: {$safeDesc}";
    }else{
