...
if((in_array($data,["botSettings","manageBotSettings"], true) or preg_match("/^changeBot(\w+)/",$data,$match)) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if($data!="botSettings"){
...
if(preg_match('/^showQr(Sub|Config)(\d+)/',$data,$match)){
    ...
    define('IMAGE_HEIGHT',540);
    if($match[1] == "Sub"){
        $subLink = getOrderSubscriptionLinkByServer($order['server_id'], $order['token']);
        if(empty($subLink)){
            alert("لینک ساب پنل برای این سرویس یافت نشد", true);
            exit;
        }
        $file = RandomString() .".png";
        $ecc = 'L';
...
if(preg_match('/changAccountConnectionLink(\d+)/', $data,$match)){
    ...
    if($serverType == "marzban"){
        ...
        $vraylink = $res->links;
        $newUuid = $newToken = extractSubscriptionToken($res->subscription_url);
    }else{
        $response = getJson($server_id)->obj;
        if($inboundId == 0){
            foreach($response as $row){
        ...
        $newToken = $order['token'] ?? '';
        $refreshResponse = getJson($server_id);
        if(is_object($refreshResponse) && isset($refreshResponse->obj)){
            $targetRow = null;
            if(!empty($inboundId)) $targetRow = findInboundRowById($refreshResponse->obj, $inboundId);
            if(!$targetRow && is_array($refreshResponse->obj)){
                foreach($refreshResponse->obj as $refreshRow){
                    $settingsRaw = $refreshRow->settings ?? '';
                    ...
                    foreach(($settings['clients'] ?? []) as $client){
                        ...
                        if(($client['id'] ?? null) == $newUuid || ($client['password'] ?? null) == $newUuid || ($client['email'] ?? null) == $remark){
                            $targetRow = $refreshRow;
                            break 2;
                        }
                    }
                }
            ...
            if($targetRow){
                $detectedSubId = getClientSubIdFromInboundRow($targetRow, $newUuid, $remark);
                if(!empty($detectedSubId)) $newToken = $detectedSubId;
            }
        }
    }
