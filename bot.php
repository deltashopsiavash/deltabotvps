...
// ------------------------------------------------------------
// VPSBot integration (bridge mode)
// This bot remains the ONLY webhook receiver.
// When user enters "cloud" section, we forward updates to a local
// VPSBot bridge service (aiogram) which will send responses using the
// SAME bot token.
//
// Bridge default: http://127.0.0.1:8090/update
// Override via setting: VPSBOT_BRIDGE_URL
// ------------------------------------------------------------
if(!function_exists('vpsbot_bridge_url')){
    function vpsbot_bridge_url(){
        $u = getSettingValue('VPSBOT_BRIDGE_URL','http://127.0.0.1:8090/update');
        return $u ?: 'http://127.0.0.1:8090/update';
    }
}

if(!function_exists('vpsbot_forward')){
    function vpsbot_forward($payload){
        $url = vpsbot_bridge_url();
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload, 488));
...
if(!function_exists('vpsbot_synth_message_update')){
    function vpsbot_synth_message_update($from_id, $chat_id, $text){
        $now = time();
        return [
            'update_id' => (int)($now % 1000000000),
            'message' => [
                'message_id' => (int)($now % 1000000000),
                'date' => $now,
                'chat' => ['id' => (int)$chat_id, 'type' => 'private'],
                'from' => ['id' => (int)$from_id, 'is_bot' => false, 'first_name' => 'User'],
                'text' => (string)$text,
            ],
        ];
    }
...
if(!function_exists('vpsbot_synth_callback_update')){
    function vpsbot_synth_callback_update($from_id, $chat_id, $data){
        $now = time();
        return [
            'update_id' => (int)($now % 1000000000),
            'callback_query' => [
                'id' => (string)($now . '_' . $from_id),
                'from' => ['id' => (int)$from_id, 'is_bot' => false, 'first_name' => 'User'],
                'message' => [
                    'message_id' => (int)($now % 1000000000),
                    'date' => $now,
...
// ------------------------------------------------------------
// VPSBot integration routing
// ------------------------------------------------------------
$stepNow = $userInfo['step'] ?? 'none';

// Entry from main menu
if(isset($data) && $data === 'cloudMenu'){
    setUser('vpsbot', 'step');
    setUser('', 'temp');
    // Ask VPSBot to show its home menu
    vpsbot_forward(['update' => vpsbot_synth_message_update($from_id, $from_id, '/start')]);
    // Do not let DeltaBot handle this update further
    exit();
}

// Entry from admin panel
if(isset($data) && $data === 'cloudAdmin' && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    setUser('vpsbot_admin', 'step');
    setUser('', 'temp');
    // Open VPSBot admin panel
    vpsbot_forward(['update' => vpsbot_synth_callback_update($from_id, $from_id, 'admin:home')]);
    exit();
}

// Forward all updates while user is inside VPSBot section
if(is_string($stepNow) && preg_match('/^vpsbot(\b|_)/', $stepNow)){
    // Allow leaving back to DeltaBot
    if(
        (isset($data) && ($data === 'mainMenu' || $data === 'managePanel')) ||
        (isset($text) && (preg_match('/^\/(start)/i', (string)$text) || $text === ($buttonValues['back_to_main'] ?? '')))
    ){
        resetUserFlow();
        // continue normal flow
    }else{
        // Forward raw telegram update to bridge
        $raw = json_decode(json_encode($update), true);
        vpsbot_forward(['update' => $raw]);
        exit();
    }
}
