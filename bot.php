...
if(preg_match('/^yesSend2All(\d+)/', $data,$match) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if(file_exists($worker) && function_exists('shell_exec')){
        // IMPORTANT:
        // In reseller bots, $currentBotInstanceId can be unset in some PHP configurations.
        // Always prefer the resolved bid from the current request or exported globals.
        $bidArg = 0;
        if(isset($_GET['bid'])){
            $bidArg = (int)$_GET['bid'];
