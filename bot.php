...
if(preg_match('/^uConfigsSearch(\d+)/',$data,$match) and canManageUserConfigPanel()){
    answerQuery();
    $uid = (int)$match[1];
    delMessage();
...
if(preg_match('/^uConfigs(\d+)_(\d+)/',$data,$match) && canManageUserConfigPanel()){
    answerQuery();
    $uid = (int)$match[1];
    $offset = (int)$match[2];
    $keys = getUserConfigsListKeys($uid, $offset);
    smartSendOrEdit($message_id, renderUserConfigsPanelTitle($uid) . "\n\n🔎 لیست کانفیگ‌ها:", $keys, "HTML");
}
