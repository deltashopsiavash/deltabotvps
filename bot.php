...
if(!$isChildBot && $data === 'vpsbotStart'){
    // Prevent some Telegram clients from hanging on "Loading..."
    if(isset($update->callback_query) && !empty($update->callback_query->id)){
        bot('answerCallbackQuery', ['callback_query_id'=>$update->callback_query->id]);
    }
    setUser('vpsbot','step');
    $resp = forwardUpdateToVpsbot($rawUpdate);
    // If VPS bridge is down or no response, show a friendly message instead of silent failure
    if($resp === false || $resp === null || $resp === ''){
        bot('sendMessage',[
            'chat_id'=>$chat_id,
            'text'=>'⚠️ بخش سرور ابری الان پاسخگو نیست. لطفاً چند لحظه بعد دوباره امتحان کنید. اگر مشکل ادامه داشت، سرویس Bridge روی سرور (پورت 9010) را بررسی کنید.'
        ]);
    }
    exit;
}
if(!$isChildBot && $data === 'vpsbotAdminEntry' && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if($resp === false || $resp === null || $resp === ''){
        bot('sendMessage',[
            'chat_id'=>$chat_id,
            'text'=>'⚠️ پنل مدیریت سرور ابری الان پاسخگو نیست. سرویس Bridge روی سرور (پورت 9010) را بررسی کنید.'
        ]);
    }
    exit;
}
