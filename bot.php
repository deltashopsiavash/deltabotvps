...
if(preg_match('/^uSearchUserConfigs_(\d+)$/', $userInfo['step'] ?? '', $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    $uid = (int)$match[1];
    sendMessage($mainValues['please_wait_message'], $removeKeyboard);
    sendMessage(renderUserInfoTitle($uid) . "
