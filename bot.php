...
function pgUserRenewSuggestionEnabled($userId){
    global $connection;
    $stmt=$connection->prepare("SELECT `pg_renew_suggestion` FROM `users` WHERE `userid`=? LIMIT 1");
    if(!$stmt) return true;
...
if(preg_match('/selectServer(?<continue>Continue)?(?<serverId>\d+)_(?<buyType>\w+)/',$data, $match) && ($botState['sellState']=="on" || ($from_id == $admin || $userInfo['isAdmin'] == true)) ) {
    ...
    if(empty($match['continue']) && pgUserRenewSuggestionEnabled($from_id)){
        $stmtPg=$connection->prepare("SELECT `type` FROM `server_config` WHERE `id`=? LIMIT 1");
        $stmtPg->bind_param('i',$sid);
        $stmtPg->execute();
        $pgType=strtolower((string)($stmtPg->get_result()->fetch_assoc()['type'] ?? ''));
        $stmtPg->close();
        if($pgType === 'pasarguard' && pgHasRenewableOrders($from_id,$sid)){
