...
لطفا دوباره توکن صحیح رو ارسال کن.");
        ...
        if(!$dbOk){
            ...
            sendMessage("⚠️ توجه: ایجاد دیتابیس اختصاصی برای ربات نمایندگی انجام نشد.\n\nاین یعنی ربات نمایندگی فعلاً با دیتابیس ربات مادر مشترک است.\nبرای درست شدن، یوزر دیتابیس باید دسترسی CREATE DATABASE داشته باشد.");
        }
...
// گزارش ساخت ربات نمایندگی برای مدیران
$dbn = !empty($row['db_name']) ? $row['db_name'] : '---';
$reportTxt = "📌 گزارش ساخت ربات نمایندگی\n\n"
    ."شناسه ربات (RID): {$rid}\n"
    ."یوزرنیم ربات: {$uname}\n"
    ."نام دیتابیس: {$dbn}\n"
    ."آیدی عددی سازنده: {$from_id}\n"
    ."آیدی عددی ادمین ربات: {$row['admin_userid']}\n"
    ."تاریخ انقضا: {$exp}";
...
function renderUserInfoTitle($uid){
    ...
    return "اطلاعات کاربر <a href='tg://user?id=$uid'>$fullName</a>";
}
...
// ===================== Cloud Server (Hetzner) - User Flow =====================

if($data == 'buyCloudServer'){
    ensureCloudTables();
    // reset flow state
    setUser(json_encode(['flow'=>'cloud_buy','v'=>1], JSON_UNESCAPED_UNICODE), 'temp');
    setUser('cloud_buy_method','step');

    $ik = [
        'inline_keyboard' => [
            [ ['text'=>'🌩 Hetzner Cloud','callback_data'=>'cloudBuy_method_hetzner'] ],
            [ ['text'=>'🧑‍💼 فروش دستی (تحویل توسط ادمین)','callback_data'=>'cloudBuy_method_manual'] ],
            [ ['text'=>$buttonValues['cancel'],'callback_data'=>'mainMenu'] ],
        ]
    ];
    smartSendOrEdit($message_id, "☁️ خرید سرور ابری\n\nروش تهیه سرویس را انتخاب کنید:", json_encode($ik));
    exit;
}

if(preg_match('/^cloudBuy_method_(hetzner|manual)$/',$data,$m)){
    ensureCloudTables();
    $method = $m[1];

    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];
    $temp['method'] = $method;
    setUser(json_encode($temp, JSON_UNESCAPED_UNICODE), 'temp');

    if($method === 'manual'){
        setUser('cloud_manual_note','step');
        smartSendOrEdit($message_id, "🧑‍💼 *فروش دستی*\n\nلطفاً نیازتان را در یک پیام بنویسید (کشور/لوکیشن، پلن، سیستم‌عامل، توضیحات).\nادمین بعد از بررسی، مشخصات سرور را برایتان ارسال می‌کند.", null, 'Markdown');
        exit;
    }

    // Hetzner flow: choose location
    $r = hcloudRequest('GET','/locations');
    if(!$r['ok']){
        smartSendOrEdit($message_id, "❌ اتصال به Hetzner انجام نشد.\n\nلطفاً مقدار HCLOUD Token را در baseInfo.php تنظیم کنید.\nخطا: ".json_encode($r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    ...
    $byCountry = [];
    foreach($locs as $loc){
        $cc = strtoupper($loc['country'] ?? '');
        $name = $loc['name'] ?? '';
        $city = $loc['city'] ?? $name;
        if(empty($cc) || empty($name)) continue;
        if(!isset($byCountry[$cc])) $byCountry[$cc] = [];
        $byCountry[$cc][] = ['name'=>$name,'city'=>$city];
    }
    ksort($byCountry);

    $rows = [];
    foreach($byCountry as $cc=>$items){
        // show up to 6 per country to avoid giant menus
        $items = array_slice($items,0,6);
        foreach($items as $it){
            $rows[] = [ ['text'=>"{$cc} - {$it['city']} ({$it['name']})", 'callback_data'=>'cloudBuy_loc_'.$it['name']] ];
            if(count($rows) >= 24) break 2;
        }
    }
    $rows[] = [ ['text'=>$buttonValues['cancel'], 'callback_data'=>'mainMenu'] ];

    setUser('cloud_buy_location','step');
    smartSendOrEdit($message_id, "🌍 لوکیشن را انتخاب کنید:", json_encode(['inline_keyboard'=>$rows]));
    exit;
}

if(preg_match('/^cloudBuy_loc_([a-z0-9]+)$/',$data,$m)){
    ensureCloudTables();
    $loc = $m[1];
    ...
    // Choose OS image
    $r = hcloudRequest('GET','/images?type=system');
    if(!$r['ok']){
        smartSendOrEdit($message_id, "❌ دریافت لیست سیستم‌عامل‌ها از Hetzner انجام نشد.\nخطا: ".json_encode($r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    $imgs = $r['data']['images'] ?? [];

    // Keep only common distros (similar to VPSBot menu)
    $wanted = ['ubuntu','debian','centos','fedora','rocky','alma'];
    $picked = [];
    foreach($imgs as $img){
        $n = strtolower($img['name'] ?? '');
        $desc = $img['description'] ?? '';
        foreach($wanted as $w){
            if(strpos($n,$w)!==false){
                $picked[] = ['id'=>$img['id'], 'name'=>$img['name'], 'desc'=>$desc];
                break;
            }
        }
    }
    // sort by name
    usort($picked, function($a,$b){ return strcmp($a['name'],$b['name']); });
    $picked = array_slice($picked,0,20);

    $rows = [];
    foreach($picked as $img){
        $rows[] = [ ['text'=>$img['name'], 'callback_data'=>'cloudBuy_img_'.$img['id']] ];
    }
    $rows[] = [ ['text'=>$buttonValues['cancel'], 'callback_data'=>'mainMenu'] ];

    setUser('cloud_buy_os','step');
    smartSendOrEdit($message_id, "💽 سیستم‌عامل را انتخاب کنید:", json_encode(['inline_keyboard'=>$rows]));
    exit;
}

if(preg_match('/^cloudBuy_img_(\d+)$/',$data,$m)){
    ensureCloudTables();
    $imgId = (int)$m[1];
    ...
    // Choose server type (plan)
    $r = hcloudRequest('GET','/server_types');
    if(!$r['ok']){
        smartSendOrEdit($message_id, "❌ دریافت پلن‌ها از Hetzner انجام نشد.\nخطا: ".json_encode($r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    $types = $r['data']['server_types'] ?? [];
    $loc = $temp['location'] ?? '';

    // Build menu using prices for selected location (EUR -> IRT)
    $eurToIrt = (float)(getSettingValue('CLOUD_EUR_TO_IRT','0') ?: 0);
    if($eurToIrt <= 0) $eurToIrt = 60000; // fallback
    $margin = (float)(getSettingValue('CLOUD_MARGIN_PERCENT','0') ?: 0);

    $items = [];
    foreach($types as $t){
        if(($t['deprecated'] ?? false) === true) continue;
        $name = $t['name'] ?? '';
        ...
        if(!$price) continue;
        $monthEur = (float)($price['price_monthly']['gross'] ?? 0);
        $hourEur  = (float)($price['price_hourly']['gross'] ?? 0);
        $monthIrt = (int)round($monthEur * $eurToIrt * (1 + $margin/100));
        $hourIrt  = (int)round($hourEur  * $eurToIrt * (1 + $margin/100));
        $title = $name." | ".($t['cores']??'?')." CPU | ".($t['memory']??'?')."GB | ".($t['disk']??'?')."GB";
        $items[] = ['name'=>$name,'title'=>$title,'m'=>$monthIrt,'h'=>$hourIrt];
    }

    usort($items,function($a,$b){ return $a['m'] <=> $b['m']; });
    $items = array_slice($items,0,25);

    $rows = [];
    foreach($items as $it){
        $rows[] = [ ['text'=>$it['title']."\n💳 ماهانه: ".cloudFmtMoney($it['m']), 'callback_data'=>'cloudBuy_type_'.$it['name']] ];
    }
    $rows[] = [ ['text'=>$buttonValues['cancel'], 'callback_data'=>'mainMenu'] ];

    // store the pricing map in temp for later
    $temp['pricing'] = [];
    foreach($items as $it){
        $temp['pricing'][$it['name']] = ['m'=>$it['m'],'h'=>$it['h']];
    }
    setUser(json_encode($temp, JSON_UNESCAPED_UNICODE), 'temp');

    setUser('cloud_buy_type','step');
    smartSendOrEdit($message_id, "🧩 پلن/مدل سرور را انتخاب کنید:", json_encode(['inline_keyboard'=>$rows]));
    exit;
}

if(preg_match('/^cloudBuy_type_([a-z0-9-]+)$/',$data,$m)){
    ensureCloudTables();
    $type = $m[1];
    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];

    $temp['server_type'] = $type;
    setUser(json_encode($temp, JSON_UNESCAPED_UNICODE), 'temp');

    // Ask hostname
    setUser('cloud_buy_hostname','step');
    smartSendOrEdit($message_id, "⌨️ نام سرور (hostname) را ارسال کنید:\n\nفقط حروف انگلیسی/عدد/خط‌تیره.\nمثال: my-server-1");
    exit;
}

if(($userInfo['step'] ?? '') === 'cloud_buy_hostname' && !empty($text) && $text !== $buttonValues['cancel']){
    ensureCloudTables();
    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];

    $hn = cloudSanitizeHostname($text);
    $temp['hostname'] = $hn;
    setUser(json_encode($temp, JSON_UNESCAPED_UNICODE), 'temp');

    // Billing type menu
    $allowHourly = (getSettingValue('CLOUD_HOURLY_ON','0') === '1');
    $ik = [
        'inline_keyboard' => [
            [ ['text'=>'📆 ماهانه','callback_data'=>'cloudBuy_bill_monthly'] ],
            ($allowHourly ? [ ['text'=>'⏱ ساعتی','callback_data'=>'cloudBuy_bill_hourly'] ] : []),
            [ ['text'=>$buttonValues['cancel'],'callback_data'=>'mainMenu'] ]
        ]
    ];
    // Remove empty rows
    $ik['inline_keyboard'] = array_values(array_filter($ik['inline_keyboard'], function($r){ return is_array($r) && count($r)>0; }));

    setUser('cloud_buy_billing','step');
    smartSendOrEdit($message_id, "💳 نوع صورتحساب را انتخاب کنید:", json_encode($ik));
    exit;
}

if(preg_match('/^cloudBuy_bill_(monthly|hourly)$/',$data,$m)){
    ensureCloudTables();
    $billing = $m[1];
    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];
    $temp['billing'] = $billing;

    $type = $temp['server_type'] ?? '';
    $pricing = $temp['pricing'][$type] ?? null;
    if(!$pricing){
        smartSendOrEdit($message_id, "❌ خطا: قیمت پلن پیدا نشد. دوباره از ابتدا اقدام کنید.", getMainKeys());
        setUser('none','step');
        exit;
    }

    $amount = ($billing==='hourly') ? (int)($pricing['h'] ?? 0) : (int)($pricing['m'] ?? 0);
    if($amount <= 0) $amount = (int)($pricing['m'] ?? 0);
    $temp['amount_irt'] = $amount;
    setUser(json_encode($temp, JSON_UNESCAPED_UNICODE), 'temp');

    // Payment menu
    $ik = [
        'inline_keyboard' => [
            [ ['text'=>$buttonValues['pay_with_wallet'],'callback_data'=>'cloudPay_wallet'] ],
            [ ['text'=>$buttonValues['cart_to_cart'],'callback_data'=>'cloudPay_card'] ],
            [ ['text'=>$buttonValues['cancel'],'callback_data'=>'mainMenu'] ],
        ]
    ];

    setUser('cloud_buy_payment','step');
    smartSendOrEdit($message_id, "✅ خلاصه سفارش\n\n📍 لوکیشن: ".($temp['location'] ?? '-')."\n🧩 پلن: ".($temp['server_type'] ?? '-')."\n💽 OS: ".($temp['image_id'] ?? '-')."\n🖥 hostname: ".($temp['hostname'] ?? '-')."\n💳 صورتحساب: ".($billing==='hourly'?'ساعتی':'ماهانه')."\n\nمبلغ: ".cloudFmtMoney($amount)."\n\nروش پرداخت را انتخاب کنید:", json_encode($ik));
    exit;
}

// Wallet payment => create server immediately
if($data === 'cloudPay_wallet'){
    ensureCloudTables();
    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];

    $amount = (int)($temp['amount_irt'] ?? 0);
    if($amount <= 0){
        smartSendOrEdit($message_id, "❌ مبلغ سفارش نامعتبر است.", getMainKeys());
        setUser('none','step');
        exit;
    }
    ...
    $wallet = (int)($userInfo['wallet'] ?? 0);
    if($wallet < $amount){
        smartSendOrEdit($message_id, "❌ موجودی کافی نیست.\n\nموجودی شما: ".cloudFmtMoney($wallet)."\nمبلغ سفارش: ".cloudFmtMoney($amount));
        exit;
    }

    // create order row
    $now = time();
    $stmt = $connection->prepare("INSERT INTO cloud_orders (user_id,method,status,billing,amount_irt,hostname,location,server_type,image_id,created_at,expires_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $method = 'hetzner';
    $status = 'creating';
    $billing = ($temp['billing'] ?? 'monthly') === 'hourly' ? 'hourly' : 'monthly';
    $hn = $temp['hostname'] ?? null;
    $loc = $temp['location'] ?? null;
    $stype = $temp['server_type'] ?? null;
    $img = (int)($temp['image_id'] ?? 0);
    $exp = ($billing==='monthly') ? ($now + 30*86400) : 0;
    $stmt->bind_param('isssisssiii', $from_id, $method, $status, $billing, $amount, $hn, $loc, $stype, $img, $now, $exp);
    $stmt->execute();
    $orderId = (int)$stmt->insert_id;
    $stmt->close();

    // deduct wallet
    setUser((string)($wallet - $amount),'wallet');

    smartSendOrEdit($message_id, "⏳ در حال ساخت سرور...", null);
    ...
    $r = hcloudRequest('POST','/servers', $payload);
    if(!$r['ok']){
        $connection->query("UPDATE cloud_orders SET status='failed' WHERE id={$orderId} LIMIT 1");
        smartSendOrEdit($message_id, "❌ ساخت سرور ناموفق بود.\n\nجزئیات: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }

    $srv = $r['data']['server'] ?? [];
    $rootPass = $r['data']['root_password'] ?? null;
    $sid = (int)($srv['id'] ?? 0);

    // fetch server to get IP
    $ip = null;
    $tries = 0;
    while($tries < 12){
        $tries++;
        usleep(500000); // 0.5s
        $gr = hcloudRequest('GET','/servers/'.$sid);
        if($gr['ok']){
            $s = $gr['data']['server'] ?? [];
            $ip = $s['public_net']['ipv4']['ip'] ?? null;
            $st = $s['status'] ?? '';
            if(!empty($ip) && $st === 'running') break;
        }
    }

    $ipEsc = $connection->real_escape_string((string)$ip);
    $rpEsc = $connection->real_escape_string((string)$rootPass);
    $connection->query("UPDATE cloud_orders SET status='active', hcloud_server_id={$sid}, ip='{$ipEsc}', root_password='{$rpEsc}' WHERE id={$orderId} LIMIT 1");

    setUser('none','step');
    smartSendOrEdit($message_id, "✅ سرور شما ساخته شد\n\nIP: `{$ip}`\nUser: `root`\nPassword: `{$rootPass}`\n\n🆔 سفارش: {$orderId}", getMainKeys(), 'Markdown');
    exit;
}

// Card-to-card payment => create invoice, wait for receipt
if($data === 'cloudPay_card'){
    ensureCloudTables();
    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    if(!is_array($temp)) $temp = [];
    ...
    // create order row as pending_payment
    $now = time();
    $stmt = $connection->prepare("INSERT INTO cloud_orders (user_id,method,status,billing,amount_irt,hostname,location,server_type,image_id,created_at,expires_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $method = 'hetzner';
    ...
    $stmt2 = $connection->prepare("INSERT INTO cloud_invoices (user_id,order_id,amount_irt,pay_method,state,created_at) VALUES (?,?,?,?,?,?)");
    $pm = 'card';
    $st = 'pending';
    $stmt2->bind_param('iiissi', $from_id, $orderId, $amount, $pm, $st, $now);
    $stmt2->execute();
    $invId = (int)$stmt2->insert_id;
    $stmt2->close();

    // save invoice id in temp and wait for receipt
    setUser(json_encode(['flow'=>'cloud_receipt','invoice_id'=>$invId], JSON_UNESCAPED_UNICODE), 'temp');
    setUser('cloud_wait_receipt','step');

    smartSendOrEdit($message_id, "💳 کارت به کارت\n\nمبلغ: ".cloudFmtMoney($amount)."\n\n✅ لطفاً *عکس رسید پرداخت* را همینجا ارسال کنید.\nبعد از تایید ادمین، سرور ساخته می‌شود.\n\n🧾 کد فاکتور: {$invId}", null, 'Markdown');
    exit;
}

// Receipt upload
if(($userInfo['step'] ?? '') === 'cloud_wait_receipt' && isset($update->message) && (isset($update->message->photo) || isset($update->message->document)) && $text !== $buttonValues['cancel']){
    ensureCloudTables();

    $temp = json_decode($userInfo['temp'] ?? '{}', true);
    $invId = (int)($temp['invoice_id'] ?? 0);
    if($invId <= 0){
        smartSendOrEdit($message_id, "❌ فاکتور پیدا نشد. دوباره تلاش کنید.");
        exit;
    }
    ...
    if(empty($fileId)){
        smartSendOrEdit($message_id, "❌ فایل رسید قابل تشخیص نیست. لطفاً عکس رسید را ارسال کنید.");
        exit;
    }

    $fileEsc = $connection->real_escape_string($fileId);
    $connection->query("UPDATE cloud_invoices SET receipt_file_id='{$fileEsc}' WHERE id={$invId} LIMIT 1");

    // notify admin
    $inv = $connection->query("SELECT * FROM cloud_invoices WHERE id={$invId} LIMIT 1")->fetch_assoc();
    $orderId = (int)($inv['order_id'] ?? 0);
    $amount = (int)($inv['amount_irt'] ?? 0);

    $ik = [
        'inline_keyboard' => [
            [ ['text'=>'✅ تایید','callback_data'=>'cloudAdmin_approve_'.$invId], ['text'=>'❌ رد','callback_data'=>'cloudAdmin_decline_'.$invId] ],
        ]
    ];

    sendPhoto($fileId, "🧾 رسید پرداخت سرور ابری\n\nفاکتور: {$invId}\nسفارش: {$orderId}\nکاربر: {$from_id}\nمبلغ: ".cloudFmtMoney($amount), json_encode($ik), $admin);

    setUser('none','step');
    smartSendOrEdit($message_id, "✅ رسید شما ثبت شد. بعد از بررسی ادمین اطلاع‌رسانی می‌شود.", getMainKeys());
    exit;
}

// ===================== Cloud Server (Hetzner) - Admin Panel =====================

if($data === 'cloudServerAdmin' && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();

    $pending = $connection->query("SELECT COUNT(*) c FROM cloud_invoices WHERE state='pending'")->fetch_assoc();
    $cPend = (int)($pending['c'] ?? 0);

    $ik = [
        'inline_keyboard' => [
            [ ['text'=>"🧾 فاکتورهای در انتظار ({$cPend})", 'callback_data'=>'cloudAdmin_pendingInvoices'] ],
            [ ['text'=>'🖥 لیست سرورها','callback_data'=>'cloudAdmin_orders_0'] ],
            [ ['text'=>$buttonValues['back_to_main'],'callback_data'=>'managePanel'] ],
        ]
    ];

    smartSendOrEdit($message_id, "☁️ پنل مدیریت سرور ابری", json_encode($ik));
    exit;
}

if($data === 'cloudAdmin_pendingInvoices' && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $res = $connection->query("SELECT * FROM cloud_invoices WHERE state='pending' ORDER BY id DESC LIMIT 20");
    $rows = [];
    if($res){
        while($i = $res->fetch_assoc()){
            $invId = (int)$i['id'];
            $rows[] = [ ['text'=>"🧾 #{$invId} | ".cloudFmtMoney((int)$i['amount_irt'])." | user:".$i['user_id'], 'callback_data'=>'cloudAdmin_invoice_'.$invId] ];
        }
    }
    $rows[] = [ ['text'=>$buttonValues['back_button'],'callback_data'=>'cloudServerAdmin'] ];
    smartSendOrEdit($message_id, "فاکتورهای در انتظار:", json_encode(['inline_keyboard'=>$rows]));
    exit;
}

if(preg_match('/^cloudAdmin_invoice_(\d+)$/',$data,$m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $invId = (int)$m[1];
    $inv = $connection->query("SELECT * FROM cloud_invoices WHERE id={$invId} LIMIT 1")->fetch_assoc();
    if(!$inv){
        smartSendOrEdit($message_id, "❌ فاکتور پیدا نشد.");
        exit;
    }
    ...
    $txt = "🧾 فاکتور #{$invId}\n".
        "کاربر: {$inv['user_id']}\n".
        "سفارش: {$inv['order_id']}\n".
        "مبلغ: ".cloudFmtMoney((int)$inv['amount_irt'])."\n".
        "وضعیت: {$inv['state']}";

    $ik = [
        'inline_keyboard' => [
            [ ['text'=>'✅ تایید','callback_data'=>'cloudAdmin_approve_'.$invId], ['text'=>'❌ رد','callback_data'=>'cloudAdmin_decline_'.$invId] ],
            [ ['text'=>$buttonValues['back_button'],'callback_data'=>'cloudAdmin_pendingInvoices'] ],
        ]
    ];

    smartSendOrEdit($message_id, $txt, json_encode($ik));
    exit;
}

if(preg_match('/^cloudAdmin_(approve|decline)_(\d+)$/',$data,$m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $act = $m[1];
    $invId = (int)$m[2];
    ...
    if(($inv['state'] ?? '') !== 'pending'){
        smartSendOrEdit($message_id, "ℹ️ این فاکتور قبلاً بررسی شده است.");
        exit;
    }

    if($act === 'decline'){
        $connection->query("UPDATE cloud_invoices SET state='declined', approved_at=".time().", admin_id={$from_id} WHERE id={$invId} LIMIT 1");
        $oid = (int)($inv['order_id'] ?? 0);
        if($oid>0) $connection->query("UPDATE cloud_orders SET status='declined' WHERE id={$oid} LIMIT 1");
        sendMessage("❌ پرداخت شما برای سرور ابری تایید نشد.\nفاکتور: {$invId}", null, 'HTML', (int)$inv['user_id']);
        smartSendOrEdit($message_id, "❌ رد شد.", null);
        exit;
    }

    // approve: mark paid then create server
    $connection->query("UPDATE cloud_invoices SET state='paid', approved_at=".time().", admin_id={$from_id} WHERE id={$invId} LIMIT 1");

    $oid = (int)($inv['order_id'] ?? 0);
    $order = $connection->query("SELECT * FROM cloud_orders WHERE id={$oid} LIMIT 1")->fetch_assoc();
    if(!$order){
        smartSendOrEdit($message_id, "✅ تایید شد، اما سفارش پیدا نشد.");
        exit;
    }

    $hn = $order['hostname'];
    $loc = $order['location'];
    $stype = $order['server_type'];
    $img = (int)$order['image_id'];

    smartSendOrEdit($message_id, "⏳ تایید شد. در حال ساخت سرور...", null);
    ...
    $r = hcloudRequest('POST','/servers', $payload);
    if(!$r['ok']){
        $connection->query("UPDATE cloud_orders SET status='failed' WHERE id={$oid} LIMIT 1");
        sendMessage("❌ ساخت سرور بعد از تایید پرداخت ناموفق بود.\nفاکتور: {$invId}", null, 'HTML', (int)$order['user_id']);
        smartSendOrEdit($message_id, "❌ ساخت سرور ناموفق بود.\n".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    ...
    $ipEsc = $connection->real_escape_string((string)$ip);
    $rpEsc = $connection->real_escape_string((string)$rootPass);
    $connection->query("UPDATE cloud_orders SET status='active', hcloud_server_id={$sid}, ip='{$ipEsc}', root_password='{$rpEsc}' WHERE id={$oid} LIMIT 1");

    sendMessage("✅ سرور شما ساخته شد\n\nIP: <code>{$ip}</code>\nUser: <code>root</code>\nPassword: <code>{$rootPass}</code>\n\n🆔 سفارش: {$oid}", null, 'HTML', (int)$order['user_id']);

    smartSendOrEdit($message_id, "✅ تایید شد و سرور ساخته شد.\nسفارش: {$oid}", null);
    exit;
}

// Orders list (admin)
if(preg_match('/^cloudAdmin_orders_(\d+)$/',$data,$m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $offset = (int)$m[1];
    if($offset<0) $offset=0;

    $res = $connection->query("SELECT * FROM cloud_orders ORDER BY id DESC LIMIT 10 OFFSET {$offset}");
    $rows = [];
    if($res){
        while($o = $res->fetch_assoc()){
            ...
            $st=$o['status'];
            $ip=$o['ip'] ?: '-';
            $rows[] = [ ['text'=>"#{$oid} | {$st} | {$ip} | user:".$o['user_id'], 'callback_data'=>'cloudAdmin_order_'.$oid] ];
        }
    }
    $nav=[];
    if($offset>0) $nav[] = ['text'=>'⬅️ قبلی','callback_data'=>'cloudAdmin_orders_'.max(0,$offset-10)];
    $nav[] = ['text'=>'➡️ بعدی','callback_data'=>'cloudAdmin_orders_'.($offset+10)];
    $rows[] = $nav;
    $rows[] = [ ['text'=>$buttonValues['back_button'],'callback_data'=>'cloudServerAdmin'] ];
    smartSendOrEdit($message_id, "🖥 لیست سرورها:", json_encode(['inline_keyboard'=>$rows]));
    exit;
}

if(preg_match('/^cloudAdmin_order_(\d+)$/',$data,$m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $oid=(int)$m[1];
    $o = $connection->query("SELECT * FROM cloud_orders WHERE id={$oid} LIMIT 1")->fetch_assoc();
    if(!$o){
        smartSendOrEdit($message_id, "❌ سفارش پیدا نشد.");
        exit;
    }
    ...
    $txt = "🖥 سفارش #{$oid}\n".
        ...
        "status: {$o['status']}\n".
        "billing: {$o['billing']}\n".
        "amount: ".cloudFmtMoney((int)$o['amount_irt'])."\n".
        "location: {$o['location']}\n".
        "type: {$o['server_type']}\n".
        "ip: ".($o['ip'] ?: '-')."\n".
        "hcloud_id: ".($o['hcloud_server_id'] ?: '-');

    $sid = (int)($o['hcloud_server_id'] ?? 0);

    $ikRows = [];
    if($sid>0){
        $ikRows[] = [ ['text'=>'⏻ خاموش','callback_data'=>'cloudAdmin_poweroff_'.$oid], ['text'=>'⏻ روشن','callback_data'=>'cloudAdmin_poweron_'.$oid] ];
        $ikRows[] = [ ['text'=>'♻️ ریبلد','callback_data'=>'cloudAdmin_rebuild_'.$oid], ['text'=>'🔑 ریست پسورد','callback_data'=>'cloudAdmin_resetpw_'.$oid] ];
        $ikRows[] = [ ['text'=>'🗑 حذف سرور','callback_data'=>'cloudAdmin_delete_'.$oid] ];
    }
    $ikRows[] = [ ['text'=>$buttonValues['back_button'],'callback_data'=>'cloudAdmin_orders_0'] ];

    smartSendOrEdit($message_id, $txt, json_encode(['inline_keyboard'=>$ikRows]));
    exit;
}

// Server actions (admin)
if(preg_match('/^cloudAdmin_(poweroff|poweron|rebuild|resetpw|delete)_(\d+)$/',$data,$m) && ($from_id == $admin || ($userInfo['isAdmin'] ?? false) == true)){
    ensureCloudTables();
    $act = $m[1];
    $oid = (int)$m[2];
    $o = $connection->query("SELECT * FROM cloud_orders WHERE id={$oid} LIMIT 1")->fetch_assoc();
    ...
    $sid = (int)($o['hcloud_server_id'] ?? 0);
    if($sid<=0){
        smartSendOrEdit($message_id, "❌ شناسه سرور Hetzner ثبت نشده.");
        exit;
    }

    if($act==='poweroff'){
        $r = hcloudRequest('POST',"/servers/{$sid}/actions/poweroff");
        smartSendOrEdit($message_id, $r['ok']?"✅ خاموش شد":"❌ خطا: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    if($act==='poweron'){
        $r = hcloudRequest('POST',"/servers/{$sid}/actions/poweron");
        smartSendOrEdit($message_id, $r['ok']?"✅ روشن شد":"❌ خطا: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    if($act==='rebuild'){
        $img = (int)($o['image_id'] ?? 0);
        $r = hcloudRequest('POST',"/servers/{$sid}/actions/rebuild", ['image'=>$img]);
        smartSendOrEdit($message_id, $r['ok']?"✅ ریبلد شروع شد":"❌ خطا: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        exit;
    }
    if($act==='resetpw'){
        $r = hcloudRequest('POST',"/servers/{$sid}/actions/reset_password");
        if($r['ok']){
            $pw = $r['data']['root_password'] ?? null;
            $pwEsc = $connection->real_escape_string((string)$pw);
            $connection->query("UPDATE cloud_orders SET root_password='{$pwEsc}' WHERE id={$oid} LIMIT 1");
            smartSendOrEdit($message_id, "✅ پسورد جدید: `{$pw}`", null, 'Markdown');
        }else{
            smartSendOrEdit($message_id, "❌ خطا: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        }
        exit;
    }
    if($act==='delete'){
        $r = hcloudRequest('DELETE',"/servers/{$sid}");
        if($r['ok']){
            $connection->query("UPDATE cloud_orders SET status='deleted' WHERE id={$oid} LIMIT 1");
            smartSendOrEdit($message_id, "🗑 حذف شد.");
        }else{
            smartSendOrEdit($message_id, "❌ خطا: ".json_encode($r['detail'] ?? $r['error'], JSON_UNESCAPED_UNICODE));
        }
        exit;
...
if(($userInfo['step'] ?? '') === 'cloud_manual_note' && !empty($text) && $text !== $buttonValues['cancel']){
    ensureCloudTables();
    // create a manual order ticket
    $now = time();
    $stmt = $connection->prepare("INSERT INTO cloud_orders (user_id,method,status,billing,amount_irt,hostname,created_at) VALUES (?,?,?,?,?,?,?)");
    $method = 'manual';
    $status = 'manual_request';
    $billing = 'monthly';
    $amount = 0;
    $hn = null;
    $stmt->bind_param('isssisi', $from_id, $method, $status, $billing, $amount, $hn, $now);
    $stmt->execute();
    $oid = (int)$stmt->insert_id;
    $stmt->close();

    sendMessage("🧑‍💼 درخواست فروش دستی سرور ابری\n\nکاربر: {$from_id}\nسفارش: {$oid}\nمتن درخواست:\n{$text}", null, 'HTML', $admin);

    setUser('none','step');
    smartSendOrEdit($message_id, "✅ درخواست شما ثبت شد. ادمین بعد از بررسی با شما هماهنگ می‌کند.\n\n🆔 کد درخواست: {$oid}", getMainKeys());
    exit;
}
