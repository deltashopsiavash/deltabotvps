...
function deltaSetXuiStateExact($order,$wantActive){
    ...
    $row=$found['row']; $settings=$found['settings']; $client_key=$found['client_key']; $client=$found['client']; $actualUuid=$found['uuid'];
    $serverType=$server_info['type'];
    ...
    $settings['clients'][$client_key]['enable'] = (bool)$wantActive;
    ...
    $panel_url=rtrim($server_info['panel_url'],'/');
    $attempts=[];
    if(($serverType == 'sanaei' || $serverType == 'alireza') && $inboundId>0){
        $newSetting=json_encode(['clients'=>[$editedClient]],488);
        $dataArr=['id'=>$inboundId,'settings'=>$newSetting];
        // پنل‌های مختلف 3x-ui/sanaei/alireza مسیرهای متفاوتی دارند؛ هر دو مسیر تست می‌شود.
        $attempts[]="$panel_url/panel/inbound/updateClient/".rawurlencode($actualUuid);
        $attempts[]="$panel_url/xui/inbound/updateClient/".rawurlencode($actualUuid);
    }else{
        $newInboundEnable = ($inboundId==0) ? (bool)$wantActive : true;
        $dataArr=['up'=>$row->up,'down'=>$row->down,'total'=>$row->total,'remark'=>$row->remark,'enable'=>$newInboundEnable,
            'expiryTime'=>$row->expiryTime,'listen'=>'','port'=>$row->port,'protocol'=>$row->protocol,'settings'=>$settingsJson,
    ...
    foreach(array_unique($attempts) as $url){
        ...
        if($res && isset($res->success)){
            curl_close($curl);
            if($res->success) return (object)['success'=>true];
            return (object)['success'=>false,'msg'=>($res->msg ?? 'عملیات پنل ناموفق بود')];
        }
    }
    curl_close($curl);
...
function deltaInactiveRenewAndReturn($oid){
    ...
    if($serverType==='marzban'){
        ...
        $upd=deltaInactiveRenewXuiSafe($order,$found);
        if(!$upd || empty($upd->newUuid)) return (object)['success'=>false,'msg'=>($upd->msg??'تغییر uid ناموفق بود')];
