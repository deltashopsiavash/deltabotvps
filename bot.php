...
if(preg_match('/^adminOfferStock_(\d+)$/',$userInfo['step'] ?? '',$m) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $required=['plan_id','title','discount_percent','starts_at','ends_at']; foreach($required as $key){ if(!isset($draft[$key])){ sendMessage('اطلاعات ساخت ناقص شد؛ دوباره تلاش کنید.'); setUser(); exit(); } }
    $stmt=$connection->prepare("INSERT INTO `special_offers` (`plan_id`,`title`,`discount_percent`,`starts_at`,`ends_at`,`stock_total`,`stock_sold`,`active`,`created_by`,`created_at`) VALUES (?,?,?,?,?,?,0,1,?,?)");
    $planId=(int)$draft['plan_id']; $title=(string)$draft['title']; $percent=(int)$draft['discount_percent']; $starts=(int)$draft['starts_at']; $ends=(int)$draft['ends_at']; $created=time();
    $stmt->bind_param('isiiiiii',$planId,$title,$percent,$starts,$ends,$stock,$from_id,$created); $stmt->execute(); $offerId=$stmt->insert_id; $stmt->close();
...
if($data==='specialOfferToday'){
    ...
    // Upcoming campaigns must stay visible in the main menu.  Before the
    // configured start time only show a live-on-refresh countdown; purchasing
    // is deliberately disabled until getActiveSpecialOffer() accepts it.
    if((int)$offer['starts_at']>time()){
        $offerTitle=htmlspecialchars((string)$offer['title'],ENT_QUOTES|ENT_SUBSTITUTE,'UTF-8');
        $txt="⏳ <b>پیشنهاد ویژه در راه است</b>\n\n🔥 <b>{$offerTitle}</b>\n📦 ".(int)$offer['plan_volume']."GB / ".(int)$offer['plan_days']." روزه\n🏷 تخفیف: <b>".(int)$offer['discount_percent']."٪</b>\n🕐 شروع: <b>".specialOfferFormatDate($offer['starts_at'])."</b>\n⏱ زمان مانده تا شروع: <b>".specialOfferCountdownText($offer['starts_at'])."</b>";
        $keys=['inline_keyboard'=>[[['text'=>'🔄 بروزرسانی شمارش معکوس','callback_data'=>'specialOfferToday']],[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]];
    ...
    $keys=['inline_keyboard'=>[[['text'=>'🛍 خرید این پیشنهاد','callback_data'=>'specialOfferBuy'.(int)$offer['id']]],[['text'=>$buttonValues['back_to_main'],'callback_data'=>'mainMenu']]]];
    smartSendOrEdit($message_id,$txt,json_encode($keys,JSON_UNESCAPED_UNICODE),'HTML'); exit();
}
...
برای این که Device ID هر گوشی توی برنامه متفاوت است و با این کد میتوان اشتراک رو قفل کرد جهت تک کاربره شدن اشتراک و هیچ سود و یا خطرات دیگری ندارد این کد فقط صرفا جهت قفل اشتراک روی همان دستگاه است✅", $cancelKey, 'HTML');
    ...
    if($specialOffer && !empty($hash_id) && !preg_match('/^discountSelectPlan/', (string)($userInfo['step'] ?? ''))){
        ...
        if(!specialOfferAttachPay($hash_id,(int)$specialOffer['id'],$offerQty)){
            alert('موجودی این پیشنهاد همین حالا تمام شد؛ لطفاً پیشنهادهای فعال را دوباره بررسی کنید.',true);
            exit();
        }
    }
