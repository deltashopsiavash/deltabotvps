...
if(preg_match('/^adminResDBDropYes_([A-Za-z0-9_]+)/',$data,$m) && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    // Use a separate MySQL connection for DROP DATABASE to avoid blocking/killing the main connection
    // (and to allow setting a read timeout for the DROP query).
    $dbEsc = str_replace('`','',$dbn);

    $dropOk = false;
    $dropErr = '';

    $dropConn = @new mysqli('localhost', $GLOBALS['dbUserName'] ?? ($dbUserName ?? ''), $GLOBALS['dbPassword'] ?? ($dbPassword ?? ''));
    if($dropConn && !$dropConn->connect_error){
        // Set a query read timeout (seconds) to avoid hanging forever in some environments
        @mysqli_options($dropConn, MYSQLI_OPT_READ_TIMEOUT, 60);
        @$dropConn->set_charset("utf8mb4");
        // Select a safe database (not the one being dropped)
        @$dropConn->select_db('information_schema');

        $dbEsc2 = str_replace('`','',$dbEsc);
        $dropOk = @$dropConn->query("DROP DATABASE `{$dbEsc2}`");
        if(!$dropOk){
            $dropErr = $dropConn->error;
        }
        @$dropConn->close();
    }else{
        $dropErr = $dropConn ? $dropConn->connect_error : 'Cannot connect to MySQL';
    }
    ...
    if($dropOk){
        ...
        if(!$dropErr){
            $dropErr = "unknown error";
        }
        smartSendOrEdit($message_id, "❌ حذف انجام نشد.\n\nخطا: {$dropErr}", ['inline_keyboard'=>[[['text'=>"🔙 برگشت",'callback_data'=>"adminResDBInfo_".$dbn]]]], "Markdown");
    }
    exit;
}
