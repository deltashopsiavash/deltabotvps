...
async def admin_general(cq: CallbackQuery, db: DB, state: FSMContext):
    """
    Admin: General settings menu.

    Why this exists:
    - In bridge mode, some messages cannot be edited (message too old / not editable / etc.)
      so we must fallback to sending a new message instead of silently crashing.
    - Also clear any pending FSM state so callbacks always work.
    """
    if not is_admin(cq.from_user.id):
        return await cq.answer("دسترسی ندارید.", show_alert=True)
    ...
    text = f"{glass_header('مدیریت عمومی')}\n{GLASS_DOT} وضعیت‌ها:"
    markup = kb([
