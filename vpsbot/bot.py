...
async def main_menu(db: DB, user_id: int) -> Tuple[str, InlineKeyboardMarkup]:
    ...
    default_order = ["buy", "orders", "profile", "ip_status", "renew"]
    try:
    ...
    for k in menu_order:
        if k == "renew" and not renew_enabled:
            continue
        if k in keymap:
            visible_keys.append(k)

    for i in range(0, len(visible_keys), cols):
        row_keys = visible_keys[i:i+cols]
        row: List[Tuple[str, str]] = [keymap[k] for k in row_keys]
        if row:
            rows.append(row)
    ...
    # Custom buttons (always appended)    # Custom buttons (always appended)
    try:
...
async def admin_layout(cq: CallbackQuery, db: DB, state: FSMContext):
    ...
    lines = [
        ...
        "4) 🛰وضعیت ای پی🛰",
        f"5) {'renew' if renew_enabled else 'renew (خاموش)'}",
        "",
        "مثال: 3,1,2,4,5",
...
async def admin_layout_set(msg: Message, db: DB, state: FSMContext):
    ...
    num_to_key = {1:"buy", 2:"orders", 3:"profile", 4:"ip_status", 5:"renew"}
