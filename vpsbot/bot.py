...
def hcloud_delete_server(server_id: int) -> bool:
    """Delete a Hetzner Cloud server by id. Returns True if deleted (or not found)."""
    if not HCLOUD_TOKEN:
        raise RuntimeError("HCLOUD_TOKEN missing")
    url = f"https://api.hetzner.cloud/v1/servers/{int(server_id)}"
    r = requests.delete(url, headers={"Authorization": f"Bearer {HCLOUD_TOKEN}"}, timeout=30)
    if r.status_code in (200, 204, 202):
        return True
    if r.status_code == 404:
        return True
    raise RuntimeError(f"Hetzner delete failed: {r.status_code} {r.text}")
...
async def admin_backup_auto_prompt(cq: CallbackQuery, db: DB, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=kb([[("⬅️ برگشت","admin:backup")]])
    )
    await cq.answer()
...
async def admin_backup_auto_set(msg: Message, db: DB, state: FSMContext):
    ...
    if n <= 0:
        ...
        await state.clear()
        return await msg.answer("✅ بکاپ خودکار خاموش شد.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
    ...
    if not path or not os.path.exists(path):
        await state.clear()
        return await msg.answer("❌ بکاپی پیدا نشد.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
    ...
    try:
        ...
        await state.clear()
        return await msg.answer(f"❌ خطا در ارسال فایل: {htmlesc(str(e))}", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))

    await state.clear()
    await msg.answer("✅ فعال شد و بکاپ همین الان ارسال شد.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
...
async def admin_backup_manual(cq: CallbackQuery, db: DB):
    ...
    try:
        ...
        return await cq.message.answer(f"❌ خطا در ارسال فایل: {htmlesc(str(e))}")
    await cq.message.answer("✅ بکاپ ارسال شد.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
...
async def admin_backup_upload_apply(msg: Message, db: DB, state: FSMContext):
    if not is_admin(msg.from_user.id):
        return
    if not msg.document:
        return await msg.answer("❌ لطفاً فایل دیتابیس را به صورت Document ارسال کن.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
    ...
    try:
        ...
        await state.clear()
        return await msg.answer(f"❌ خطا: {htmlesc(str(e))}", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))

    await state.clear()
    await msg.answer("✅ دیتابیس اعمال شد.", reply_markup=kb([[("⬅️ برگشت","admin:backup")]]))
