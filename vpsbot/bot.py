...
def _fallback_admin_ids_from_baseinfo() -> set:
    # If ADMIN_IDS is empty, try to read admin id from DeltaBot baseInfo.php
    candidates = [
        "/var/www/html/deltabotvps/baseInfo.php",
        "/var/www/html/deltabotvps-main/baseInfo.php",
        "/var/www/html/deltabot/baseInfo.php",
        "/opt/deltabotvps/baseInfo.php",
        "/opt/deltabot/baseInfo.php",
    ]
    for p in candidates:
        try:
            if os.path.exists(p):
                txt = open(p, "r", encoding="utf-8", errors="ignore").read()
                m = re.search(r"\$admin\s*=\s*(\d+)", txt)
                if m:
...
# Ensure admin ids not empty (for integrated deployment)
if not ADMIN_IDS:
    ADMIN_IDS = _fallback_admin_ids_from_baseinfo()
