...
async def deltabotvps_entry(cq: CallbackQuery, db: DB, state: FSMContext):
    ...
    # If phone not registered, start registration flow (same behavior as /start)
...
# Legacy callbacks (older keyboards) → redirect to current buy flow / home.
LEGACY_CALLBACK_MAP = {
    # main buy button
    'buy': 'buy:start',
    'buy_service': 'buy:start',
    'buy:service': 'buy:start',
    'buy:server': 'buy:start',
    'cloud:buy': 'buy:start',
    'server:buy': 'buy:start',
    # some older home/menu keys
    'home:start': 'home',
    'main:menu': 'home',
    'main': 'home',
}

@router.callback_query(F.data.in_(list(LEGACY_CALLBACK_MAP.keys())))
async def legacy_callback_redirect(cq: CallbackQuery, db: DB, state: FSMContext):
    target = LEGACY_CALLBACK_MAP.get(cq.data)
    if target == 'buy:start':
        return await buy_start(cq, db, state)
    if target == 'home':
        # show main menu
        await state.clear()
        text, markup = await main_menu(db, cq.from_user.id)
...
async def admin_user_view_common(bot_: Bot, chat_id: int, db: DB, uid: int):
    ...
    await bot_.send_message(chat_id, text, parse_mode="HTML", reply_markup=kb([
        ...
        [("🗑 حذف از دیتابیس", f"admin:udel:ask:{uid}")],
        [("برگشت","admin:users")]
    ]))


@router.callback_query(F.data.startswith("admin:udel:ask:"))
async def admin_user_delete_confirm(cq: CallbackQuery, db: DB):
    if not is_admin(cq.from_user.id):
        return await cq.answer("دسترسی ندارید.", show_alert=True)
    ...
    txt = (
        f"{glass_header('حذف از دیتابیس')}\n"
        f"{GLASS_DOT} کاربر <code>{uid}</code> (@{htmlesc(username)}) حذف شود؟\n\n"
        f"⚠️ با این کار تمام سفارش‌ها، فاکتورها، خریدهای ترافیک و تیکت‌های این کاربر از دیتابیس پاک می‌شود.\n"
        f"بعد از حذف، کاربر اگر دوباره /start بزند مثل کاربر جدید ثبت‌نام می‌کند."
    )
    await cq.message.edit_text(
        txt,
        parse_mode="HTML",
        reply_markup=kb([
            [("✅ بله حذف کن", f"admin:udel:do:{uid}")],
            [("❌ لغو", f"admin:user:{uid}")],
            [("برگشت", "admin:users")],
        ]),
    )
    await cq.answer()
...
@router.callback_query(F.data.startswith("admin:udel:do:"))
async def admin_user_delete_do(cq: CallbackQuery, db: DB):
    if not is_admin(cq.from_user.id):
        return await cq.answer("دسترسی ندارید.", show_alert=True)
    uid = int(cq.data.split(":")[-1])
    await db.delete_user_full(uid)
    await cq.message.edit_text(
        f"✅ کاربر <code>{uid}</code> از دیتابیس حذف شد.",
        parse_mode="HTML",
        reply_markup=kb([[ ("📋 لیست کاربران", "admin:userlist:0") ], [("برگشت","admin:users")]]),
    )
    await cq.answer("حذف شد.")
...
# Fallback for unknown callbacks (usually because user clicked an old message keyboard)
@router.callback_query()
async def unknown_callback_fallback(cq: CallbackQuery, db: DB, state: FSMContext):
    # Try soft recovery: show main menu so user gets fresh buttons
    try:
        await state.clear()
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            pass
    # Tell user briefly (alert) so they understand what happened
    try:
        await cq.answer('منوی شما قدیمی بود؛ منو به‌روزرسانی شد. اگر باز هم مشکل بود /start بزنید.', show_alert=True)
    except Exception:
        pass
