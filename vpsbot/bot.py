...
async def buy_country(cq: CallbackQuery, db: DB, state: FSMContext):
    ...
    if provider == "manual":
        ...
        await cq.message.edit_text(
            f"{glass_header('انتخاب پلن: ترافیک تمامی پلن ها 10 گیگ میباشد میتوانید بعد از خرید از قسمت سفارش های من ترافیک اضافه برای سرویس خود خریداری نمایید قیمت هر یک ترابایت ترافیک 700,000 هزار تومان میباشد')}\n" + "\n\n".join(lines),
...
async def order_view(cq: CallbackQuery, db: DB):
    ...
    if mon_url:
        text += f"{GLASS_DOT} مانیتورینگ: {mon_url}\n"
        if mon_user:
            text += f"{GLASS_DOT} نام کاربری مانیتورینگ: `{mon_user}`\n"
        if mon_pass:
            text += f"{GLASS_DOT} رمز عبور مانیتورینگ: `{mon_pass}`\n"
    rows = []
    ...
    if is_manual:
        # For manual deliveries: hide provider actions and show monitoring page instead.
        if mon_url:
            rows.append([("📡 صفحه مانیتورینگ", f"url:{mon_url}")])
        rows += [
