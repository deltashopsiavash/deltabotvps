...
function xuiBuildOrderLinksText($botState, $botUrl, $uniqid, $subLink = ''){
    ...
    if(xuiBotStateIsOn($botState, 'subLinkState', 'on') && trim((string)$subLink) !== ''){
        $sections[] = "🌐 subscription : <code>" . $subLink . "</code>";
    }
...
 /");
    ...
    $extractWsHostValue = function($streamSettings, $fallback = '') use ($normalizeHostValue) {
        $candidates = [];
        if(is_object($streamSettings) && isset($streamSettings->wsSettings)){
            $wsSettings = $streamSettings->wsSettings;
            if(isset($wsSettings->headers)){
                $headers = $wsSettings->headers;
                if(is_object($headers)){
                    foreach(['Host','host',':authority','authority','serverName','server_name'] as $key){
                        if(isset($headers->$key)) $candidates[] = $headers->$key;
                    }
            ...
            foreach(['host','Host','serverName','server_name'] as $key){
                if(isset($wsSettings->$key)) $candidates[] = $wsSettings->$key;
            }
        }
        if($fallback !== '') $candidates[] = $fallback;
        foreach($candidates as $candidate){
            $candidate = $normalizeHostValue($candidate);
            if($candidate !== '') return $candidate;
        }
        return '';
    };

    $collectExternalEndpoints = function($node) use (&$collectExternalEndpoints, $normalizeHostValue) {
        $results = [];
        if(is_array($node)){
            foreach($node as $item){
                $results = array_merge($results, $collectExternalEndpoints($item));
            }
            return $results;
        }
        if(!is_object($node)) return $results;

        $looksLikeProxyContainer = false;
        foreach(['externalProxy','externalProxies','proxySettings','proxies','proxy'] as $key){
            if(isset($node->$key)){
                $looksLikeProxyContainer = true;
                $results = array_merge($results, $collectExternalEndpoints($node->$key));
            }
        }

        $hostValue = '';
        foreach(['host','address','server','domain','dest','destination'] as $key){
            if(isset($node->$key)){
                $hostValue = $normalizeHostValue($node->$key);
                if($hostValue !== '') break;
            }
        }
        $portValue = 0;
        foreach(['port','serverPort','targetPort','listenPort'] as $key){
            if(isset($node->$key) && intval($node->$key) > 0){
                $portValue = intval($node->$key);
                break;
            }
        }
        if($hostValue !== '' && $portValue > 0){
            $results[] = $hostValue . ':' . $portValue;
        }elseif(!$looksLikeProxyContainer){
            foreach($node as $value){
                if(is_object($value) || is_array($value)){
                    $results = array_merge($results, $collectExternalEndpoints($value));
                }
            }
        }
        return $results;
    ...
    foreach($response as $row){
        if($inbound_id == 0){
            ...
            if($clients[0]->id == $uniqid || $clients[0]->password == $uniqid) {
                ...
                if($netType == 'ws') {
                    $header_type = xuiDecodeField($row->streamSettings)->wsSettings->header->type;
                    $path = xuiDecodeField($row->streamSettings)->wsSettings->path;
                    $host = $extractWsHostValue(xuiDecodeField($row->streamSettings), '');
                }
    ...
    $streamSettingsObj = isset($row->streamSettings) ? xuiDecodeField($row->streamSettings) : null;
    $externalEndpoints = [];
    if($streamSettingsObj){
        $externalEndpoints = $collectExternalEndpoints($streamSettingsObj);
        $externalEndpoints = array_values(array_unique(array_filter($externalEndpoints)));
    }
    $serverIp = explode("\n",$server_ip);
    $outputLink = array();
    foreach($serverIp as $server_ip){
        ...
        $endpointList = [$server_ip . ':' . intval($port)];
        if(!empty($externalEndpoints)) $endpointList = array_merge($endpointList, $externalEndpoints);
        $endpointList = array_values(array_unique(array_filter($endpointList)));
        foreach($endpointList as $endpoint){
            $endpointHost = $server_ip;
            $endpointPort = intval($port);
            if(strpos($endpoint, ':') !== false){
                $parts = explode(':', $endpoint);
                $endpointPort = intval(array_pop($parts));
                $endpointHost = implode(':', $parts);
            }
            if($endpointHost === '' || $endpointPort <= 0) continue;
        if($inbound_id == 0) {
            if($protocol == 'vless'){
                if($rahgozar == true){
                ...
                $outputlink = "$protocol://$uniqid@$endpointHost:" . ($rahgozar == true?($customPort!="0"?$customPort:"443"):$endpointPort) . "?type=$netType&security=" . ($rahgozar==true?"tls":$tlsStatus) . "{$psting}#$remark";
                if($netType == 'grpc' && $tlsStatus != "reality"){
                    if($tlsStatus == 'tls'){
                        $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&serviceName=$serviceName&sni=$sni#$remark";
                    }else{
                        $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&serviceName=$serviceName#$remark";
                    }
            ...
            if($protocol == 'trojan'){
                ...
                $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus{$psting}#$remark";
                ...
                $vmessArr = [
                    ...
                    "ps"=> $remark,
                    "add"=> $endpointHost,
                    "port"=> $rahgozar == true?($customPort!=0?$customPort:443):$endpointPort,
                    "id"=> $uniqid,
                    "aid"=> 0,
                    "net"=> $netType,
                    "type"=> $kcpType ? $kcpType : "none",
                    "host"=> ($rahgozar == true && empty($host))? $endpointHost:$normalizeHostValue($host),
            ...
            if($protocol == 'vless'){
                ...
                if($netType == 'tcp'){
                    ...
                    if($rahgozar == true)$outputlink = "$protocol://$uniqid@$endpointHost:" . ($customPort!=0?$customPort:"443") . "?type=$netType&security=tls" . $buildWsQuery(($path ?? '/') . ($customPath == true?"?ed=2048":""), $host ?? '', true) . "{$psting}#$remark";
                    else $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus" . $buildWsQuery($path ?? '/', $host ?? '') . "{$psting}#$remark";
                }
                elseif($netType == 'kcp')
                    $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&headerType=$kcpType&seed=$kcpSeed#$remark";
                elseif($netType == 'grpc'){
                    if($tlsStatus == 'tls'){
                        $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&serviceName=$serviceName&sni=$sni#$remark";
                    }
                    elseif($tlsStatus=="reality"){
                        $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&serviceName=$serviceName&fp=$fp&pbk=$pbk&sni=$sni" . ($flow != ""?"&flow=$flow":"") . "&sid=$sid&spx=$spiderX#$remark";
                    }
                    else{
                        $outputlink = "$protocol://$uniqid@$endpointHost:$endpointPort?type=$netType&security=$tlsStatus&serviceName=$serviceName#$remark";
                    }
    ...
    $outputLink = array_values(array_unique(array_filter($outputLink)));
    return $outputLink;
}
