...
function getResellerBotBuiltVolumeTotal($rid){
    ...
    $stmt = $conn->prepare("SELECT o.`fileid`, SUM(CASE WHEN COALESCE(sp.`quota_charge_volume`,0) > 0 THEN sp.`quota_charge_volume` WHEN sp.`volume` IS NULL THEN 0 ELSE sp.`volume` END) AS `sum_volume` FROM `orders_list` o LEFT JOIN `server_plans` sp ON sp.`id` = o.`fileid` WHERE o.`status`=1 GROUP BY o.`fileid`");
    if($stmt){
        $stmt->execute();
        $res = $stmt->get_result();
        $stmt->close();
...
function userCanBuyVolume($userId, $requestedVolume, &$errorMessage=null){
    global $isChildBot, $currentBotInstanceId;
    $requestedVolume = (int)ceil((float)$requestedVolume);
