<?php
// PasarGuard renewal suggestion / expiry cleanup schema (safe, idempotent)
@$connection->query("ALTER TABLE `users` ADD `pg_renew_suggestion` TINYINT(1) NOT NULL DEFAULT 1 AFTER `spam_info`");
@$connection->query("ALTER TABLE `orders_list` ADD `expired_warned_at` INT NOT NULL DEFAULT 0 AFTER `agent_bought`");
@$connection->query("ALTER TABLE `orders_list` ADD `delete_after` INT NOT NULL DEFAULT 0 AFTER `expired_warned_at`");
