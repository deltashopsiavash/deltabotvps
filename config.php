...
function getBotSettingKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=>$nativeSubLink,'callback_data'=>"changeBotnativeSubLinkState"],
            ['text'=>"ساب پنل اصلی",'callback_data'=>"deltach"]
        ],
        [
...
function getUserOrderDetailKeys($id, $offset = 0){
    ...
    if($order->num_rows==0){
        ...
        $nativeSubLink = (($botState['nativeSubLinkState'] ?? 'off') == "on") ? getClientNativeSubscriptionLink($server_id, $uuid, $inbound_id, $remark) : "";
        if(!empty($nativeSubLink)) $subLink .= (empty($subLink)?"":"\n") . "<code>" . $nativeSubLink . "</code>";
...
function getClientNativeSubscriptionLink($server_id, $uuid, $inbound_id = 0, $remark = null){
    global $connection;
    ...
    if($serverType == "marzban"){
        if(!empty($remark)){
            ...
            if(isset($userInfo->subscription_url)){
                $subscriptionPath = $userInfo->subscription_url;
                if(strpos($subscriptionPath, 'http') === 0) return $subscriptionPath;
                return $panel_url . (substr($subscriptionPath, 0, 1) == '/' ? $subscriptionPath : '/' . $subscriptionPath);
            }
        }
        if(!empty($uuid)) return $panel_url . '/sub/' . $uuid;
        return "";
    }

    $response = getJson($server_id);
    if(!$response || !isset($response->obj)) return "";

    $targetSubId = "";
    foreach($response->obj as $row){
        if($inbound_id != 0 && $row->id != $inbound_id) continue;
        ...
        foreach($clients as $client){
            ...
            if($clientId == $uuid || $clientPassword == $uuid){
    ...
    if(empty($targetSubId)) return "";
    return $panel_url . '/sub/' . $targetSubId;
}
