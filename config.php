...
function pasarguardFetchGroups($server_id, $token = null){
    ...
    $paths = ['/api/groups', '/api/group', '/api/user-groups'];
    ...
    foreach($bases as $base){
        foreach($paths as $path){
            ...
            $items = $json;
            foreach(['items','results','data','groups'] as $k){
            ...
            foreach($items as $g){
                ...
                $gid = (int)$g['id'];
                if($gid <= 0) continue;
                $name = $g['name'] ?? ($g['title'] ?? ($g['remark'] ?? ('Group ' . $gid)));
...
function addPasarguardUser($server_id, $remark, $volume, $days, $plan_id = null, $group_id = 0){
    ...
    $panel_url = pasarguardPanelApiBase($server_info['panel_url']);
    $token = getPasarguardToken($server_id);
    ...
    $groupIds = array_values(array_unique(array_filter($groupIds)));
    if(empty($groupIds)){
        // If you use exactly one PasarGuard group, the bot automatically attaches that group.
    ...
    foreach(pasarguardPanelApiBases($server_info['panel_url']) as $apiBase){
        ...
        if(!$err && $http >= 200 && $http < 300 && $response) break;
    }
    if($err) return (object)['success'=>false,'msg'=>$err];
    if($http < 200 || $http >= 300 || !$response){
        $msg = $raw;
        if($response && isset($response->detail)) $msg = is_string($response->detail) ? $response->detail : json_encode($response->detail, JSON_UNESCAPED_UNICODE);
        return (object)['success'=>false,'msg'=>'PasarGuard create user failed: ' . $msg, 'payload'=>$payload];
    }

    $sub = $response->subscription_url ?? ($response->sub_link ?? ($response->subscription ?? ''));
    if($sub === ''){
        // Some PasarGuard versions do not return subscription_url on create; fetch the created user again.
        $createdInfo = getPasarguardUserInfo($server_id, $remark);
        if(is_object($createdInfo)){
            $sub = $createdInfo->subscription_url ?? ($createdInfo->sub_link ?? ($createdInfo->subscription ?? ''));
        }
    }
    if($sub !== ''){
    ...
    return (object)[
        ...
        'vray_links' => $sub !== '' ? [$sub] : [],
        'obj' => $response,
        'pasarguard_user_id' => $response->id ?? null
    ];
}
...
function getPasarguardUserInfo($server_id, $remark){
    ...
    foreach(pasarguardPanelApiBases($server_info['panel_url']) as $apiBase){
        $curl = curl_init();
        curl_setopt_array($curl, [
            CURLOPT_URL => $apiBase . '/api/user/by-username/' . rawurlencode($remark),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_HTTPGET => true,
            CURLOPT_CONNECTTIMEOUT => 10,
            CURLOPT_TIMEOUT => 20,
        ...
        $http = curl_getinfo($curl, CURLINFO_HTTP_CODE);
        curl_close($curl);
        if(!$err && $http >= 200 && $http < 300 && trim((string)$raw) !== '') break;
    }
    if($err) return (object)['success'=>false,'msg'=>$err];
    return json_decode($raw) ?: (object)['success'=>false,'msg'=>$raw];
}
...
function editMarzbanConfig($server_id,$info){
    ...
    if(($typeRow['type'] ?? '') === 'pasarguard'){
        ...
        if(isset($info['plus_day'])){
            ...
            $base = (!empty($user->expire) && strtotime($user->expire) > time()) ? strtotime($user->expire) : time();
            $fields['expire'] = gmdate('c', $base + ((int)$info['plus_day'] * 86400));
        }
        if(isset($info['volume'])) $fields['data_limit'] = (int)floor(((float)$info['volume']) * 1073741824);
