...
function getPasarguardToken($server_id){
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => $panel_url . '/api/admin/token',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST => true,
    ...
    if(!$json || empty($json->access_token)) return (object)['success'=>false,'msg'=>'PasarGuard login failed: ' . $raw];
    $json->success = true;
    return $json;
}
...
function pasarguardFetchGroups($server_id, $token = null){
    ...
    $base = pasarguardPanelApiBase($server_info['panel_url']);
...
function addPasarguardUser($server_id, $remark, $volume, $days, $plan_id = null, $group_id = 0){
    ...
    $curl = curl_init();
    curl_setopt_array($curl, [
        CURLOPT_URL => $panel_url . '/api/user',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST => true,
        CURLOPT_CONNECTTIMEOUT => 10,
        CURLOPT_TIMEOUT => 30,
...
function getPasarguardUserInfo($server_id, $remark){
    ...
    $curl = curl_init();
    curl_setopt_array($curl, [
        CURLOPT_URL => $panel_url . '/api/user/by-username/' . rawurlencode($remark),
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HTTPGET => true,
        CURLOPT_CONNECTTIMEOUT => 10,
        CURLOPT_TIMEOUT => 20,
