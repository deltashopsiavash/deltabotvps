...
function getActiveSpecialOffer($offerId = 0){
    ...
    try{
        ...
        $stmt->close();
        if($row) $row['remaining'] = max(0, (int)$row['stock_total'] - (int)$row['stock_sold']);
        return $row ?: null;
    }catch(Throwable $e){
...
function specialOfferAttachPay($hashId, $offerId, $quantity=1, &$errorCode=null){
    ...
    if(!$offer || (int)$offer['remaining'] < $quantity){ $errorCode='stock'; return false; }
    $payStmt=$connection->prepare("SELECT `user_id`,`plan_id` FROM `pays` WHERE `hash_id`=? LIMIT 1");
    $payStmt->bind_param('s',$hashId); $payStmt->execute();
    $pay=$payStmt->get_result()->fetch_assoc(); $payStmt->close();
    if(!$pay || (int)$pay['plan_id']!==(int)$offer['plan_id']){ $errorCode='payment'; return false; }
    ...
    $stmt=$connection->prepare("UPDATE `pays` SET `special_offer_id`=?,`special_offer_qty`=? WHERE `hash_id`=? AND `plan_id`=?");
    $planId=(int)$offer['plan_id'];
    $stmt->bind_param('iisi',$offerId,$quantity,$hashId,$planId);
    $stmt->execute();
    $ok=$stmt->affected_rows>0;
    $stmt->close();
    if(!$ok) $errorCode='payment';
    return $ok;
}
