...
function getUserBoughtVolumeTotal($userId){
    global $connection;
    $stmt = $connection->prepare("SELECT COALESCE(SUM(`volume`),0) AS total FROM `pays` WHERE `user_id`=? AND `volume` IS NOT NULL AND `volume` > 0 AND `state` IN ('approved','paid_with_wallet')");
