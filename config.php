...
function specialOfferReservedQuantity($offerId, $excludeHash=''){
    ...
    $now=time();
    $sql="SELECT COALESCE(SUM(`special_offer_qty`),0) AS reserved_count
          FROM `pays`
          WHERE `special_offer_id`=?
            AND `special_offer_reserved_until`>?
            AND `state` IN ('pending','have_sent','0')";
    if($excludeHash!=='') $sql.=" AND `hash_id`<>?";
    try{
...
function specialOfferReservationIsValid($payInfo, $expireIfNeeded=true){
    ...
    $valid=((string)($payInfo['state'] ?? '')==='pending' && (int)($payInfo['special_offer_reserved_until'] ?? 0)>time());
...
// The normal one-minute cron remains the fallback, while this worker wakes the
// notification endpoint at the exact campaign start time.
function specialOfferScheduleStartWakeup($offer){
    ...
    $endpoint=rtrim((string)$botUrl,'/').'/settings/specialOffers.php';
    $botInstanceId=(int)($GLOBALS['currentBotInstanceId'] ?? 0);
    if($botInstanceId>0) $endpoint.='?bid='.$botInstanceId;

    $php=(defined('PHP_BINARY') && PHP_BINARY!=='') ? PHP_BINARY : 'php';
    $command='nohup '.escapeshellarg($php).' '.escapeshellarg($worker).' '
        .escapeshellarg($endpoint).' '.escapeshellarg((string)$startsAt).' '
        .escapeshellarg((string)$offerId).' >/dev/null 2>&1 &';
...
function specialOfferAttachPay($hashId, $offerId, $quantity=1, &$errorCode=null){
    ...
    if(((int)$offer['ends_at']-time())<300){ $errorCode='ending_soon'; return false; }
    ...
    try{
        ...
        // Never create a five-minute reservation which outlives the campaign.
        // At exactly five minutes it is valid; with even one second less it is blocked.
        if(((int)$locked['ends_at']-$now)<300){
            $connection->rollback(); $errorCode='ending_soon'; return false;
        }
        ...
        $reservedUntil=$now+300; $planId=(int)$offer['plan_id'];
