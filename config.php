...
function getOrderDetailKeys($from_id, $id, $offset = 0){
    ...
    if($order->num_rows==0){
        ...
        if($botState['subLinkState'] == "on"){
            ...
            $subLink = $subResolved !== '' ? "<code>" . htmlspecialchars($subResolved, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</code>" : "";
        }else $subLink = "";
...
function xuiBuildOrderFallbackButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if((($botState['configLinkState'] ?? 'on') !== 'off') && !empty(xuiNormalizeConfigLinks($configLinks)) && $lookupKey !== ''){
        $rows[] = [[ 'text'=>'📋 کپی کانفیگ', 'callback_data'=>'xuiCopyConfig_' . $lookupKey ]];
    }
    if((($botState['subLinkState'] ?? 'on') !== 'off') && trim((string)$subLink) !== '' && $lookupKey !== ''){
        $rows[] = [[ 'text'=>'🌐 کپی سابسکریپشن', 'callback_data'=>'xuiCopySub_' . $lookupKey ]];
    }
...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    // IMPORTANT: Do not use Telegram copy_text here. On some bot/API setups it makes
    // Telegram reject the whole delivery message, so the user only receives service specs.
    // Normal callback buttons are stable; when clicked, the bot sends all links in one <pre> block.
    ...
    if((($botState['subLinkState'] ?? 'on') !== 'off') && $subLink !== '' && $lookupKey !== ''){
        $rows[] = [[
            'text' => '🌐 کپی سابسکریپشن',
