...
 /");
    ...
    $extractWsHeaderHost = function($wsSettingsObj, $wsSettingsAssoc, $requestHeaderRaw = '', $streamSettingsRaw = '') use ($normalizeHostValue) {
        ...
        $bags = [];
        foreach(['headers', 'header', 'requestHeaders', 'requestHeader'] as $key){
            if(is_object($wsSettingsObj) && isset($wsSettingsObj->$key)) $bags[] = $wsSettingsObj->$key;
            if(is_array($wsSettingsAssoc) && isset($wsSettingsAssoc[$key])) $bags[] = $wsSettingsAssoc[$key];
        }

        foreach($bags as $bag){
            if(is_object($bag)) $bag = get_object_vars($bag);
            ...
            foreach($bag as $key => $value){
                ...
                if(is_array($value) || is_object($value)){
                    ...
                    if($name === 'host'){
                        $host = $tryHost($item['value'] ?? ($item['val'] ?? ($item['host'] ?? '')));
                        if($host !== '') return $host;
                    }
                }
            }
        ...
        foreach([
            $wsSettingsObj->host ?? null,
            $wsSettingsAssoc['host'] ?? null,
            $wsSettingsObj->Host ?? null,
            $wsSettingsAssoc['Host'] ?? null,
            $wsSettingsObj->authority ?? null,
            $wsSettingsAssoc['authority'] ?? null
        ] as $candidate){
            $host = $tryHost($candidate);
            if($host !== '') return $host;
        }
        ...
        foreach($rawCandidates as $raw){
            ...
            foreach([
                '/"(?:host|Host|authority)"\s*:\s*"([^"\\]+)"/i',
                '/"(?:host|Host|authority)"\s*:\s*\[\s*"([^"\\]+)"/i',
                '/"name"\s*:\s*"Host"\s*,\s*"value"\s*:\s*"([^"\\]+)"/i',
                '/"key"\s*:\s*"Host"\s*,\s*"value"\s*:\s*"([^"\\]+)"/i'
            ] as $pattern){
    ...
    $extractWsExternalTargets = function($wsSettingsAssoc, $streamSettingsRaw = '') use ($normalizeHostValue) {
        ...
        $parseItem = function($item) use (&$appendTarget, $normalizeHostValue) {
            if(is_object($item)) $item = get_object_vars($item);
            if(is_string($item)){
                $trimmed = trim($item);
                if($trimmed === '') return;
                if(preg_match_all('/([a-z0-9._-]+)\s*:\s*(\d{2,5})/i', $trimmed, $matches, PREG_SET_ORDER)){
                    foreach($matches as $match) $appendTarget($match[1], $match[2]);
                }
                return;
            }
            ...
            if(count($vals) >= 2){
                if(!is_array($vals[0]) && !is_object($vals[0]) && !is_numeric($vals[0]) && is_numeric($vals[1])){
                    $appendTarget($vals[0], $vals[1]);
                }
                if(count($vals) >= 3 && !is_array($vals[1]) && !is_object($vals[1]) && !is_numeric($vals[1]) && is_numeric($vals[2])){
                    $appendTarget($vals[1], $vals[2]);
                }
            }
        };

        $containers = [];
        $collectContainers = function($value, $keyPath = '') use (&$collectContainers, &$containers) {
            if(is_object($value)) $value = get_object_vars($value);
            if(!is_array($value)) return;
            foreach($value as $key => $child){
                $keyLower = strtolower((string)$key);
                $childPath = $keyPath === '' ? $keyLower : ($keyPath . '.' . $keyLower);
                if((strpos($keyLower, 'external') !== false && strpos($keyLower, 'proxy') !== false) || $keyLower === 'externalproxy' || $keyLower === 'externalproxies'){
                    $containers[] = $child;
                }
                $collectContainers($child, $childPath);
            }
        };

        if(is_array($wsSettingsAssoc)) $collectContainers($wsSettingsAssoc);

        if(empty($containers) && $streamSettingsRaw !== ''){
            $decoded = json_decode((string)$streamSettingsRaw, true);
            if(json_last_error() === JSON_ERROR_NONE && is_array($decoded)){
                $collectContainers($decoded);
            }
            if(preg_match_all('/([a-z0-9._-]+)\s*:\s*(\d{2,5})/i', (string)$streamSettingsRaw, $matches, PREG_SET_ORDER)){
        ...
        foreach($containers as $container){
            ...
            $items = array_keys($container) === range(0, count($container) - 1) ? $container : [$container];
