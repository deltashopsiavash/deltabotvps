...
function forwardUpdateToVpsbot($raw){
    ...
    // (VPSBot will respond to Telegram via Bot API using the same BOT_TOKEN).
...
// Cloud VPS buy button (ONLY on mother bot, and only when enabled)
...
function getAdminKeys(){
    ...
    // QRCode background (available on BOTH mother + reseller bots) - single column
    $rows[] = [
...
function getBotSettingKeys(){    global $connection, $mainValues, $buttonValues, $isChildBot;    // Load bot state settings    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'BOT_STATES'");    $stmt->execute();    $botState = $stmt->get_result()->fetch_assoc()['value'];    if(!is_null($botState)) $botState = json_decode($botState,true);    else $botState = array();    $stmt->close();    // Ensure defaults    if(!isset($botState['cloudVpsPanelState'])) $botState['cloudVpsPanelState'] = 'on';    $changeProtocole = ($botState['changeProtocolState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $renewAccount = ($botState['renewAccountState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $plandelkhahwiz = ($botState['plandelkhahState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $switchLocation = ($botState['switchLocationState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $increaseTime = ($botState['increaseTimeState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $increaseVolume = ($botState['increaseVolumeState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $subLink = ($botState['subLinkState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $configLink = ($botState['configLinkState']??'on')=="off"?$buttonValues['off']:$buttonValues['on'];    $renewConfigLink = ($botState['renewConfigLinkState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $updateConfigLink = ($botState['updateConfigLinkState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $individualExistence = ($botState['individualExistence']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $sharedExistence = ($botState['sharedExistence']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $testAccount = ($botState['testAccount']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $agency = ($botState['agencyState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $agencyPlanDiscount = ($botState['agencyPlanDiscount']??'off')=="on"?$buttonValues['plan_discount']:$buttonValues['server_discount'];    $qrConfig = ($botState['qrConfigState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $qrSub = ($botState['qrSubState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $cloudVpsPanel = ($botState['cloudVpsPanelState']??'on')=="on"?$buttonValues['on']:$buttonValues['off'];    $requirePhone = ($botState['requirePhone']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $requireIranPhone = ($botState['requireIranPhone']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $sellState = ($botState['sellState']??'on')=="on"?$buttonValues['on']:$buttonValues['off'];    $robotState = ($botState['botState']??'on')=="on"?$buttonValues['on']:$buttonValues['off'];    $searchState = ($botState['searchState']??'off')=="on"?$buttonValues['on']:$buttonValues['off'];    $inviteBtn = (($botState['inviteButton']??'on')=='on')?$buttonValues['on']:$buttonValues['off'];    $updateConnectionState = ($botState['updateConnectionState']??'robot')=="robot"?"از روی ربات":"از روی سایت";    $rewaredTime = ($botState['rewaredTime']??0) . " ساعت";    switch($botState['remark']??''){        case "digits": $remarkType = "عدد رندم 5 حرفی"; break;        case "manual": $remarkType = "توسط کاربر"; break;        default: $remarkType = "آیدی و عدد رندوم"; break;    }    // payment keys are loaded elsewhere; keep for backward compatibility    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'PAYMENT_KEYS'");    $stmt->execute();    $paymentKeys = $stmt->get_result()->fetch_assoc()['value'];    if(!is_null($paymentKeys)) $paymentKeys = json_decode($paymentKeys,true);    else $paymentKeys = array();    $stmt->close();    $rows = [        [            ['text'=>"🎗 بنر بازاریابی 🎗",'callback_data'=>"inviteSetting"]            ],        [            ['text'=> $inviteBtn,'callback_data'=>"toggleInviteButton"],            ['text'=>"زیرمجموعه گیری",'callback_data'=>"deltach"]            ],        [            ['text'=> $updateConnectionState,'callback_data'=>"changeUpdateConfigLinkState"],            ['text'=>"آپدیت کانفیگ",'callback_data'=>"deltach"]            ],        [            ['text'=> $agency,'callback_data'=>"changeBotagencyState"],            ['text'=>"نمایندگی",'callback_data'=>"deltach"]            ],        [            ['text'=> $agencyPlanDiscount,'callback_data'=>"changeBotagencyPlanDiscount"],            ['text'=>"نوع تخفیف نمایندگی",'callback_data'=>"deltach"]            ],        [            ['text'=>$individualExistence,'callback_data'=>"changeBotindividualExistence"],            ['text'=>"موجودی اختصاصی",'callback_data'=>"deltach"]        ],        [            ['text'=>$sharedExistence,'callback_data'=>"changeBotsharedExistence"],            ['text'=>"موجودی اشتراکی",'callback_data'=>"deltach"]        ],        [            ['text'=>$testAccount,'callback_data'=>"changeBottestAccount"],            ['text'=>"اکانت تست",'callback_data'=>"deltach"]        ],        [            ['text'=>$changeProtocole,'callback_data'=>"changeBotchangeProtocolState"],            ['text'=>"تغییر پروتکل",'callback_data'=>"deltach"]        ],        [            ['text'=>$renewAccount,'callback_data'=>"changeBotrenewAccountState"],            ['text'=>"تمدید سرویس",'callback_data'=>"deltach"]        ],        [            ['text'=>$plandelkhahwiz,'callback_data'=>"changeBotplandelkhahState"],            ['text'=>"پلن دلخواه",'callback_data'=>"deltach"]        ],        [            ['text'=>$switchLocation,'callback_data'=>"changeBotswitchLocationState"],            ['text'=>"تغییر لوکیشن",'callback_data'=>"deltach"]        ],        [            ['text'=>$increaseTime,'callback_data'=>"changeBotincreaseTimeState"],            ['text'=>"افزایش زمان",'callback_data'=>"deltach"]        ],        [            ['text'=>$increaseVolume,'callback_data'=>"changeBotincreaseVolumeState"],            ['text'=>"افزایش حجم",'callback_data'=>"deltach"]        ],        [            ['text'=>$requirePhone,'callback_data'=>"changeBotrequirePhone"],            ['text'=>"تأیید شماره",'callback_data'=>"deltach"]        ],        [            ['text'=>$requireIranPhone,'callback_data'=>"changeBotrequireIranPhone"],            ['text'=>"تأیید شماره ایرانی",'callback_data'=>"deltach"]        ],        [            ['text'=>$sellState,'callback_data'=>"changeBotsellState"],            ['text'=>"فروش",'callback_data'=>"deltach"]        ],        [            ['text'=>$robotState,'callback_data'=>"changeBotbotState"],            ['text'=>"وضعیت ربات",'callback_data'=>"deltach"]        ],        [            ['text'=>$subLink,'callback_data'=>"changeBotsubLinkState"],            ['text'=>"لینک ساب و مشخصات وب",'callback_data'=>"deltach"]        ],        [            ['text'=>$configLink,'callback_data'=>"changeBotconfigLinkState"],            ['text'=>"لینک کانفیگ",'callback_data'=>"deltach"]        ],        [            ['text'=>$searchState,'callback_data'=>"changeBotsearchState"],            ['text'=>"مشخصات کانفیگ",'callback_data'=>"deltach"]        ],        [            ['text'=>$renewConfigLink,'callback_data'=>"changeBotrenewConfigLinkState"],            ['text'=>"دریافت لینک جدید",'callback_data'=>"deltach"]        ],        [            ['text'=>$updateConfigLink,'callback_data'=>"changeBotupdateConfigLinkState"],            ['text'=>"بروز رسانی لینک",'callback_data'=>"deltach"]        ],        [            ['text'=>$qrConfig,'callback_data'=>"changeBotqrConfigState"],            ['text'=>"کیو آر کد کانفیگ",'callback_data'=>"deltach"]        ],        [            ['text'=>$qrSub,'callback_data'=>"changeBotqrSubState"],            ['text'=>"کیو آر کد ساب",'callback_data'=>"deltach"]        ],        [            ['text'=>$remarkType,'callback_data'=>"changeConfigRemarkType"],            ['text'=>"نوع ریمارک",'callback_data'=>"deltach"]        ],        [            ['text'=>$rewaredTime,'callback_data'=>'editRewaredTime'],            ['text'=>"ارسال گزارش درآمد", 'callback_data'=>'deltach']            ],        [            ['text'=>$botState['cartToCartAutoAcceptState']=="on"?$buttonValues['on']:$buttonValues['off'],'callback_data'=>"changeBotcartToCartAutoAcceptState"],            ['text'=>"تأیید خودکار کارت به کارت",'callback_data'=>"deltach"]        ],        ($botState['cartToCartAutoAcceptState']=="on"?[            ['text'=>($botState['cartToCartAutoAcceptType'] == "0"?"نماینده":($botState['cartToCartAutoAcceptType'] == "1"?"کاربر":"همه")),'callback_data'=>"changeBotcartToCartAutoAcceptType"],            ['text'=>"نوع تأیید",'callback_data'=>"deltach"]        ]:[]),        ($botState['cartToCartAutoAcceptState']=="on"?[            ['text'=>($botState['cartToCartAutoAcceptTime']??"10") . " دقیقه",'callback_data'=>"editcartToCartAutoAcceptTime"],            ['text'=>"زمان تأیید خودکار ",'callback_data'=>"deltach"]        ]:[]),        [            ['text'=>$cloudVpsPanel,'callback_data'=>'changeBotcloudVpsPanelState'],            ['text'=>'سرور ابری','callback_data'=>'deltach']        ],        [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]]    ];    // Remove empty rows generated by conditional blocks    $rows = array_values(array_filter($rows, function($r){ return is_array($r) && count($r) > 0; }));    // Hide cloud VPS toggle inside reseller bots    if(!empty($isChildBot)){        $rows = array_values(array_filter($rows, function($r){            return !(is_array($r) && isset($r[0]['callback_data']) && $r[0]['callback_data']==='changeBotcloudVpsPanelState');        }));    }    $rows = normalizeKeyboard($rows);    return json_encode(['inline_keyboard'=>$rows], 488);}function getBotReportKeys(){
    global $connection, $buttonValues;

    // Use safe helpers so reseller DBs without some tables don't break this page
    $allUsers      = _safeCountRows($connection, 'users');
    $allOrders     = _safeCountRows($connection, 'orders_list');
    $allServers    = _safeCountRows($connection, 'server_config');
    $allCategories = _safeCountRows($connection, 'server_categories');
    $allPlans      = _safeCountRows($connection, 'server_plans');

    // Total income
    $total = _safeSum($connection, 'pays', 'price', "(`state`='paid' OR `state`='approved')");
    $totalRewards = number_format((int)$total) . " تومان";

    // Month income (from first day of Jalali month)
    $monthReward = 0;
    $weekReward  = 0;
    $dayReward   = 0;
    ...
    if(function_exists('jdate') && function_exists('jalali_to_gregorian')){
        ...
        $gregorian = jalali_to_gregorian((int)$persian[0], (int)$persian[1], (int)$persian[2]);
        $date = $gregorian[0] . '-' . $gregorian[1] . '-' . $gregorian[2];
        $monthStart = strtotime($date);

        if($monthStart){
            $monthReward = _safeSum($connection, 'pays', 'price', "`request_date` > ? AND (`state`='paid' OR `state`='approved')", ['i', (int)$monthStart]);
        }

        $weekStart = strtotime('-' . (date('w')+1) . ' days');
        if($weekStart){
            $weekReward = _safeSum($connection, 'pays', 'price', "`request_date` > ? AND (`state`='paid' OR `state`='approved')", ['i', (int)$weekStart]);
        }

        $todayStart = strtotime('today');
        if($todayStart){
            $dayReward = _safeSum($connection, 'pays', 'price', "`request_date` > ? AND (`state`='paid' OR `state`='approved')", ['i', (int)$todayStart]);
        }
    }

    $monthRewardTxt = number_format((int)$monthReward) . " تومان";
    $weekRewardTxt  = number_format((int)$weekReward) . " تومان";
    $dayRewardTxt   = number_format((int)$dayReward) . " تومان";
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=>$dayRewardTxt,'callback_data'=>'deltach'],
            ['text'=>'درآمد امروز','callback_data'=>'deltach']
        ],
        [
            ['text'=>$weekRewardTxt,'callback_data'=>'deltach'],
            ['text'=>'درآمد هفته','callback_data'=>'deltach']
        ],
        [
            ['text'=>$monthRewardTxt,'callback_data'=>'deltach'],
            ['text'=>'درآمد ماه','callback_data'=>'deltach']
        ],
        [
            ['text'=>$buttonValues['back_button'] ?? 'برگشت','callback_data'=>'managePanel']
        ]
    ]]);
}

function getAdminsKeys(function getAdminsKeys(){
    global $connection, $mainValues, $buttonValues;
    $keys = array();
...
// ==================================================
// Broadcast starter (main + reseller bots)
// - Uses shell_exec if available
// - Falls back to async HTTP trigger (works even when shell_exec is disabled)
// ==================================================
if(!function_exists('startBroadcastWorker')){
    function startBroadcastWorker(int $bid = 0): void {
        $worker = __DIR__ . '/broadcast_worker.php';

        // 1) Try CLI background execution (fastest)
        $disabled = strtolower((string)ini_get('disable_functions'));
        $shellOk = function_exists('shell_exec') && ($disabled === '' || (strpos($disabled, 'shell_exec') === false));
        if($shellOk && file_exists($worker)){
            $cmd = 'nohup php ' . escapeshellarg($worker) . ' ' . escapeshellarg((string)$bid) . ' >/dev/null 2>&1 &';
            @shell_exec($cmd);
            return;
        }

        // 2) Fallback: async HTTP trigger to run the worker
        triggerBroadcastWorkerHttp($bid);
    }
}

if(!function_exists('triggerBroadcastWorkerHttp')){
    function triggerBroadcastWorkerHttp(int $bid = 0): void {
        global $botUrl, $botToken, $admin;

        $token = $GLOBALS['botToken'] ?? $botToken;
        $adm   = $GLOBALS['admin'] ?? $admin;

        $key = hash('sha256', $token . '|' . $adm . '|broadcast');
        $base = rtrim((string)$botUrl, '/') . '/';
        $url  = $base . 'broadcast_trigger.php?bid=' . (int)$bid . '&key=' . $key;
        ...
        $fp = @fsockopen($target, $port, $errno, $errstr, 1.5);
        if(!$fp) return;
...
// ==================================================
// Safe SQL helpers (avoid blank pages when some tables do not exist in reseller DB)
// ==================================================
if(!function_exists('_tableExists')){
    function _tableExists(mysqli $conn, string $table): bool {
        $table = trim($table);
...
if(!function_exists('_safeCountRows')){
    function _safeCountRows(mysqli $conn, string $table, string $whereSql = ''): int {
        if(!_tableExists($conn, $table)) return 0;
        $sql = "SELECT COUNT(*) as c FROM `$table`" . ($whereSql ? (" WHERE $whereSql") : '');
...
if(!function_exists('_safeSum')){
    function _safeSum(mysqli $conn, string $table, string $col, string $whereSql = '', array $bind = []): float {
        if(!_tableExists($conn, $table)) return 0;
        $sql = "SELECT SUM($col) as s FROM `$table`" . ($whereSql ? (" WHERE $whereSql") : '');
        $stmt = $conn->prepare($sql);
        if(!$stmt) return 0;
        if($bind){
            // bind is like ['i', $val]
            $types = array_shift($bind);
