...
function xuiFinalizeCreateResponse($server_id, $client_id, $inbound_id, $remark, $rawResponse, $generatedSubId = ''){
    ...
    if($decoded && isset($decoded->success) && $decoded->success){
        if(empty($decoded->sub_link)){
            foreach(['subLink', 'subscription_url', 'subscriptionUrl', 'subscriptionURL'] as $altKey){
                if(isset($decoded->$altKey) && trim((string)$decoded->$altKey) !== ''){
                    $decoded->sub_link = trim((string)$decoded->$altKey);
                    break;
                }
            }
        }
        if(empty($decoded->sub_id) && $generatedSubId !== '') $decoded->sub_id = $generatedSubId;
        if(empty($decoded->sub_link)){
            if($generatedSubId !== ''){
                $found = xuiFindClientRecord($server_id, $client_id, $inbound_id, $remark);
    ...
    if($found){
        return (object) [
            ...
            'sub_id' => $found->subId ?? $generatedSubId,
            'obj' => $found
        ];
    }
...
 /");
    ...
    $extractWsHeaderHost = function($wsSettingsObj, $wsSettingsAssoc, $requestHeaderRaw = '', $streamSettingsRaw = '') use ($normalizeHostValue) {
        $tryHost = function($candidate) use ($normalizeHostValue) {
            $host = $normalizeHostValue($candidate);
            return $host !== '' ? $host : '';
        };

        $searchValue = function($node) use (&$searchValue, $tryHost) {
            ...
            foreach($node as $key => $value){
                ...
                if(in_array($keyLower, ['host', 'authority', ':authority'], true)){
                    $host = $tryHost($value);
                    if($host !== '') return $host;
                }
                if(in_array($keyLower, ['headers', 'header', 'requestheaders', 'requestheader'], true)){
                    $host = $searchValue($value);
                    if($host !== '') return $host;
                }
                if(is_array($value) || is_object($value)){
                    $item = is_object($value) ? get_object_vars($value) : $value;
                    $name = strtolower((string)($item['name'] ?? $item['key'] ?? ''));
                    if($name === 'host' || $name === 'authority'){
                        $host = $tryHost($item['value'] ?? ($item['val'] ?? ($item['host'] ?? ($item['authority'] ?? ''))));
                        if($host !== '') return $host;
                    }
                    $host = $searchValue($value);
                    if($host !== '') return $host;
        ...
        foreach([$wsSettingsObj, $wsSettingsAssoc] as $bag){
            $host = $searchValue($bag);
            if($host !== '') return $host;
        }

        $rawCandidates = [(string)$requestHeaderRaw, (string)$streamSettingsRaw, json_encode($wsSettingsAssoc, 448) ?: ''];
        foreach($rawCandidates as $raw){
            if($raw === '') continue;
            if(preg_match('/^Host\s*:\s*(.+)$/im', $raw, $match)){
                $host = $tryHost($match[1]);
                if($host !== '') return $host;
            }
            foreach([
                '/"(?:host|Host|authority|Authority|:authority)"\s*:\s*"([^"\\]+)"/i',
                '/"(?:host|Host|authority|Authority|:authority)"\s*:\s*\[\s*"([^"\\]+)"/i',
                '/"name"\s*:\s*"(?:Host|host|authority|Authority)"\s*,\s*"value"\s*:\s*"([^"\\]+)"/i',
                '/"key"\s*:\s*"(?:Host|host|authority|Authority)"\s*,\s*"value"\s*:\s*"([^"\\]+)"/i'
            ] as $pattern){
    ...
    $extractWsExternalTargets = function($wsSettingsAssoc, $streamSettingsRaw = '') use ($normalizeHostValue) {
        $targets = [];
        ...
        $appendTarget = function($host, $port) use (&$targets, $normalizeHostValue) {
            ...
            if(!isset($targets[$key])) $targets[$key] = ['host' => $host, 'port' => $port];
        };

        $parseHostPortString = function($value) use (&$appendTarget) {
            $value = trim((string)$value);
            if($value === '') return;
            if(preg_match('/^([^\s,;|]+)\s*:\s*(\d{2,5})$/', $value, $match)){
                $appendTarget($match[1], $match[2]);
                return;
            }
            if(preg_match_all('/([a-z0-9._-]+)\s*:\s*(\d{2,5})/i', $value, $matches, PREG_SET_ORDER)){
                foreach($matches as $match){
                    if(strpos($match[1], '.') !== false) $appendTarget($match[1], $match[2]);
                }
            }
        };

        $parseNode = function($node, $proxyContext = false) use (&$parseNode, &$appendTarget, &$parseHostPortString) {
            if(is_scalar($node) || $node === null){
                if($proxyContext) $parseHostPortString($node);
                return;
            }
            ...
            $pairs = [
                ...
                ['dest','port'], ['destination','port'], ['target','port'], ['targethost','targetport'],
                ['remotehost','remoteport'], ['ip','port'], ['h','p'], ['server','serverport'],
                ['host','serverport'], ['address','serverport'], ['domain','serverport'], ['addr','serverport']
            ];
            foreach($pairs as $pair){
                if(isset($lower[$pair[0]]) && isset($lower[$pair[1]])) $appendTarget($lower[$pair[0]], $lower[$pair[1]]);
            }

            foreach(['dest', 'destination', 'target', 'address', 'addr', 'server', 'host', 'domain'] as $key){
                if(isset($lower[$key]) && !isset($lower['port']) && !isset($lower['serverport']) && !isset($lower['targetport']) && !isset($lower['destport'])){
                    $parseHostPortString($lower[$key]);
                }
            }

            $vals = array_values($node);
            if(count($vals) >= 2){
                if(!is_array($vals[0]) && !is_object($vals[0]) && !is_numeric($vals[0]) && is_numeric($vals[1])) $appendTarget($vals[0], $vals[1]);
                if(count($vals) >= 3 && !is_array($vals[1]) && !is_object($vals[1]) && !is_numeric($vals[1]) && is_numeric($vals[2])) $appendTarget($vals[1], $vals[2]);
            }

            foreach($node as $key => $value){
                $keyLower = strtolower((string)$key);
                $childProxyContext = $proxyContext || strpos($keyLower, 'external') !== false || strpos($keyLower, 'proxy') !== false || in_array($keyLower, ['dest','destination','target'], true);
                if($childProxyContext && is_scalar($value)) $parseHostPortString($value);
                $parseNode($value, $childProxyContext);
            }
        };

        if(is_array($wsSettingsAssoc)) $parseNode($wsSettingsAssoc, true);

        if($streamSettingsRaw !== ''){
            $decoded = json_decode((string)$streamSettingsRaw, true);
            if(json_last_error() === JSON_ERROR_NONE && is_array($decoded)) $parseNode($decoded, true);
        }
