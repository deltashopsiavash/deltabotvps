...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if($configButtonEnabled && !empty($configLinks)){
        // Copy all config links directly from the same button, one link per line.
        // Do not use callback_data here, because callback sends a separate message instead of copying.
        $rows[] = [[
            'text' => '📋 کپی کانفیگ',
