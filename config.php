...
 /");
    ...
    $extractWsHeaderHost = function($wsSettingsObj, $wsSettingsAssoc, $requestHeaderRaw = '') use ($normalizeHostValue) {
        $bags = [];
        ...
        foreach($bags as $bag){
            ...
            foreach($bag as $key => $value){
                ...
                if(is_array($value) || is_object($value)){
                    ...
                    if($name === 'host'){
                        $host = $normalizeHostValue($item['value'] ?? ($item['val'] ?? ($item['host'] ?? '')));
                        if($host !== '') return $host;
                    }
                }
            }
    ...
    $extractWsExternalTargets = function($node) use ($normalizeHostValue) {
        $targets = [];
        ...
        $walk = function($value) use (&$walk, &$appendTarget, $normalizeHostValue) {
            if(is_string($value)){
                $trimmed = trim($value);
                if($trimmed === '') return;
            ...
            if($isList){
                if(count($value) >= 3){
                    $host = $normalizeHostValue($value[1] ?? '');
                    $port = $value[2] ?? 0;
            ...
            foreach($value as $key => $child){
                $lookup[strtolower((string)$key)] = $child;
            }
        ...
        if($inbound_id == 0){
            ...
            if($clients[0]->id == $uniqid || $clients[0]->password == $uniqid) {
                ...
                if($netType == 'ws') {
                    ...
                    $host = $extractWsHeaderHost($wsSettingsObj, $wsSettingsAssoc, $request_header);
                    $wsExternalTargets = $extractWsExternalTargets($wsSettingsAssoc);
                }
