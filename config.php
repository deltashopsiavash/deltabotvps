...
function sendPhoto($photo, $caption = null, $keyboard = null, $parse = "MarkDown", $ci =null){
    ...
    if($res && !empty($res->ok) && isset($res->result->message_id) && is_string($caption) && mb_strpos($caption, '😍 سفارش جدید شما') !== false){
        @bot('pinChatMessage',[
            'chat_id'=>$ci,
...
function getBotSettingKeys(){
    ...
    $dailySalesReport = (($botState['dailySalesReportState'] ?? 'off')=='on')?$buttonValues['on']:$buttonValues['off'];
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=>$dailySalesReport,'callback_data'=>'changeBotdailySalesReportState'],
            ['text'=>'گزارش فروش 24 ساعته','callback_data'=>'deltach']
        ],
        [
