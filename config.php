...
function forwardUpdateToVpsbot($raw){
    ...
    // IMPORTANT: On first call VPSBot runtime may take a few seconds to initialize.
    // If we timeout too aggressively, the request may get cancelled and the update
    // won't be processed until the next user action.
    ...
    $resp = curl_exec($ch);
    if($resp === false){
        // Don't break mother bot flow if bridge is down; just return error string.
        $err = curl_error($ch);
        curl_close($ch);
