...
function xuiBuildConfigBlockHtml($botState, $serverType, $configLinks){
    ...
    // One single code block: in Telegram the user can tap/hold the block and copy all links together.
    return "💝 config :
<code>" . implode("
", $safeLinks) . "</code>";
}
...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if($configButtonEnabled && !empty($configLinks)){
        $allConfigText = implode("
", $configLinks);
        // copy_text copies all config links in one tap. If Telegram rejects an oversized
        // button, delivery code falls back to a normal callback button.
        $rows[] = [[
            'text' => '📋 کپی کانفیگ',
            'copy_text' => ['text' => $allConfigText]
        ]];
    }
