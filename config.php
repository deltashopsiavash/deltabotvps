...
// ---------------- Cloud Server (Hetzner) module ----------------
// baseInfo.php باید این مقدار را تعریف کند (نمونه: $hcloudToken = "...";)

function ensureCloudTables(){
    global $connection;

    $connection->query("CREATE TABLE IF NOT EXISTS `cloud_plans` (
        `id` INT NOT NULL AUTO_INCREMENT,
        `location` VARCHAR(32) NOT NULL,
        `country` VARCHAR(8) NULL,
        `server_type` VARCHAR(64) NOT NULL,
        `title` VARCHAR(120) NOT NULL,
        `price_month_irt` BIGINT NOT NULL DEFAULT 0,
        `price_hour_irt` BIGINT NOT NULL DEFAULT 0,
    ...
    $connection->query("CREATE TABLE IF NOT EXISTS `cloud_orders` (
        `id` INT NOT NULL AUTO_INCREMENT,
        `user_id` BIGINT NOT NULL,
        `method` ENUM('hetzner','manual') NOT NULL DEFAULT 'hetzner',
        `status` VARCHAR(32) NOT NULL DEFAULT 'pending',
        `billing` ENUM('monthly','hourly') NOT NULL DEFAULT 'monthly',
        `amount_irt` BIGINT NOT NULL DEFAULT 0,
        `hostname` VARCHAR(64) NULL,
        `location` VARCHAR(32) NULL,
        `server_type` VARCHAR(64) NULL,
        `image_id` BIGINT NULL,
        `hcloud_server_id` BIGINT NULL,
        `ip` VARCHAR(64) NULL,
        `root_password` VARCHAR(128) NULL,
        `traffic_limit_gb` INT NOT NULL DEFAULT 0,
        `traffic_used_gb` INT NOT NULL DEFAULT 0,
        `expires_at` INT NOT NULL DEFAULT 0,
        `created_at` INT NOT NULL DEFAULT 0,
        PRIMARY KEY (`id`),
        KEY `user_id` (`user_id`),
        KEY `status` (`status`),
        KEY `hcloud_server_id` (`hcloud_server_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");

    $connection->query("CREATE TABLE IF NOT EXISTS `cloud_invoices` (
        `id` INT NOT NULL AUTO_INCREMENT,
        `user_id` BIGINT NOT NULL,
        `order_id` INT NULL,
        `amount_irt` BIGINT NOT NULL DEFAULT 0,
        `pay_method` ENUM('wallet','card') NOT NULL DEFAULT 'card',
        `state` ENUM('pending','paid','declined') NOT NULL DEFAULT 'pending',
...
function hcloudRequest($method, $path, $payload = null){
    $token = $GLOBALS['hcloudToken'] ?? null;
    if(empty($token)){
        return ['ok'=>false,'error'=>'HCLOUD_TOKEN_NOT_SET'];
    }
    ...
    if($code < 200 || $code >= 300){
        return ['ok'=>false,'error'=>'HTTP_'.$code,'detail'=>$json ?: $res];
    }
    return ['ok'=>true,'data'=>$json];
}

function cloudSanitizeHostname($h){
    $h = strtolower(trim((string)$h));
    $h = preg_replace('/[^a-z0-9-]/','-', $h);
    $h = preg_replace('/-+/', '-', $h);
    $h = trim($h, '-');
    if(strlen($h) < 3) $h = 'server-' . rand(100,999);
    if(strlen($h) > 60) $h = substr($h,0,60);
    return $h;
}

function cloudFmtMoney($irt){
    return number_format((float)$irt) . ' تومان';
}
...
function getMainKeys(){
    ...
    // Cloud server
    $mainKeys = array_merge($mainKeys,[
        [['text'=>$buttonValues['buy_cloud_server'],'callback_data'=>"buyCloudServer"]],
    ]);
...
function getAdminKeys(){
    ...
    // Cloud server admin
    $rows[] = [
        ['text'=>$buttonValues['cloud_server_admin'],'callback_data'=>"cloudServerAdmin"],
        ['text'=>' ','callback_data'=>"deltach"],
    ];
