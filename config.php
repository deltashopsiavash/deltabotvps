...
if(!function_exists('deltaPrettyMainButton')){
    /**
     * Telegram inline keyboards do not support real background colors.
     * This function gives the main menu the same colorful/clean look by
     * styling the visible labels with distinct emojis. It is applied at
     * render time, so it also works for reseller/child bots that use this
     * shared bot.php?bid=... code path.
     */
    function deltaPrettyMainButton($btn){
        if(!is_array($btn)) return $btn;
        $cb = (string)($btn['callback_data'] ?? '');
        $pretty = [
            'buySubscription'    => '🛒 خرید سرویس جدید',
            'mySubscriptions'    => '💼 سرویس های من',
            'agentConfigsList'   => '💼 سرویس های من',
            'increaseMyWallet'   => '👛 کیف پول',
            'myInfo'             => '👥 حساب کاربری',
            'showUUIDLeft'       => '⏳ وضعیت سرویس',
            'getTestAccount'     => '🎁 سرویس تست رایگان',
            'inviteFriends'      => '🏆 زیر مجموعه گیری',
            'requestAgency'      => '🤝 درخواست نمایندگی',
            'agencySettings'     => '➰ پنل همکاری',
            'agentOneBuy'        => '➕ خرید تکی',
            'agentMuchBuy'       => '♾ خرید انبوه',
            'resellerShop'       => '🤖 خرید ربات نمایندگی',
            'myResellerBots'     => '🤖 ربات های من',
            'availableServers'   => '📦 موجودی اشتراکی',
            'availableServers2'  => '💎 موجودی اختصاصی',
            'vpsbotStart'        => '☁️ خرید سرور ابری',
            'managePanel'        => '⚙️ مدیریت ربات',
        ];
        if(isset($pretty[$cb])) $btn['text'] = $pretty[$cb];
        return $btn;
    }
}
...
if(count($menuOrder) > 0){
    ...
    if($from_id == $admin || $userInfo['isAdmin'] == true){
        $mainKeys[] = [['text'=>"⚙️ مدیریت ربات",'callback_data'=>"managePanel"]];
    }

    // Apply colorful/pretty main-menu labels for all users and reseller bots.
    foreach($mainKeys as &$__deltaRow){
        if(!is_array($__deltaRow)) continue;
        foreach($__deltaRow as &$__deltaBtn){
            $__deltaBtn = deltaPrettyMainButton($__deltaBtn);
        }
        unset($__deltaBtn);
    }
    unset($__deltaRow);

    return json_encode(['inline_keyboard'=>$mainKeys], JSON_UNESCAPED_UNICODE);
}
