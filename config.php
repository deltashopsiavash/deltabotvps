...
function getUserBoughtVolumeTotal($userId){
    ...
    // Also count paid volume-upgrade purchases from pays history
    $stmt = $connection->prepare("SELECT `type`,`volume` FROM `pays` WHERE `user_id`=? AND `state` IN ('approved','paid_with_wallet','paid') AND `type` LIKE 'INCREASE_VOLUME%'");
    if($stmt){
        $stmt->bind_param('i', $userId);
        $stmt->execute();
        $res = $stmt->get_result();
...
function pgRenewPaymentKeyboard($hash, $price){
    global $botState,$buttonValues,$botUrl;
