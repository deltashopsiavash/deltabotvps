...
function xuiExtractOrderSubLink($order){
    ...
    if($serverType === 'pasarguard' && $remark !== ''){
        ...
        foreach(['subscription_url','sub_link','subscription','sub'] as $k){
            if(is_object($info) && !empty($info->{$k})) $candidates[] = $info->{$k};
        }
    ...
    foreach($candidates as $candidate){
        ...
        // Avoid broken old values like https://domain:2053TOKEN; prefer real /sub/ URLs or plain tokens.
        if(preg_match('#^https?://#i', $candidate) && strpos($candidate, '/sub/') === false && !preg_match('#/api/#i', $candidate)){
            $parts = parse_url($candidate);
            if(!empty($parts['path']) && preg_match('#^/sub/#', $parts['path'])){
                return xuiBuildPanelSubLink($base, $candidate, $base);
            }
            continue;
        }
