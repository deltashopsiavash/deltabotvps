...
function getUserBoughtVolumeTotal($userId){
    ...
    $sql = "SELECT `type`,`plan_id`,`volume`,`agent_count` FROM `pays` WHERE `user_id`=? AND `state` IN ('approved','paid_with_wallet')";
        ...
        while($row = $res->fetch_assoc()){
            ...
            if($type === 'BUY_SUB'){
                if($vol > 0){
                    ...
                    $stmtPlan = $connection->prepare("SELECT `volume` FROM `server_plans` WHERE `id`=? LIMIT 1");
                    if($stmtPlan){
                        ...
                        if($planRes && $planRes->num_rows > 0){
                            $planVol = (float)($planRes->fetch_assoc()['volume'] ?? 0);
                        }
                        $total += ($planVol * $count);
                    }
                }
            }elseif(stripos($type, 'INCREASE_VOLUME') !== false){
                if($vol > 0) $total += $vol;
            }
        }
    }
...
function getUserRemainingBuyVolume($userId){
    ...
    $used = getUserBoughtVolumeTotal((int)$userId);
    $remain = $limit - $used;
...
function userCanBuyVolume($userId, $requestedVolume, &$errorMessage=null){
    ...
    $used = getUserBoughtVolumeTotal((int)$userId);
    $remain = $limit - $used;
    if($remain < $requestedVolume){
        $errorMessage = "⛔️ شما دیگر نمی‌توانید خرید انجام دهید.

سقف خرید شما: <code>{$limit}</code> گیگ
مقدار خریدشده تا الان: <code>{$used}</code> گیگ
باقیمانده: <code>" . max(0,$remain) . "</code> گیگ

برای ادامه، باید محدودیت خرید شما افزایش داده شود.";
        return false;
    }
    return true;
}
...
function getAdminKeys(){
    ...
    $rows[] = [
        ['text'=>'📦 سهمیه‌دارها','callback_data'=>'adminQuotaUsers0'],
...
function getAdminQuotaUsersKeys($offset=0){
    global $connection;
    $offset = (int)$offset;
    if($offset < 0) $offset = 0;
    $limit = 20;

    $stmt = $connection->prepare("SELECT `type`,`value` FROM `setting` WHERE `type` LIKE 'USER_BUY_VOLUME_LIMIT_%' AND `value` IS NOT NULL AND `value` != '' ORDER BY `id` DESC");
    $stmt->execute();
    $res = $stmt->get_result();
    $stmt->close();
    ...
    while($r = $res->fetch_assoc()){
        $type = (string)($r['type'] ?? '');
        if(strpos($type, 'USER_BUY_VOLUME_LIMIT_') !== 0) continue;
        $uid = (int)substr($type, strlen('USER_BUY_VOLUME_LIMIT_'));
        if($uid <= 0) continue;
        $limitValue = trim((string)($r['value'] ?? ''));
        if($limitValue === '' || !is_numeric($limitValue)) continue;
        $limitValue = max(0, (int)$limitValue);
        $all[] = ['uid'=>$uid, 'limit'=>$limitValue];
    }
    ...
    foreach($slice as $it){
        ...
        $used = getUserBoughtVolumeTotal($uid);
        $remain = getUserRemainingBuyVolume($uid);
        $remainText = ($remain === null ? 'عادی' : ($remain . 'گ مانده'));
        $rows[] = [[
            'text'=>"👤 {$name} ({$uid}) | {$used}گ خرید | {$remainText}",
            'callback_data'=>"adminQuotaUser_{$uid}_{$offset}"
        ]];
    }
    if($nameStmt) $nameStmt->close();
    ...
    if($offset > 0){
        $prev = max(0, $offset-$limit);
        $nav[] = ['text'=>"⬅️ قبلی",'callback_data'=>"adminQuotaUsers{$prev}"];
    }
    if(($offset + $limit) < $total){
        $next = $offset + $limit;
        $nav[] = ['text'=>"بعدی ➡️",'callback_data'=>"adminQuotaUsers{$next}"];
    }
    if(count($nav)>0) $rows[] = $nav;
...
if($normal > 100) $normal = 100;
    ...
    $boughtVolume = getUserBoughtVolumeTotal((int)$uid);
...
$lines = [];
    ...
    $lines[] = "📦 حجم خریدشده: <code>{$boughtVolume} گیگ</code>";
...
function getUserInfoKeys($userId, $backCallback = "managePanel"){
    ...
    if($userCount->num_rows > 0){
        ...
        $boughtVolume = getUserBoughtVolumeTotal($userId);
        ...
        return json_encode(['inline_keyboard'=>[
            ...
            [
                ...
                ['text'=>"حجم خریدشده",'callback_data'=>"deltach"]
                ],
            [
            ...
            [
                ['text'=>"➕ افزایش سهمیه",'callback_data'=>"uBuyQuotaInc" . $userId],
                ['text'=>"➖ کاهش سهمیه",'callback_data'=>"uBuyQuotaDec" . $userId]
                ],
            [
                ['text'=>"♻️ صفر کردن سهمیه",'callback_data'=>"uBuyQuotaZero" . $userId],
                ['text'=>"🔄 حالت عادی سهمیه",'callback_data'=>"uBuyQuotaNormal" . $userId]
                ],
            [
