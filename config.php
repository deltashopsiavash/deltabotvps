...
function getBotSettingKeys(){
    ...
    // Hide cloud VPS toggle inside reseller bots
    if(!empty($isChildBot)){
        $rows = array_values(array_filter($rows, function($r){
            return !(is_array($r) && isset($r[0]['callback_data']) && $r[0]['callback_data']==='changeBotcloudVpsPanelState');
        }));
    }

    return json_encode(['inline_keyboard'=>normalizeKeyboard($rows)]);
tion getBotSettingKeys(){
    global $connection, $mainValues, $buttonValues;
