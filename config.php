...
function xuiSendOrderDeliveryPhoto($chatId, $protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = '', $backCallback = 'mainMenu'){
    ...
    if((!is_object($res) || empty($res->ok)) && trim((string)$caption) !== ''){
        // Telegram may reject fetching the temporary QR by URL on some hosts.
        // Always deliver the subscription/config text as a safe fallback instead of leaving the user empty-handed.
...
function xuiGetOrderConfigLinks($order){
    $storedLinks = xuiNormalizeConfigLinks(json_decode($order['link'] ?? '', true));
    $liveLinks = [];

    // Always try to rebuild from the panel when possible, because External Proxy
    // rows may be changed/added after the order was created and old stored links
    // can contain only the first route.
    $serverId = (int)($order['server_id'] ?? 0);
    $uuid = trim((string)($order['uuid'] ?? ''));
    $protocol = trim((string)($order['protocol'] ?? ''));
    if($serverId > 0 && $uuid !== '' && $protocol !== ''){
        $inboundId = xuiNormalizeInboundId($order['inbound_id'] ?? 0);
        $remark = (string)($order['remark'] ?? '');
        $rahgozar = !empty($order['rahgozar']);
        try{
            $rebuilt = getConnectionLink($serverId, $uuid, $protocol, $remark, 0, '', $inboundId, $rahgozar);
            $liveLinks = xuiNormalizeConfigLinks($rebuilt);
        }catch(Throwable $e){
            $liveLinks = [];
        }
    }

    return xuiNormalizeConfigLinks(array_merge($liveLinks, $storedLinks));
}
    ...
    $extractWsExternalTargets = function($node, $streamSettingsRaw = '') use ($normalizeHostValue) {
        ...
        $parseExternalListItem = function($item) use (&$appendTarget, $normalizeHostValue) {
            ...
            foreach([
                ...
                ['externalproxyhost','externalproxyport'], ['externalproxy','port'], ['externalhost','externalport'],
                ['proxyhost','proxyport'], ['proxyaddress','proxyport'], ['proxyserver','proxyport'],
                ['proxyhost','port'], ['proxyaddress','port'], ['proxyserver','port'], ['targetaddress','targetport']
            ] as $pair){
            ...
            // Newer 3x-ui builds/plugins may store External Proxy rows with
            // non-standard key names. If an item has a numeric port, pick the
            // first domain/IP-looking string in the same row and use it as the
            // external proxy address. This makes the bot deliver ALL rows like
            // snapp.ir:8080, tamin.ir:8080, irna.ir:8080 instead of only one link.
            $fallbackPort = 0;
            foreach($lookup as $lk => $lv){
                if(strpos($lk, 'port') !== false && is_numeric($lv)){
                    $fallbackPort = (int)$lv;
                    break;
                }
            }
            if($fallbackPort > 0){
                foreach($lookup as $lk => $lv){
                    if(is_array($lv) || is_object($lv)) continue;
                    $candidate = $normalizeHostValue($lv);
                    if($candidate === '') continue;
                    $lowerCandidate = strtolower($candidate);
                    if(in_array($lowerCandidate, ['same','tls','none','true','false','remark',''], true)) continue;
                    if(is_numeric($candidate)) continue;
                    if(preg_match('/^[a-z0-9._-]+$/i', $candidate) && (strpos($candidate, '.') !== false || filter_var($candidate, FILTER_VALIDATE_IP))){
                        $appendTarget($candidate, $fallbackPort);
                        return;
                    }
                }
            }
