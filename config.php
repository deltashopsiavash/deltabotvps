...
function xuiBuildOrderDeliveryLinksBlock($botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = ''){
    $configLinks = xuiNormalizeConfigLinks($configLinks);
    $subLink = trim((string)$subLink);
    $linkBlock = '';
...
function xuiBuildOrderDeliveryLinksMessage($botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = ''){
    $linkBlock = xuiBuildOrderDeliveryLinksBlock($botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink);
    if(trim($linkBlock) === '') return '';
    return "<blockquote>❞ <b>لینک های اتصال</b></blockquote>" . $linkBlock . "\n\nبا لمس کردن کادر لینک، گزینه Copy Code برای کپی راحت نمایش داده میشود.";
}
function xuiBuildOrderDeliveryText($protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = '', $includeLinks = true){
...
function xuiSendOrderDeliveryPhoto($chatId, $protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = '', $backCallback = 'mainMenu'){
    ...
    // In Telegram media captions, <pre><code> blocks often do not show the native Copy Code button.
    // So when QR/photo is sent, keep the caption clean and send links in a separate text message
    // where Telegram clients show copy affordances much more reliably.
    $caption = xuiBuildOrderDeliveryText($protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink, false);
    $linksMessage = xuiBuildOrderDeliveryLinksMessage($botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink);
    $keyboard = xuiBuildOrderCopyButtons($configLinks, $subLink, $backCallback, $uniqid);
    if(trim((string)$primaryPayload) === ''){
        $fullText = xuiBuildOrderDeliveryText($protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink, true);
        return sendMessage($fullText, $keyboard, 'HTML', $chatId);
    }
    ...
    if((!is_object($res) || empty($res->ok)) && trim((string)$caption) !== ''){
        ...
        $fullText = xuiBuildOrderDeliveryText($protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink, true);
        $res = sendMessage($fullText, $keyboard, 'HTML', $chatId);
        if(!is_object($res) || empty($res->ok)){
            $res = sendMessage($fullText, null, 'HTML', $chatId);
        }
    } else {
        if(trim((string)$linksMessage) !== ''){
            sendMessage($linksMessage, $keyboard, 'HTML', $chatId);
        }
    }
...
function pasarguardFetchDefaultGroupIds($server_id, $token){
    ...
    $server_info = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if(!$server_info || empty($token->access_token)) return [];
    ...
    foreach($paths as $path){
        ...
        $items = $json;
        foreach(['items','results','data','groups'] as $k){ if(isset($json[$k]) && is_array($json[$k])) { $items = $json[$k]; break; } }
        $ids=[];
        foreach($items as $g){
            if(is_array($g) && isset($g['id'])) $ids[]=(int)$g['id'];
            elseif(is_object($g) && isset($g->id)) $ids[]=(int)$g->id;
        }
        $ids = array_values(array_unique(array_filter($ids)));
