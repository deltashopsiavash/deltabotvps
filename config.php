...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if($configButtonEnabled && !empty($configLinks)){
        // Direct Telegram copy_text has a short text limit and may fail/truncate with several links.
        // Use callback when we have a lookup key, then send all fresh panel links in one copyable message.
        if($lookupKey !== ''){
            $rows[] = [[
                'text' => '📋 کپی کانفیگ',
