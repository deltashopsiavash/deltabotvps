...
function telegramPayloadHasFile($datas = []){
    foreach((array)$datas as $value){
        if($value instanceof CURLFile) return true;
    }
    return false;
}
function telegramResolveLocalPathFromUrl($url){
    $url = trim((string)$url);
    if($url === '') return null;
    $parts = parse_url($url);
...
function botWithToken($token, $method, $datas = []){
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, telegramPayloadHasFile($datas) ? $datas : http_build_query($datas));
    $res = curl_exec($ch);
    curl_close($ch);
...
function sendPhoto($photo, $caption = null, $keyboard = null, $parse = "MarkDown", $ci =null){
    ...
    if(is_string($photo)){
        $localPath = null;
        if(is_file($photo)) $localPath = $photo;
        elseif(preg_match('#^https?://#i', $photo)) $localPath = telegramResolveLocalPathFromUrl($photo);
        if($localPath && is_file($localPath)) $payload['photo'] = new CURLFile(realpath($localPath));
    }
    return bot('sendPhoto', $payload);
}
...
function xuiExtractConfigLinksFromText($raw){
    $raw = trim((string)$raw);
    if($raw === '' || stripos($raw, '<html') !== false) return [];

    $candidate = $raw;
    if(strpos($candidate, '://') === false){
        $compact = preg_replace('/\s+/', '', $candidate);
        if($compact !== '' && strlen($compact) % 4 === 0){
            $decoded = base64_decode($compact, true);
            if($decoded !== false && strpos($decoded, '://') !== false) $candidate = $decoded;
        }
    }
...
function xuiFetchConfigLinksFromSubscription($subLink){
    $subLink = trim((string)$subLink);
    if($subLink === '') return [];
    $curl = curl_init();
    curl_setopt_array($curl, [
        ...
        CURLOPT_HTTPHEADER => ['User-Agent: Mozilla/5.0', 'Accept: */*']
    ]);
    $raw = curl_exec($curl);
    curl_close($curl);
    if($raw === false || $raw === null) return [];
    return xuiExtractConfigLinksFromText($raw);
}
function xuiGetPanelClientLinks($server_id, $client_id = '', $inbound_id = null, $remark = ''){
    $found = xuiFindClientRecord($server_id, $client_id, $inbound_id, $remark);
    if(!$found || empty($found->sub_link)) return [];
    return xuiFetchConfigLinksFromSubscription($found->sub_link);
}
...
function getConnectionLink($server_id, $uniqid, $protocol, $remark, $port, $netType, $inbound_id = 0, $rahgozar = false, $customPath = false, $customPort = 0, $customSni = null){
    $panelLinks = xuiGetPanelClientLinks($server_id, $uniqid, $inbound_id, $remark);
        ...
        if($inbound_id == 0){
            ...
            if($clients[0]->id == $uniqid || $clients[0]->password == $uniqid) {
                ...
                if($netType == 'ws') {
                    ...
                    $host = xuiDecodeField($row->streamSettings)->wsSettings->headers->Host;
                }
