...
function xuiBuildPanelSubLink($panelUrl, $subId){
    $panelUrl = rtrim((string)$panelUrl, '/');
    $subId = trim((string)$subId);
    if($subId === '') return '';
    if(preg_match('#^https?://#i', $subId)) return $subId;
    if(strpos($subId, '/sub/') === 0 || strpos($subId, '/json/') === 0) return $panelUrl . $subId;
        ...
        foreach($settings->clients as $client){
            ...
            if(($client_id !== '' && (string)$candidateId === (string)$client_id) || ($remark !== '' && (string)$candidateEmail === (string)$remark)){
                ...
                return (object) [
                    ...
                    'client' => $client,
                    'subId' => $subId,
                    'sub_link' => xuiBuildPanelSubLink($server_info['panel_url'], $subId),
    ...
    if($decoded && isset($decoded->success) && $decoded->success){
        if(empty($decoded->sub_link)){
            if($generatedSubId !== ''){
                ...
                if(empty($decoded->sub_link)){
                    $stmt = $GLOBALS['connection']->prepare("SELECT panel_url FROM server_config WHERE id=?");
                    $stmt->bind_param("i", $server_id);
                    $stmt->execute();
                    $panelRow = $stmt->get_result()->fetch_assoc();
                    $stmt->close();
                    $decoded->sub_link = xuiBuildPanelSubLink($panelRow['panel_url'] ?? '', $generatedSubId);
                }
            } else {
