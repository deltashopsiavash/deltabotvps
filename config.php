...
function getConnectionLink($server_id, $uniqid, $protocol, $remark, $port, $netType, $inbound_id = 0, $rahgozar = false, $customPath = false, $customPort = 0, $customSni = null){
    ...
    foreach($response as $row){
        if($inbound_id == 0){
            ...
            foreach($clients as $clientItem){
                if(($clientItem->id ?? null) == $uniqid || ($clientItem->password ?? null) == $uniqid){
                    $matchedClient = $clientItem;
                    break;
                }
            }
            if($matchedClient) {
                if(!empty($matchedClient->subId)){
                    $exactSubLink = xuiBuildPanelSubLink($server_info['panel_url'], $matchedClient->subId, $server_info['sub_base_url'] ?? '');
                }
                if($serverType == "sanaei" || $serverType == "alireza"){
                    $settings = xuiDecodeAssoc($row->settings);
            ...
            if($row->id == $inbound_id) {
                $settingsObj = xuiDecodeField($row->settings);
                $clientsObj = $settingsObj->clients ?? [];
                foreach($clientsObj as $clientItem){
                    if(($clientItem->id ?? null) == $uniqid || ($clientItem->password ?? null) == $uniqid){
                        if(!empty($clientItem->subId)){
                            $exactSubLink = xuiBuildPanelSubLink($server_info['panel_url'], $clientItem->subId, $server_info['sub_base_url'] ?? '');
                        }
                        break;
                    }
                }
                    ...
                    foreach($clients as $key => $client){
                        if($client->id == $uniqid || $client->password == $uniqid){
                            ...
                            if(!empty($client->subId)){
                                $exactSubLink = xuiBuildPanelSubLink($server_info['panel_url'], $client->subId, $server_info['sub_base_url'] ?? '');
                            }
                            break;
                        }
                    }
    ...
    if($exactSubLink !== ''){
        $exactLinks = xuiFetchConfigLinksFromSubscription($exactSubLink);
        if(!empty($exactLinks)) return $exactLinks;
    }
