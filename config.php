...
if(!function_exists('deltaInlineButtonStyle')){
    function deltaInlineButtonStyle($btn){
        ...
        // Delivery subscription copy button
        if(isset($btn['copy_text']) || mb_strpos($text, 'کپی لینک اشتراک') !== false || mb_strpos($text, 'کپی سابسکریپشن') !== false){
            return 'success'; // green
        }

        // Navigation / destructive style
        if(mb_strpos($text, 'بازگشت') !== false || mb_strpos($text, 'لغو') !== false || mb_strpos($text, 'حذف') !== false || mb_strpos($text, 'رد') !== false){
            return 'danger'; // red
        }

        // Other useful buttons
        if(mb_strpos($text, 'آموزش') !== false || mb_strpos($text, 'برنامه') !== false || mb_strpos($text, 'کیف پول') !== false || mb_strpos($text, 'شارژ') !== false){
            return 'primary'; // blue
        }
        if(mb_strpos($text, 'تست') !== false || mb_strpos($text, 'جستجو') !== false || mb_strpos($text, 'موجود') !== false){
            return 'primary'; // blue
        }
    ...
    function deltaApplyInlineButtonStyles(&$rows){
        ...
        foreach($rows as &$row){
            ...
            foreach($row as &$btn){
                ...
                $style = deltaInlineButtonStyle($btn);
                if($style !== null && !isset($btn['style'])){
                    $btn['style'] = $style;
                }
            }
            unset($btn);
