...
function getUserOrderDetailKeys($id, $offset = 0){
    ...
    if($order->num_rows==0){
        ...
        $subLinkUrl = resolveOrderSubscriptionLink($order);
        $subLink = !empty($subLinkUrl) ? "<code>" . $subLinkUrl . "</code>" : "";
    ...
    if($delete == 1){
        ...
        if($serverType == "sanaei" || $serverType == "alireza"){
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS => (is_array($dataArr) ? http_build_query($dataArr) : $dataArr),
...
function addInboundAccount($server_id, $client_id, $inbound_id, $expiryTime, $remark, $volume, $limitip = 1, $newarr = '', $planId = null){
    ...
    $row = findInboundRowById($response->obj, $inbound_id);
    if(!$row) return "inbound not Found";

    $iid = $row->id;
    $protocol = $row->protocol;
    $settings = json_decode($row->settings, true);
    if(!is_array($settings)) $settings = [];
    if(!isset($settings['clients']) || !is_array($settings['clients'])) $settings['clients'] = [];
    ...
    if($newarr === ''){
        ...
        if($protocol != 'trojan' && !in_array($serverType, ['sanaei','alireza'], true)) $newClient['flow'] = '';
        if(($serverType == 'sanaei' || $serverType == 'alireza') && $reality == 'true'){
            ...
            if(!empty($flow)) $newClient['flow'] = $flow;
        }
    }else{
        $newClient = is_array($newarr) ? $newarr : [];
        if(!isset($newClient['subId']) || empty($newClient['subId'])) $newClient['subId'] = $generatedSubId;
        $generatedSubId = $newClient['subId'];
        if(!isset($newClient['email'])) $newClient['email'] = $remark;
    }
    ...
    $serverPass = $server_info['password'];
    $loginUrl = $panel_url . '/login';
    $postFields = array('username' => $serverName, 'password' => $serverPass);
    ...
    $loginRaw = curl_exec($curl);

    $header_size = curl_getinfo($curl, CURLINFO_HEADER_SIZE);
    $header = substr($loginRaw, 0, $header_size);
    $body = substr($loginRaw, $header_size);
    preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $header, $matches);
    $cookies = array();
    foreach($matches[1] as $item) {
        parse_str($item, $cookie);
    ...
    $loginResponse = json_decode($body, true);
    if(!$loginResponse || empty($loginResponse['success'])){
        curl_close($curl);
        return (object) ['success'=>false, 'msg'=>($loginResponse['msg'] ?? 'login failed')];
    }

    $apiClientPayload = array(
        'id' => (int) $iid,
        'settings' => json_encode(['clients' => [$newClient]], JSON_UNESCAPED_SLASHES)
    );

    $attempts = [
        [
            'url' => $panel_url . '/panel/api/inbounds/addClient',
            'postfields' => http_build_query($apiClientPayload),
            'headers' => array_filter([
                'User-Agent: Mozilla/5.0',
                'Accept: application/json, text/plain, */*',
                'Content-Type: application/x-www-form-urlencoded; charset=UTF-8',
                'X-Requested-With: XMLHttpRequest',
                !empty($cookieHeader) ? 'Cookie: ' . $cookieHeader : null
            ])
        ],
        [
            'url' => ($serverType == 'sanaei' ? $panel_url . '/panel/inbound/addClient/' : $panel_url . '/xui/inbound/addClient/'),
            'postfields' => http_build_query($apiClientPayload),
            'headers' => array_filter([
                'User-Agent: Mozilla/5.0',
                'Accept: application/json, text/plain, */*',
    ...
    foreach($attempts as $attempt){
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => $attempt['postfields'],
            CURLOPT_SSL_VERIFYHOST => false,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HEADER => false,
        ...
        $raw = curl_exec($curl);
        $decoded = normalizeXuiResponse(json_decode($raw));
        if($decoded && (!isset($decoded->success) || $decoded->success)){
            curl_close($curl);
            return (object) ['success'=>true, 'sub_link'=>'/sub/' . $generatedSubId];
        }
        if($decoded) $lastDecoded = $decoded;
        if(xuiClientExistsInInbound($server_id, $iid, $client_id, $remark)){
            curl_close($curl);
            return (object) ['success'=>true, 'sub_link'=>'/sub/' . $generatedSubId];
        }
    }
    ...
    return $lastDecoded ?: (object) ['success'=>false, 'msg'=>'failed to add client'];
}
...
function normalizeXuiResponse($response){
    if(is_null($response)) return null;
    if(is_array($response)) return (object) ['success'=>true, 'obj'=>$response, 'msg'=>''];
    if(is_object($response)){
        if(isset($response->obj) || isset($response->success) || isset($response->msg)) return $response;
        if(isset($response->data)) return (object) ['success'=>($response->success ?? true), 'obj'=>$response->data, 'msg'=>($response->msg ?? '')];
        return (object) ['success'=>true, 'obj'=>$response, 'msg'=>''];
    }
    return null;
}
function buildPanelSubscriptionLink($panelUrl, $tokenOrPath){
    if(empty($tokenOrPath)) return '';
    $panelUrl = rtrim($panelUrl, '/');
    if(preg_match('/^https?:\/\//i', $tokenOrPath)) return $tokenOrPath;
    if(strpos($tokenOrPath, '/sub/') !== false){
        $path = substr($tokenOrPath, strpos($tokenOrPath, '/sub/'));
        return $panelUrl . $path;
    }
    return $panelUrl . '/sub/' . ltrim($tokenOrPath, '/');
}
function extractSubscriptionToken($tokenOrPath){
    if(empty($tokenOrPath)) return '';
    if(strpos($tokenOrPath, '/sub/') !== false){
        return trim(substr($tokenOrPath, strpos($tokenOrPath, '/sub/') + 5), '/');
    }
    $parsed = parse_url($tokenOrPath, PHP_URL_PATH);
    if(!empty($parsed) && strpos($parsed, '/sub/') !== false){
        return trim(substr($parsed, strpos($parsed, '/sub/') + 5), '/');
    }
    return trim($tokenOrPath, '/');
}
function getOrderSubscriptionLinkByServer($server_id, $tokenOrPath){
    global $connection;
    if(empty($tokenOrPath)) return '';
    $stmt = $connection->prepare("SELECT `panel_url` FROM `server_config` WHERE `id`=?");
    $stmt->bind_param('i', $server_id);
    $stmt->execute();
    $panelUrl = $stmt->get_result()->fetch_assoc()['panel_url'] ?? '';
    $stmt->close();
    if(empty($panelUrl)) return '';
    return buildPanelSubscriptionLink($panelUrl, $tokenOrPath);
}
function resolveOrderSubscriptionLink($order){
    global $connection;
    if(empty($order) || !is_array($order)) return '';
    $server_id = (int)($order['server_id'] ?? 0);
    if($server_id <= 0) return '';

    $stmt = $connection->prepare("SELECT `panel_url`,`type` FROM `server_config` WHERE `id`=?");
    $stmt->bind_param('i', $server_id);
    $stmt->execute();
    $server = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if(empty($server['panel_url'])) return '';
    $panelUrl = $server['panel_url'];

    $resp = getJson($server_id);
    if(isset($resp->obj)){
        $rows = is_array($resp->obj) ? $resp->obj : (array) $resp->obj;
        $targetRow = null;
        $inbound_id = $order['inbound_id'] ?? 0;
        if(!empty($inbound_id)) $targetRow = findInboundRowById($rows, $inbound_id);
        if(!$targetRow){
            foreach($rows as $row){
                $clients = json_decode($row->settings ?? '{}', true)['clients'] ?? [];
                foreach($clients as $client){
                    if(!is_array($client)) $client = (array) $client;
                    if((!empty($order['uuid']) && (($client['id'] ?? null) == $order['uuid'] || ($client['password'] ?? null) == $order['uuid'] || ($client['email'] ?? null) == $order['uuid'])) || (!empty($order['remark']) && ($client['email'] ?? null) == $order['remark']) || (($row->remark ?? null) == ($order['remark'] ?? null))){
                        $targetRow = $row;
                        break 2;
                    }
                }
        ...
        if($targetRow){
            $subId = getClientSubIdFromInboundRow($targetRow, $order['uuid'] ?? null, $order['remark'] ?? null);
            if(!empty($subId)){
                $newToken = extractSubscriptionToken($subId);
                if(!empty($newToken) && ($order['token'] ?? '') !== $newToken && !empty($order['id'])){
                    $stmt = $connection->prepare("UPDATE `orders_list` SET `token`=? WHERE `id`=?");
                    $oid = (int)$order['id'];
                    $stmt->bind_param('si', $newToken, $oid);
                    $stmt->execute();
                    $stmt->close();
                }
                return buildPanelSubscriptionLink($panelUrl, $subId);
            }
        }
    }

    return buildPanelSubscriptionLink($panelUrl, $order['token'] ?? '');
}
function getClientSubIdFromInboundRow($row, $client_id = null, $remark = null){
    $settingsRaw = $row->settings ?? '';
    $settings = is_string($settingsRaw) ? json_decode($settingsRaw, true) : (array) $settingsRaw;
    $clients = $settings['clients'] ?? [];
    foreach($clients as $client){
        if(!is_array($client)) $client = (array) $client;
        $matched = false;
        if(!empty($client_id) && (($client['id'] ?? null) == $client_id || ($client['password'] ?? null) == $client_id || ($client['email'] ?? null) == $client_id)) $matched = true;
        if(!$matched && !empty($remark) && (($client['email'] ?? null) == $remark)) $matched = true;
        if($matched) return $client['subId'] ?? '';
    }
    if(isset($clients[0])){
        $first = is_array($clients[0]) ? $clients[0] : (array) $clients[0];
        return $first['subId'] ?? '';
    }
    return '';
}
function findInboundRowById($rows, $inbound_id){
    if(is_object($rows)) $rows = (array) $rows;
    if(!is_array($rows)) return null;
    foreach($rows as $row){
        if(is_object($row) && (string)($row->id ?? '') === (string)$inbound_id) return $row;
        if(is_array($row) && (string)($row['id'] ?? '') === (string)$inbound_id) return (object)$row;
    }
    return null;
}
function xuiClientExistsInInbound($server_id, $inbound_id, $client_id = null, $remark = null){
    $resp = getJson($server_id);
    if(!is_object($resp) || !isset($resp->obj)) return false;
    $row = findInboundRowById($resp->obj, $inbound_id);
    if(!$row) return false;
    $settingsRaw = $row->settings ?? '';
    $settings = is_string($settingsRaw) ? json_decode($settingsRaw, true) : (array) $settingsRaw;
    $clients = $settings['clients'] ?? [];
    foreach($clients as $client){
        if(!is_array($client)) $client = (array) $client;
        if(!empty($client_id) && (($client['id'] ?? null) == $client_id || ($client['password'] ?? null) == $client_id || ($client['email'] ?? null) == $client_id)) return true;
        if(!empty($remark) && (($client['email'] ?? null) == $remark)) return true;
    }
    return false;
}
function getJson($server_id){
    ...
    $urls = [
        ['url' => $panel_url . '/panel/api/inbounds/list', 'method' => 'GET'],
        ['url' => ($serverType == 'sanaei' ? $panel_url . '/panel/inbound/list' : $panel_url . '/xui/inbound/list'), 'method' => 'POST']
    ];
    ...
    foreach($urls as $item){
        curl_setopt_array($curl, array(
            ...
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => $item['method'],
            CURLOPT_POST => ($item['method'] === 'POST'),
            CURLOPT_HTTPGET => ($item['method'] === 'GET'),
    ...
    curl_close($curl);
    return (object) ['success'=>false, 'msg'=>'inbounds list unavailable'];
}
function getNewCert($server_id){
    ...
    $attempts = [
        $panel_url . '/panel/api/server/getNewX25519Cert',
        $panel_url . '/server/getNewX25519Cert'
    ];
    ...
    curl_close($curl);
    return (object) ['success'=>false, 'msg'=>'unable to get certificate'];
}
...
//-------------------------------------- vmess vless -------------------------------//
  ...
  "grpcSettings": {
    ...
    $attempts = [
        ...
        ($serverType == 'sanaei' ? $panel_url . '/panel/inbound/add' : $panel_url . '/xui/inbound/add')
    ];
    ...
    foreach($attempts as $url){
        ...
        $decoded = normalizeXuiResponse(json_decode($raw));
        if($decoded && (!isset($decoded->success) || $decoded->success)){
            $verify = getJson($server_id);
            if(isset($verify->obj) && is_array($verify->obj)){
                foreach($verify->obj as $inboundRow){
                    if(($inboundRow->remark ?? '') == $remark && (string)($inboundRow->port ?? '') === (string)$port){
                        $subId = getClientSubIdFromInboundRow($inboundRow, $client_id, $remark);
                        curl_close($curl);
                        return (object) ['success'=>true, 'sub_link'=>(!empty($subId)?'/sub/' . $subId:'')];
                    }
                }
            }
            curl_close($curl);
    ...
    curl_close($curl);
    return $lastDecoded ?: (object) ['success'=>false, 'msg'=>'failed to add inbound'];
}
