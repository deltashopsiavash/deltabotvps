...
", $configLinks);
        $allConfigLen = function_exists('mb_strlen') ? mb_strlen($allConfigText, 'UTF-8') : strlen($allConfigText);
        // Telegram copy_text has a strict size limit. If it is too long, using copy_text
        // makes sendPhoto/sendMessage fail and the user receives nothing after approval.
        if($lookupKey !== '' && $allConfigLen > 256){
            $rows[] = [[
                'text' => '📋 کپی کانفیگ',
