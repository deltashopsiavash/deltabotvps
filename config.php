...
function ensureUserAccessSchema(){
    global $connection;
    static $checked = false;
    if($checked) return;
    addColumnIfMissing('users', 'access_status', "`access_status` varchar(20) NOT NULL DEFAULT 'approved' AFTER `first_start`");
    addColumnIfMissing('users', 'access_updated_at', '`access_updated_at` int(11) NOT NULL DEFAULT 0 AFTER `access_status`');
    $checked = true;
}
ensureUserAccessSchema();
...
function getBotSettingKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ['text'=>(($botState['startApprovalState'] ?? 'off')=='on'?$buttonValues['on']:$buttonValues['off']),'callback_data'=>"changeBotstartApprovalState"],
            ['text'=>"تأیید استارت توسط مدیر",'callback_data'=>"deltach"]
        ],
        [
...
function setUser($value = 'none', $field = 'step'){
    ...
    elseif($field == "access_status") $stmt = $connection->prepare("UPDATE `users` SET `access_status` = ? WHERE `userid` = ?");
    elseif($field == "access_updated_at") $stmt = $connection->prepare("UPDATE `users` SET `access_updated_at` = ? WHERE `userid` = ?");
