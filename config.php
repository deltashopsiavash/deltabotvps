...
function editPasarguardUser($server_id, $remark, $fields){
    ...
    if(!$server_info)return(object)['success'=>false,'msg'=>'server not found'];
    $token=getPasarguardToken($server_id);if(empty($token->success))return(object)['success'=>false,'msg'=>$token->msg??'PasarGuard token error'];
    $paths=['/api/user/by-username/'.rawurlencode($remark),'/api/users/by-username/'.rawurlencode($remark),'/api/user/'.rawurlencode($remark),'/api/users/'.rawurlencode($remark)];$last='';$lastHttp=0;
    foreach(pasarguardPanelApiBases($server_info['panel_url']) as $base){foreach($paths as $path){foreach(['PATCH','PUT'] as $method){$c=curl_init();curl_setopt_array($c,[CURLOPT_URL=>$base.$path,CURLOPT_CUSTOMREQUEST=>$method,CURLOPT_RETURNTRANSFER=>true,CURLOPT_CONNECTTIMEOUT=>10,CURLOPT_TIMEOUT=>30,CURLOPT_SSL_VERIFYHOST=>false,CURLOPT_SSL_VERIFYPEER=>false,CURLOPT_HTTPHEADER=>['Accept: application/json','Content-Type: application/json','Authorization: Bearer '.$token->access_token],CURLOPT_POSTFIELDS=>json_encode($fields,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]);$raw=curl_exec($c);$err=curl_error($c);$http=curl_getinfo($c,CURLINFO_HTTP_CODE);curl_close($c);$lastHttp=$http;if($err){$last=$err;continue;}if($http>=200&&$http<300)return(object)['success'=>true,'obj'=>json_decode($raw)];$last=trim((string)$raw)!==''?$raw:'HTTP '.$http;if($http===401||$http===403)return(object)['success'=>false,'msg'=>$last,'http'=>$http];}}}return(object)['success'=>false,'msg'=>$last?:'PasarGuard update failed','http'=>$lastHttp];
}
...
function removePasarguardUser($server_id, $remark){
    ...
    return (object)['success'=>($http == 204 || ($http >= 200 && $http < 300)), 'msg'=>$raw];
}
...
function pgRenewApply($orderId, $days, $volume){
    ...
    $info=getPasarguardUserInfo($server_id,$remark);
    if(!is_object($info) || isset($info->success) && $info->success===false) return (object)['success'=>false,'msg'=>$info->msg ?? 'PasarGuard user not found'];
    $fields=[];
    if($days>0){$current=adminExpiredNormalizeEpoch($info->expire ?? $info->expiry_time ?? $info->expires_at ?? 0);$base=max($current,time());$fields['expire']=gmdate('c',$base+((int)$days*86400));}
    if($volume>0){$currentLimit=(float)($info->data_limit ?? $info->traffic_limit ?? 0);$fields['data_limit']=(int)$currentLimit+(int)floor(((float)$volume)*1073741824);}
    $fields['status']='active';
    $res=editPasarguardUser($server_id,$remark,$fields);
