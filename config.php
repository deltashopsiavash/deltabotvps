...
// Import a mysqldump .sql file into a database using mysqli::multi_query
// Supports standard mysqldump output (DROP/CREATE/INSERT/LOCK/UNLOCK).
function importSqlDumpToDb($mysqli, $dbName, $filePath){
    if(!is_file($filePath)) return false;
    $sql = @file_get_contents($filePath);
    if($sql === false || trim($sql) === '') return false;

    // Remove "CREATE DATABASE" / "USE" statements if present (we select db ourselves)
    $sql = preg_replace('/^\s*CREATE\s+DATABASE.*?;\s*$/mi', '', $sql);
    $sql = preg_replace('/^\s*USE\s+`?[^`;\s]+`?\s*;\s*$/mi', '', $sql);

    if(!$mysqli->select_db($dbName)) return false;
...
function ensureResellerBotDatabase($rid){
    ...
    // Prefer importing reseller template (full config) if available.
    // This ensures new features (menus/settings) exist on reseller bots after creation.
    $templateFile = __DIR__ . "/settings/reseller_template.sql";
    $imported = false;
    if(is_file($templateFile)){
        $imported = importSqlDumpToDb($connection, $newDb, $templateFile);
    }

    if(!$imported){
        // Fallback: Clone schema for all tables except reseller tables (avoid recursion)
...
// If this is a child bot but it has no dedicated database configured,
// we must NOT fall back to mother DB (danger: backups/restores would affect mother).
// We keep a flag so bot.php can show a helpful error and exit early.
$childDbNotReady = false;
if(isset($isChildBot) && $isChildBot){
    if(empty($childBotRow['db_name'])){
        $childDbNotReady = true;
    }
}
...
$range = [
        // Telegram Bot API webhook IP ranges (may change over time)
        '149.154.160.0/20',
        '91.108.4.0/22',
        '91.108.8.0/22',
        '91.108.16.0/22',
...
// Collect ALL possible client IPs behind reverse proxies (Cloudflare / Nginx / etc.)
// We will validate against Telegram ranges using ANY of these IPs.
...
function check($return = false){
    global $range;
    $ips = function_exists('get_client_ips') ? get_client_ips() : [($_SERVER['REMOTE_ADDR'] ?? '0.0.0.0')];
    ...
    // Accept if ANY candidate IP matches Telegram ranges.
    foreach($ips as $ip){
        foreach ($range as $rg) {
...
function getAdminKeys(){
    ...
    // QR Code background image (mother/reseller)
    if(true){
        $rows[] = [
    ...
    // Cloud VPS management (toggle from Bot Settings)
    global $botState;
    $cloudOn = ($botState['cloudServerState'] ?? 'on') === 'on';
    if(!$isChildBot && $cloudOn){
        $rows[] = [
...
function getRejectedAgentList(){
    ...
    if($list->num_rows>0){
        ...
        while($row = $list->fetch_assoc()){
	    	...
	    	$keys[] = [
	    	    ['text'=>$cloudServerState,'callback_data'=>'toggleCloudServerState'],
