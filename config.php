...
function forwardUpdateToVpsbot($raw){
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, $raw);
        $tokenHdr = $GLOBALS['botToken'] ?? ($GLOBALS['bot_token'] ?? null);
    $adminHdr = $GLOBALS['admin'] ?? null;
    $headers = ["Content-Type: application/json"];
    if($tokenHdr) $headers[] = "X-Bot-Token: ".$tokenHdr;
    if($adminHdr) $headers[] = "X-Admin-Id: ".$adminHdr;
