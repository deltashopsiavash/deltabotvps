...
function ensureResellerBotDatabase($rid){
    ...
    $row = $connection->query("SELECT `id`,`db_name`,`admin_userid` FROM `reseller_bots` WHERE `id`={$rid} LIMIT 1");
    ...
    // Connect to new DB
    $childConn = new mysqli('localhost', $dbUserName, $dbPassword, $newDb);
    if($childConn->connect_error){
        return false;
    }
    $childConn->set_charset('utf8mb4');

    // 1) Initialize DB with a bundled template backup if present.
    // This ensures reseller bots start with the exact same structure/config as the mother (including menu buttons).
    $tpl = __DIR__ . "/reseller_template.sql";
    if(file_exists($tpl) && filesize($tpl) > 0){
        $sql = file_get_contents($tpl);

        // Remove mysqldump comments and DELIMITER directives (basic, enough for our template)
        $sql = preg_replace('/^\s*--.*$/m', '', $sql);
        $sql = preg_replace('/^\s*\/\*![\s\S]*?\*\/\s*$/m', '', $sql);
        $sql = preg_replace('/^\s*DELIMITER\s+.*$/m', '', $sql);

        // Execute as multi query
        if($childConn->multi_query($sql)){
            do {
                // flush results
                if($res = $childConn->store_result()){
                    $res->free();
                }
            } while($childConn->more_results() && $childConn->next_result());
        }
    }else{
        // 2) Fallback: clone schema from mother DB
        ...
        if($tablesRes){
            ...
            foreach($tables as $t){
                ...
                $childConn->query("CREATE TABLE IF NOT EXISTS `{$t}` LIKE `{$mainDbName}`.`{$t}`");
            }
        }
        // Copy "configuration" tables (correct table names for this project)
        $copyDataTables = [
            'admins',
            'setting',
            'servers',
        ...
        foreach($copyDataTables as $t){
            // Only copy if the table exists in mother db
            ...
            if($ex !== 1) continue;
            @$childConn->query("TRUNCATE TABLE `{$t}`");
            @$childConn->query("INSERT INTO `{$t}` SELECT * FROM `{$mainDbName}`.`{$t}`");
        }
    }

    // Clean transactional tables for a fresh reseller bot
    $toTruncate = ['users','orders_list','pays','chats','chats_info','server_info'];
    foreach($toTruncate as $t){
        @$childConn->query("TRUNCATE TABLE `{$t}`");
    }

    // Ensure reseller admin exists as admin user in child DB (for panels / broadcasts / etc.)
    $aid = (int)($rb['admin_userid'] ?? 0);
    if($aid > 0){
        // users table
        @$childConn->query("INSERT IGNORE INTO `users` (`userid`,`username`,`name`,`step`,`add_date`,`isAdmin`,`wallet`,`is_agent`) VALUES ({$aid}, '', '', 'none', ".time().", 1, 0, 0)");
        // admins table (if exists)
        $q = $childConn->query("SELECT COUNT(*) c FROM information_schema.TABLES WHERE TABLE_SCHEMA='{$newDb}' AND TABLE_NAME='admins'");
        $ex = $q ? (int)($q->fetch_assoc()['c'] ?? 0) : 0;
        if($ex === 1){
            @$childConn->query("TRUNCATE TABLE `admins`");
            @$childConn->query("INSERT INTO `admins` (`userid`) VALUES ({$aid})");
        }
    }
...
// Return per-bot QR background image path.
// For mother bot: settings/qrcodes/qr_main.jpg
// For reseller bot (bid): settings/qrcodes/qr_rb<bid>.jpg
function getQrBackgroundPath($absolute = false){
    global $isChildBot, $currentBotInstanceId;
    $id = $isChildBot ? ('rb' . (int)$currentBotInstanceId) : 'main';
    $rel = "settings/qrcodes/qr_" . $id . ".jpg";
    if($absolute){
        return __DIR__ . "/" . $rel;
    }
    return $rel;
...
function getQrBackgroundPathForLoad(){
    ensureQrDir();
    $abs = getQrBackgroundPath(true);
    if(file_exists($abs)) return $abs;
    $legacy = __DIR__ . "/settings/QRCode.jpg";
...
// Cloud VPS buy button (ONLY on mother bot)
if(!$isChildBot){
    $mainKeys = array_merge($mainKeys,[
...
function getAdminKeys(){
    ...
    $rows[] = [
        ['text'=>'🖼 تغییر تصویر QRCODE','callback_data'=>'changeQrBg'],
        ['text'=>' ','callback_data'=>'deltach'],
    ];
...
function getPanelManagementKeys(){
    ...
    // Cloud VPS management (ONLY on mother bot) + toggle from Bot Settings
    global $isChildBot, $connection;
    $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'BOT_STATES'");
    $stmt->execute();
    $botState = $stmt->get_result()->fetch_assoc()['value'];
    $stmt->close();
    $botState = !is_null($botState) ? json_decode($botState,true) : [];
    $cloudEnabled = (($botState['cloudServerState'] ?? 'on') == 'on');

    if(!$isChildBot && $cloudEnabled){
        $rows[] = [
...
function getRejectedAgentList(){
    ...
    if($list->num_rows>0){
        ...
        $keys[] = [
            ['text'=>$cloudServerState,'callback_data'=>"changeBotcloudServerState"],
...
function getBotSettingKeys(){
    ...
    $cloudServerState = (($botState['cloudServerState']??'on')=='on')?$buttonValues['on']:$buttonValues['off'];
