...
function bot($method, $datas = []){
    ...
    if($curlError === '' && isset($datas['reply_markup']) && deltaMarkupHasPremiumIcons($datas['reply_markup']) && is_object($decoded) && empty($decoded->ok)){
        $description = strtolower((string)($decoded->description ?? ''));
        if(strpos($description,'emoji') !== false || strpos($description,'icon_custom_emoji_id') !== false){
            $datas['reply_markup'] = deltaRemovePremiumIconsFromMarkup($datas['reply_markup']);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $hasFile ? $datas : http_build_query($datas));
...
$buttonIconValues = [];
$stmt = $connection->prepare("SELECT `type`,`value` FROM `setting` WHERE `type` LIKE 'BUTTON_LABEL_%' OR `type` LIKE 'BUTTON_ICON_%' OR `type` LIKE 'MAINVALUE_%'");
$stmt->execute();
