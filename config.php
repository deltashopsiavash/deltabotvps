...
function xuiBuildOrderFallbackButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if((($botState['subLinkState'] ?? 'on') !== 'off') && $subLink !== ''){
        // Telegram's native Copy Code button is client-side and may not appear for short one-line links.
        // This visible inline button copies the subscription directly on supported Telegram clients.
        $rows[] = [[ 'text'=>'📋 کپی مستقیم سابسکریپشن', 'copy_text'=>['text'=>$subLink] ]];
        if($lookupKey !== '') $rows[] = [[ 'text'=>'🌐 ارسال سابسکریپشن', 'callback_data'=>'xuiCopySub_' . $lookupKey ]];
    }
...
function xuiBuildOrderCopyButtons($configLinks = [], $subLink = '', $backCallback = 'mainMenu', $lookupKey = ''){
    ...
    if((($botState['subLinkState'] ?? 'on') !== 'off') && $subLink !== ''){
        // This is the important fix: unlike callback buttons, copy_text copies instantly.
        // It also solves Telegram clients that do not show the small "COPY CODE" UI for one-line subscription links.
        $rows[] = [[
            'text' => '📋 کپی مستقیم سابسکریپشن',
            'copy_text' => ['text' => $subLink]
        ]];
        if($lookupKey !== ''){
            $rows[] = [[
                'text' => '🌐 ارسال سابسکریپشن',
