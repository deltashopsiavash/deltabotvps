...
function xuiBuildOrderDeliveryText($protocol, $remark, $volume, $days, $botState, $serverType, $configLinks, $botUrl, $uniqid, $subLink = ''){
    ...
    $configBlock = xuiBuildConfigBlockHtml($botState, $serverType, $configLinks);
    if($configBlock !== '') $parts[] = $configBlock;
        ...
        if($inbound_id == 0) {
            ...
            if($protocol == 'vless'){
                ...
                if($netType == 'tcp'){
                    ...
                    else $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus" . $buildWsQuery($path ?? '/', $host ?? '') . "{$psting}#$remark";
                }
                elseif($netType == 'kcp')
